...
| basemap-yields-to-the-ink |  |  |  |  |  |  |  | **R** | **R** |  |  | **R** |  |  |
...
## Why a cell is blank, where the blankness was argued — 23 of them
...
- `scrolly` cannot reach **frame-covers-the-box** — measured 2026-08-23: this vehicle asks the same question and answers it at RUNTIME, not at bake time. proof/mapscrolly-one-map-europe-carbon/map-drive.mjs resolveCamera interpolates between contain and cover against the box it is actually being drawn in, on every scroll frame ("COVER, not contain: the target box FILLS the frame ... A contain fit would letterbox a near-square European plate inside a wide frame and leave a third of the picture as bare ground"), and it moves the CAMERA rather than the plate — so there is no fixed frame for a bake-time refusal to be about, and a plate whose margin ran out is answered by a shallower scale rather than by a crop. The bake this rule guards is the one a map-web page cannot redo, because a map-web page must be right with JavaScript off. Structural, not incidental: giving scrolly this refusal would require it to declare, at bake time, a box range its own scroll states are free to leave.
...
### basemap-yields-to-the-ink — `plateSurfacesYieldToInk`

**Refuses:** a baked plate whose own flat fills carry more contrast against the story's ground than the ink the beat draws with, or a fill a mark drawn on it cannot be seen against

**Earned by:** THE OWNER'S INSTRUCTION, GIVEN TWICE: "the ocean colours have to adapt to the palette." It was built for the CHOROPLETH bake (`waterFor`/`noDataFor`/`offRampLuminance` in `map-web/assets/geo-choropleth.ts`) and the SYMBOL bake never got it. The 2026-08-23 full-width change made the consequence impossible to miss: `stories/r8-map-web-japan-bear-casualties`, a map of Japan on a newsroom ground of #16191B (relative luminance 0.0094), delivered a plate that is 66.9% water at the literal #aac9e0 (0.5570) and 32.8% land at #292929 (0.0222). The sea measured 10.22:1 against the ground while the accent carrying the whole argument measured 8.02:1, and the accent over the sea measured 1.27:1 against a 3:1 non-text floor — the largest and brightest thing on the page carried no data at all, and the subject read as a hole cut out of a bright blue page. `plateFollowsGround` PASSED it, and not by a tuning slip: that decision reads the plate's MEAN, the mean of that plate is 0.3809, and 0.3809 sits inside the 0.25-0.6 band it deliberately says nothing about. A symbol plate is mostly basemap, so its mean is a number no bake controls and no reader ever sees. A mean is the wrong statistic for a picture made of a few large flat fills; this decision reads the fills, measured pixel by pixel off the finished plate, and which ink is drawn over which fill is sampled at each mark's own projected pixel rather than typed. Measured across all 26 plates committed in this tree the day it was written: three plates carry a fill that out-shouts their beat's own ink, and the Japan plate is the only one of the three its neighbour did not already refuse

**Also reached by:** scrolly, which bakes a plate and records a palette exactly as the other two do, but reads its delivered page's ground off a live `--ground` custom property and its plate through an `OffscreenCanvas`. Its copy is therefore called from the place in that skill that DOES read a plate off disk — `verifyBeatFiles`, the pass that walks a beat's own plate directories — rather than from the browser pass, and its bake carries the same derivation the other two do so a scrolly plate baked after 2026-08-23 is right by construction rather than only checked afterwards
...
### frame-covers-the-box — `frameCoversTheBoxRange`

**Refuses:** a baked plate whose ocean margin does not reach the shapes the delivered box actually takes, so filling that box would crop the study set instead of the basemap

**Earned by:** the owner, on a real delivered page in a 2990px window: *the map must take all the available width, every time* — and, on the correction that followed, *the height is not an editorial choice either; like the scrolly, it must take all the space available.* stories/r8-map-web-japan-bear-casualties put a 520.1px box in a 1568px container at 1600x900 (33.2%), 428.3 in 1248 at 1280x800 and 1175.7 in 2958 at 2990x1600, and it passed every check: the plate is 1000x1089, a CORRECT bake of a portrait camera, the box was bound on height and filled it, and the frame guard — re-measured the day before from an area to the fraction of the axis the box is bound on, precisely so a correct portrait bake would stop being punished — read 62.9%. The box now takes the whole container on both axes and the plate is drawn to COVER it, which makes the bake responsible for the crop: this decision refuses a frame whose margin does not survive the narrowest and widest boxes the page is measured delivering into. The one study set it cannot be solved for is derived rather than excused — a camera already spanning a full turn of longitude has no more world east or west, so at real-owid-life-expectancy's widest box (2.572:1 against a 1.472:1 world) filling the width would cost 42.7% of the latitude range, and such a plate records cannotCover, keeps the old centred layout, and says so on the verdict
