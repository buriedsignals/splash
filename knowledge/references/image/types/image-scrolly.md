...
- **Use** for: a before/after the reader experiences as a crossfade transition.
...
1. **Every frame carries its own `alt` and `credit`, distinct from its caption.** `alt` describes
   ...
   string. `checkImageConformance` throws on a missing/blank `alt`, an `alt` equal to the
   `caption`, or a missing `credit.name`.
