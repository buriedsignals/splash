...
Splash's skills run natively in [Codex](https://developers.openai.com/codex) through Codex's
...
- **[Bun](https://bun.sh)** — Splash's producers run under Bun (`bun scripts/produce.mjs …`).
- **Git**.
...
1. **Clone the repo:**
   ...
   ```bash
   git clone https://github.com/buriedsignals/splash.git ~/Splash
   ```

2. **Symlink every skill into `~/.agents/skills/`** — Codex discovers skills from
   `~/.agents/skills/<name>/SKILL.md` (user tier). Splash ships nine skills, so we link each one by
   name (not one umbrella link), matching Codex's documented per-skill layout:

   ```bash
   mkdir -p ~/.agents/skills
   for d in ~/Splash/skills/*/; do
   ...
   **Windows (PowerShell):**
   ...
   foreach ($d in Get-ChildItem "$env:USERPROFILE\Splash\skills" -Directory) {
...
4. **Provide your keys.** Splash reads provider keys from the environment when a producer runs
   (`VITE_MAPTILER_KEY`, `REMOTION_MAPTILER_KEY`, `DATAWRAPPER_API_TOKEN`, optional
   `SPLASH_EMBED_APP` / `FLY_API_TOKEN`). The bootstrap installer writes these to `~/Splash/.env`;
   when running Codex by hand, export them (or `set -a && . ~/Splash/.env && set +a`) before
...
Launch Codex from `~/Splash` and list skills:
...
You should see all nine Splash skills:
`splash`, `chart-native`, `dw-chart`, `image-native`, `map-dw`, `map-native`, `scrolly`,
`suggest-article`, `suggest-chart`.

Start a project by describing your article to the `splash` skill (auto-matches by description, or
invoke it explicitly with `$splash`). It orchestrates the rest.
...
```bash
for d in ~/Splash/skills/*/; do rm -f ~/.agents/skills/"$(basename "$d")"; done
```

Optionally delete the clone (`rm -rf ~/Splash`) and remove the Splash stanza from
`~/.codex/config.toml`.
