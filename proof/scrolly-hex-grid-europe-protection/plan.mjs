 ...
 */
export function hexMapPlan({ hosts, origin, colours, strokes, camera, referenceWidth, referenceHeight }) {
  const codes = hosts.map(iso2Of);
  const small = codes.filter((c) => SMALL_BELOW_Z4.includes(c));
  const originCode = iso2Of(origin);
  ...
  const layers = [
    ...
    fillLayer("origin", [originCode], colours.originFill),
    {
      id: "origin-edge",
      type: "line",
      beneath: "water",
