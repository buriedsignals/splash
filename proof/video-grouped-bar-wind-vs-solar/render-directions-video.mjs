// « Dans 5 de ces 6 pays l’éolien devance le solaire, la Suisse est l’exception » — rendered as video once per filed direction: the scrolly's picture told
...
//
// Usage:  bun proof/video-grouped-bar-wind-vs-solar/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = "video-grouped-bar-wind-vs-solar-landscape";
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(GroupedBarFrame, { ...props, at: frame })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const envDir = await mkdtemp(join(tmpdir(), "video-grouped-bar-env-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · bar ${report.barW}px`,
    );
    ...
    if (lookDir) {
      ...
      const frames = [
        ...
        { name: "first", frame: 0 },
        { name: "reference-mix", frame: Math.round(T.reference.start + T.reference.duration * 0.7) },
        ...[0.15, 0.45, 0.8].map((t) => ({ name: `reveal-${t}`, frame: Math.round(T.reveal.start + T.reveal.duration * t) })),
        { name: "subject-compare", frame: Math.round(T.subject.start + T.subject.duration * 0.6) },
      ];
