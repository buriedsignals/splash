...
describe("statesFor", () => {
  it("should open on the title card alone, then each country's whole mix", () => {
    expect(establish).toEqual({ title: 1, furniture: 0, mix: 0, others: 0, split: 0, camera: 0, compare: 0, focus: 0, source: 0 });
    expect(reference).toEqual({ ...establish, title: 0, furniture: 1, mix: 1 });
  });

  it("should take wind and solar out of the mix and close the scale at reveal, then compare at subject", () => {
    expect(reveal).toEqual({ ...reference, others: 1, split: 1, camera: 1 });
    expect(subject).toEqual({ ...reveal, compare: 1 });
  });

  it("should keep the exception with the credit at conclusion, and hold it exactly", () => {
