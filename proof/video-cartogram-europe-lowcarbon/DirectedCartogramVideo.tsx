/**
 * The Remotion composition: `CartogramFrame` at the current frame, drawn only once the embedded faces are in,
...
export type DirectedCartogramVideoProps = CartogramFrameProps & { faces: EmbeddedFace[] };
...
export function DirectedCartogramVideo(props: DirectedCartogramVideoProps) {
  ...
  return <AbsoluteFill style={{ backgroundColor: props.colours?.ground }}>{ready ? <CartogramFrame {...props} at={frame} svgRef={ref} /> : null}</AbsoluteFill>;
}
