...
import { HISTOGRAM_VIDEO_TIMING as T } from "./timing-contract";

const broken = (patch: Partial<BeatTiming>): BeatTiming => ({ ...T, ...patch });

describe("the shipped histogram video timing", () => {
  ...
  it("should refuse a subject that starts before the reveal has finished", () => {
    expect(checkTiming(broken({ subject: { start: T.subject.start - 1, duration: T.subject.duration } })).join(" ")).toContain(`subject starts at ${T.subject.start - 1}`);
  });
});
