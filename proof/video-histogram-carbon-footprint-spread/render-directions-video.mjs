// « 6 pays sur 10 émettent moins de 4 tonnes de CO₂ par personne » — rendered as video once per filed direction: the histogram
// piled from its countries, its tail stacked against its first bin (BRIEF.md). Everything is measured and asserted in Bun (`build.mjs`); the markup of every event's last
...
//
// Usage:  bun proof/video-histogram-carbon-footprint-spread/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = "video-histogram-carbon-footprint-spread-landscape";
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(HistogramFrame, { ...props, at: frame })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const envDir = await mkdtemp(join(tmpdir(), "video-histogram-env-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · bin ${report.binBand}px · ${report.unit}px a country`,
    );
    ...
    if (lookDir) {
      ...
      const frames = [
        ...
        { name: "first", frame: 0 },
        { name: "reference-rug", frame: Math.round(T.reference.start + T.reference.duration * 0.6) },
        { name: "reveal-falling", frame: Math.round(T.reveal.start + T.reveal.duration * 0.3) },
        { name: "subject-stacking", frame: Math.round(T.subject.start + T.subject.duration * 0.45) },
        { name: "subject-tenths", frame: Math.round(T.subject.start + T.subject.duration * 0.9) },
        { name: "conclusion-return", frame: Math.round(T.conclusion.start + T.conclusion.duration * 0.55) },
      ];
