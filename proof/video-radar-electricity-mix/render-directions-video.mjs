// « La France et l’Allemagne produisent presque autant d’électricité, avec des mix opposés » — rendered as video once per filed
...
//
// Usage:  bun proof/video-radar-electricity-mix/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = "video-radar-electricity-mix-landscape";
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(RadarFrame, { ...props, at: frame })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const envDir = await mkdtemp(join(tmpdir(), "video-radar-env-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · wheel r=${report.radius}px`,
    );
    ...
    if (lookDir) {
      ...
      const frames = [
        ...
        { name: "first", frame: 0 },
        { name: "reveal-cut", frame: mid("reveal", WINDOWS.reveal.cut) },
        { name: "subject-carry", frame: mid("subject", WINDOWS.subject.fly) },
        { name: "conclusion-close", frame: mid("conclusion", WINDOWS.conclusion.close) },
      ];
