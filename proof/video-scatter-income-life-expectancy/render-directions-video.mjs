// « Au-delà de 30 000 $ par personne, l’espérance de vie tient dans une bande 3 fois plus étroite » — rendered as video once per filed
...
//
// Usage:  bun proof/video-scatter-income-life-expectancy/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = "video-scatter-income-life-expectancy-landscape";
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(ScatterFrame, { ...props, at: frame })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const envDir = await mkdtemp(join(tmpdir(), "video-scatter-env-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · dot ${report.radius.toFixed(1)}px in a ${report.plotHeight}px plot · « ${report.yName} »`,
    );
    ...
    if (lookDir) {
      ...
      const frames = [
        ...
        { name: "subject-fold", frame: mid("subject", WINDOWS.subject.fold) },
        { name: "subject-stack", frame: mid("subject", WINDOWS.subject.stack) },
        { name: "conclusion-unfold", frame: mid("conclusion", WINDOWS.conclusion.unfold) },
      ];
