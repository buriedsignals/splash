...
| `reference` | where sixteen countries started, on one scale | **reveal in order → a whole splitting into members** | sixteen pale bars grow side by side in one row on one baseline; the row is cut — each bar, keeping its height, travels to its own panel, its stretch of baseline shrinking to the panel's own, its name arriving | every bar's height = its 2000 share on the one scale, through the cut |
