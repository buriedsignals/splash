// « Les seize ont tous progressé, et ceux qui partaient de plus bas le plus » — rendered as video once per filed direction:
// one row cut into sixteen panels, the gains pulled down and the panels re-sorted by their start (BRIEF.md). Everything is measured and asserted in Bun (`build.mjs`); the markup of every event's last
...
//
// Usage:  bun proof/video-small-multiples-lowcarbon/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = `video-small-multiples-lowcarbon-${SIZE}`;
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(SmallMultiplesFrame, { ...props, at: frame })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const envDir = await mkdtemp(join(tmpdir(), "video-small-multiples-env-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · panel ${report.panel} · air ${report.air}px`,
    );
    // R8 IS A DECISION, SO IT IS REPORTED. A frame that drew every panel says nothing extra — landscape's
    // line is the one it has always printed — and a frame that drew fewer prints the count it took and
    // the exact words the plate carries, so the reduction is never something only the render knows.
    if (report.said) console.log(`  drew ${report.drawn} of ${report.total} panels · « ${report.said} »`);
    ...
    if (lookDir) {
      ...
      const frames = [
        ...
        { name: "first", frame: 0 },
        { name: "reference-row", frame: mid("reference", [WINDOWS.reference.level[1], WINDOWS.reference.cut[0]]) },
        { name: "reference-cut", frame: mid("reference", WINDOWS.reference.cut) },
        { name: "reveal-grow", frame: mid("reveal", WINDOWS.reveal.grow) },
        ...["detach", "reorder"].map((f) => ({ name: `subject-${f}`, frame: mid("subject", WINDOWS.subject[f]) })),
        { name: "conclusion-back", frame: mid("conclusion", WINDOWS.conclusion.back) },
      ];
