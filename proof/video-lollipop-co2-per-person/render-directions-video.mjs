// « La Chine a triplé son CO₂ par personne » — rendered as video once per filed direction: the scrolly's picture told
...
//
// Usage:  bun proof/video-lollipop-co2-per-person/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = "video-lollipop-co2-per-person-landscape";
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(LollipopFrame, { ...props, at: frame })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const envDir = await mkdtemp(join(tmpdir(), "video-lollipop-env-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · pair ${report.apart}px apart`,
    );
    ...
    if (lookDir) {
      ...
      const frames = [
        ...
        { name: "first", frame: 0 },
        { name: "reveal-stacking", frame: Math.round(T.reveal.start + T.reveal.duration * 0.55) },
        { name: "subject-travelling", frame: Math.round(T.subject.start + T.subject.duration * 0.45) },
      ];
