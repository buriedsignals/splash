...
describe("the shipped streamgraph video timing", () => {
  ...
  it("should give each year at least eight frames of the flow", () => {
    expect((T.reveal.duration * 0.92) / 24).toBeGreaterThanOrEqual(8);
  });

  it("should refuse a subject that starts before the reveal has finished", () => {
    expect(checkTiming(broken({ subject: { start: 300, duration: 105 } })).join(" ")).toContain("subject starts at 300");
  });
});
