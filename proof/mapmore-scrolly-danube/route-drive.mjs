...
export function initRouteScrolly(root, config, onCamera) {
  ...
  function paint() {
    ...
    // The line grows by being REDRAWN from the points the river has reached — the halo and the line
    // take the same `d`, so one can never lag the other. It used to be a dash whose offset ran to
    // zero, which draws head, hole and tail the moment the pattern is computed in a space the
    // path's length does not live in; see `MapFrame.tsx` for what that cost. One string built per
    // paint, assigned twice: `drawnSoFar` walks the samples by LENGTH off the same `cum` the dash
    // was measured against, so the head lands exactly where it used to.
