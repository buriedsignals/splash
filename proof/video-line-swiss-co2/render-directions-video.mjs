// « En 2024, la Suisse a émis moins de CO₂ sur son territoire qu’en 1967 » — rendered as video once per filed direction: the scrolly's picture told
...
//
// Usage:  bun proof/video-line-swiss-co2/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = "video-line-swiss-co2-landscape";
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(LineFrame, { ...props, at: frame })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const envDir = await mkdtemp(join(tmpdir(), "video-line-env-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · plot ${Math.round(report.plot.left)}–${Math.round(report.plot.right)} × ${Math.round(report.plot.top)}–${Math.round(report.plot.bottom)}`,
    );
    ...
    if (lookDir) {
      ...
      const frames = [
        ...
        { name: "first", frame: 0 },
        ...[0.35, 0.7].map((t) => ({ name: `reveal-trace-${t}`, frame: Math.round(T.reveal.start + T.reveal.duration * (WINDOWS.reveal.trace[0] + t * (WINDOWS.reveal.trace[1] - WINDOWS.reveal.trace[0]))) })),
      ];
