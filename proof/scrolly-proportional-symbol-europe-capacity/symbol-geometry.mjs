// THE LAND AND THE STATIONS IN ONE COORDINATE SPACE, computed once in node from the frozen files — the geometry of
// `scrolly-dot-density-europe-stations`, whose stations and shapes this beat shares.
//
// PROJECTION. Lambert azimuthal equal-area centred on 52°N 10°E — the projection the sibling map beats measure
// in. A circle's area encodes its capacity, not the ground under it; an equal-area camera keeps the ground honest
// too, so a crowd of circles reads against land drawn at its true size.
//
 ...
 */
export function symbolGeometry(geo, points, { window: win, width }) {
  const border = [];
  for (let i = 0; i <= 120; i++) {
    const t = i / 120;
  ...
  for (const f of geo.features)
    for (const poly of f.geometry.coordinates)
      for (const ring of poly) {
        ...
        if (kept.length >= 3) rings.push(`M${kept.map((p) => `${r1(p[0])} ${r1(p[1])}`).join("L")}Z`);
      }
  ...
  return {
    ...
    margin: M,
    land: rings.join(""),
    place: (p) => toFrame(laea([p.lon, p.lat])).map(r1),
  };
}
