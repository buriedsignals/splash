// proof/web-flow-map-danube/bake.mjs — SCAFFOLDED PLUMBING. The camera below is NOT scaffolded.
//
// One camera, one basemap capture per filed direction, every study shape projected to pixel space.
// This is the beat's OWN copy: no beat imports another's bake, and the static sibling carries the
// same arithmetic without either importing the other.
//
...
//
//   bun proof/web-flow-map-danube/bake.mjs --size 1600x1216
...
/** THE CAMERA HOLDS THE COURSE, AND NOTHING ELSE IT DOES NOT HAVE TO.
 *
 *  What the frame owes this reader is EVERY POINT OF THE FROZEN ROUTE — all 911 of them, from the
 *  Black Forest spring at 8,18° E to the delta at 28,75° E, and from 49,03° N down to 43,65° N. A
 *  route beat whose window cuts the course draws a river that leaves the map and comes back, which
 *  is a claim the polyline does not make; the runner measures the route against this declared window
 *  and refuses rather than trusting the overshoot.
 *
 *  WHAT THE FRAME DELIBERATELY DOES NOT HOLD WHOLE: the nine territories. Germany reaches Flensburg
 *  at 54,9° N, Romania's Carpathian north sits well above this box, Ukraine's own shape runs 1 500 km
 *  east of it. A choropleth's camera must hold its units whole because its marks ARE the units. This
 *  beat's marks are nine stretches of one river; the territories are the ground under them, and they
 *  are drawn cut on purpose. The still sibling made the same call with the same window's shape.
 *
 *  WHY NOT WIDER. A window wide enough to hold Germany and Ukraine whole reaches 5,9° E to 40,2° E
 *  and 43,6° N to 54,9° N — more than half again the longitude for a course that spans 20,6°, and all
 *  of it ground with no river on it. The one unit the wider window would buy is the one the narrower
 *  window already draws: this map is about a line, and the line is entirely inside this box.
 *
 *  The frame's own shape follows from that: 1600 x 660 is the declared window's own Mercator aspect
 *  to within a percent, so neither axis overshoots into empty continent. */
const BEAT = {
  bounds: [
    [7.2, 42.8],
    [29.8, 49.9],
...
const shapes = collection.features.map((f) => ({
  // THE JOIN KEY THIS BEAT IS FROZEN ON. `ADM0_A3` is what the shapes carry and what the route's
  // own point-in-polygon classification answers in; nothing here joins against a MapTiler tileset
  // key, because this beat draws its own nine territories from its own frozen file rather than
  // filtering a provider's Countries layer. A code that matched nothing would paint no ground under
  // a stretch, and the runner refuses on the territory count before that can ship.
