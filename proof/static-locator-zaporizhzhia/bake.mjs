...
const BEAT = { bounds: null, style: "dataviz-light" };
...
function boundsOf() {
  if (!boundsFlag) throw new Error("this beat's camera comes from its subject: pass --bounds west,south,east,north");
  const [w, s2, e, n] = boundsFlag.split(",").map(Number);
  if (![w, s2, e, n].every(Number.isFinite)) throw new Error(`--bounds ${boundsFlag} is not four numbers`);
  return [[w, s2], [e, n]];
}
...
/** A LOCATOR'S CAMERA IS DERIVED FROM ITS SUBJECT, not written down here: the beat finds the largest
 *  station in its own frozen file and boxes the map around it, so the bounds arrive from the runner.
...
const gate = await page.evaluate(
  ...
  { key, style: styleFlag ?? BEAT.style, bounds: boundsOf(), settleMs, width, height, water: waterFlag, land: landFlag },
);
...
assertCameraReachesBounds(frameCorners, boundsOf(), width);
