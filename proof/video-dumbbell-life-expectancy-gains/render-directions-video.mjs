// « La Pologne a gagné 5,0 ans d’espérance de vie depuis 2000, les États-Unis 2,5 » — rendered as video once per filed
...
//
// Usage:  bun proof/video-dumbbell-life-expectancy-gains/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = `video-dumbbell-life-expectancy-gains-${SIZE}`;
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(DumbbellFrame, { ...props, at: frame })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const envDir = await mkdtemp(join(tmpdir(), "video-dumbbell-env-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · row pitch ${report.pitch}px · ${report.ticks} ticks · head ${report.head}`,
    );
    ...
    if (lookDir) {
      ...
      const frames = [
        ...
        { name: "subject-reorder", frame: mid("subject", WINDOWS.subject.reorder) },
        { name: "subject-detach", frame: mid("subject", WINDOWS.subject.detach) },
