// « Sept pays européens dépassent 94 % d’électricité bas-carbone, par trois chemins » — rendered as video once per filed
// direction: each country's whole split into its sources, the seven regrouped into three routes (BRIEF.md). Everything is
// measured and asserted in Bun (`build.mjs`); the markup of every event's last frame is held to the type floor; the faces
// are embedded; Remotion renders the still at the last frame, then the mp4. An EMPTY `--env-file` on every `remotion` spawn.
//
// Usage:  bun proof/video-heatmap-europe-electricity/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = "video-heatmap-europe-electricity-landscape";
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(HeatmapFrame, { ...props, at: frame })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const envDir = await mkdtemp(join(tmpdir(), "video-heatmap-env-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · cell ${report.cell}`,
    );
    ...
    if (lookDir) {
      ...
      const frames = [
        ...
        { name: "first", frame: 0 },
        { name: "reference-growing", frame: Math.round(T.reference.start + T.reference.duration * 0.45) },
        { name: "reveal-splitting", frame: Math.round(T.reveal.start + T.reveal.duration * 0.4) },
        { name: "subject-parting", frame: Math.round(T.subject.start + T.subject.duration * 0.5) },
      ];
