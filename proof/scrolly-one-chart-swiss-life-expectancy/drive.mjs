  ...
  const marked = Array.from(root.querySelectorAll("[data-annotation],[data-ytick],[data-xtick],[data-part=credit],[data-part=band-label]"))
    .map((el) => ({
      what: el.dataset.annotation !== undefined
        ? `annotation ${el.dataset.annotation}`
        : el.dataset.ytick !== undefined
          ? `ytick ${el.dataset.ytick}`
          : el.dataset.xtick !== undefined
            ? `xtick ${el.dataset.xtick}`
            : el.dataset.part,
...
async function main() {
  ...
  const discrete = rmSamples.every((s) => s.state && [0, 1, 2, 3].some((k) => Math.abs(s.state.hiOpacity - [0, 1, 1, 1][k]) < 1e-9 && true));
  ...
  const nojsFacts = await nojs.evaluate(() => ({
    visual: !!document.querySelector('[data-visual="one-chart"]'),
    linePoints: (document.querySelector("[data-part=base]")?.getAttribute("points") ?? "").split(" ").length,
    ...
    visualVisible: Number(getComputedStyle(document.querySelector('[data-visual="one-chart"]').parentElement).opacity),
  }));
