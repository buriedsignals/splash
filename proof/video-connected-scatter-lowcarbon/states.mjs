...
//   furniture  the axes, the key                                                                 0..1
//   bars       each country's 2000 weight as a bar from zero at its own mix, collapsing to its ring 0..1
...
//   release    every country back — the whole chart — the five and France kept marked            0..1
...
export function statesFor() {
  const blank = { title: 0, furniture: 0, bars: 0, travel: 0, zoom: 0, back: 0, lighter: 0, legs: 0, release: 0, source: 0 };
  const establish = { ...blank, title: 1 };
  const reference = { ...establish, title: 0, furniture: 1, bars: 1 };
  const reveal = { ...reference, travel: 1 };
  const subject = { ...reveal, zoom: 1 };
  const conclusion = { ...subject, back: 1, lighter: 1, legs: 1, release: 1, source: 1 };
