...
describe("statesFor", () => {
  it("should open on the title card alone, then the axes and the 2000 rings", () => {
    expect(establish).toEqual({ title: 1, furniture: 0, bars: 0, travel: 0, zoom: 0, back: 0, lighter: 0, legs: 0, release: 0, source: 0 });
    expect(reference).toEqual({ ...establish, title: 0, furniture: 1, bars: 1 });
  });
  ...
  it("should pull back, pick out the five, split France's move and set the credit at conclusion, and hold it exactly", () => {
    expect(conclusion).toEqual({ ...subject, back: 1, lighter: 1, legs: 1, release: 1, source: 1 });
