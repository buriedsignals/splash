...
const COMPOSITION = "video-gantt-top-ten-tenure-landscape";
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · row pitch ${report.pitch}`,
    );
