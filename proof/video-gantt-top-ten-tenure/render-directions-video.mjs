// « Six pays n’ont jamais quitté le top 10 des émetteurs depuis 1990 » — rendered as video once per filed direction: the scrolly's picture told
...
//
// Usage:  bun proof/video-gantt-top-ten-tenure/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = `video-gantt-top-ten-tenure-${SIZE}`;
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(GanttFrame, { ...props, at: frame })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const envDir = await mkdtemp(join(tmpdir(), "video-gantt-env-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · row pitch ${report.pitch} · names ${report.axisPx}px · ${report.ticks} year ticks`,
    );
    ...
    if (lookDir) {
      ...
      const frames = [
        ...
        { name: "first", frame: 0 },
        ...[0.3, 0.6].map((t) => ({ name: `reveal-clock-${t}`, frame: Math.round(T.reveal.start + T.reveal.duration * (WINDOWS.reveal.clock[0] + t * (WINDOWS.reveal.clock[1] - WINDOWS.reveal.clock[0]))) })),
      ];
