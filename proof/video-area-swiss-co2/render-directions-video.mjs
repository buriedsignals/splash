// « La moitié du CO₂ suisse depuis 1858 a été émise après 1986 » — rendered as video once per filed direction: the scrolly's picture told
...
//
// Usage:  bun proof/video-area-swiss-co2/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = `video-area-swiss-co2-${SIZE}`;
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(AreaFrame, { ...props, at: frame })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const envDir = await mkdtemp(join(tmpdir(), "video-area-env-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · name form ${report.nameForm + 1} · stock form ${report.stockForm + 1} · year form ${report.yearForm + 1} · k ${report.k.toFixed(3)} · midpoint ${report.midpoint} · later ${report.shareAfter.toFixed(1)} %`,
    );
