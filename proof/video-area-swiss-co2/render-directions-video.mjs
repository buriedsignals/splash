...
const COMPOSITION = "video-area-swiss-co2-landscape";
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · midpoint ${report.midpoint} · later ${report.shareAfter.toFixed(1)} %`,
    );
