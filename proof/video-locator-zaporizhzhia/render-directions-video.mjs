// « La plus grosse centrale bas-carbone d’Europe est en Ukraine » — rendered as video once per filed direction: the scrolly's
// picture told in time (BRIEF.md), on the live MapTiler map.
//
// Everything is measured and asserted HERE, in Bun, before Chrome is started (`build.mjs`): the subject, the map plan and
// its two cameras, the overlay placed from the measured map, the states. Then the markup of every event's last frame is
// rendered in Bun and held to the type floor, the faces are embedded, and Remotion renders the live map under the overlay
// through the local key proxy (`renderVideoMap`: `--gl=swangle`, `--concurrency=1`): the still at the last frame, then the
// mp4. The key stays in this process; the page reaches MapTiler through the proxy, and the tiles are cached outside the
// repository.
//
...
//
// Usage:  set -a && . ./.env && set +a && bun proof/video-locator-zaporizhzhia/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = "video-locator-zaporizhzhia-landscape";
 ...
 *  every event, the middle of the camera's travel and the capacity counting. */
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(LocatorFrame, { ...props, at: frame, liveMap: () => null })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const scratch = await mkdtemp(join(tmpdir(), "video-locator-props-"));
  try {
    ...
    console.log(`  title form ${report.titleForm + 1} · source « ${report.sourceText} » · k ${report.k.toFixed(3)} · ${report.names} names, ${report.waters} waters`);
    ...
    if (lookDir) {
      ...
      const frames = [
        ...
        { name: "reveal-camera", frame: mid("reveal", WINDOWS.reveal.zoom) },
        { name: "subject-counting", frame: mid("subject", WINDOWS.subject.subject) },
      ];
