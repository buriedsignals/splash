...
describe("statesFor", () => {
  it("should open on the title card alone, then show Europe with Ukraine named", () => {
    expect(establish).toEqual({ title: 1, country: 0, zoom: 0, names: 0, subject: 0, source: 0 });
    expect(reference).toEqual({ ...establish, title: 0, country: 1 });
  });

  it("should zoom in and name the places at reveal, then name the station and count its capacity at subject", () => {
    expect(reveal).toEqual({ ...reference, zoom: 1, names: 1 });
