// THE LAND AND EVERY COUNTRY'S SEAT IN ONE COORDINATE SPACE, computed once in node from the frozen shapes.
//
...
// the projection only decides where a band ends and the bearing it leaves on.
//
 ...
 */
export function flowGeometry(geo, { window: win, width }) {
  const border = [];
  for (let i = 0; i <= 120; i++) {
    const t = i / 120;
