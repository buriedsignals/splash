...
const COMPOSITION = "video-bullet-low-carbon-share-landscape";
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · ${report.steps} insertion steps · row ${report.pitch}px`,
    );
