// « Pologne : +17,3 points de bas-carbone depuis 2015 » — rendered as video once per filed direction: the scrolly's picture told
...
//
// Usage:  bun proof/video-bullet-low-carbon-share/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = `video-bullet-low-carbon-share-${SIZE}`;
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(BulletFrame, { ...props, at: frame })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const envDir = await mkdtemp(join(tmpdir(), "video-bullet-env-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · header ${report.header} · ${report.steps} insertion steps · row ${report.pitch}px`,
    );
    ...
    if (lookDir) {
      ...
      const frames = [
        ...
        { name: "first", frame: 0 },
        { name: "reveal-after", frame: Math.round(T.reveal.start + T.reveal.duration * 0.75) },
        ...[0.2, 0.45].map((t) => ({ name: `subject-reorder-${t}`, frame: Math.round(T.subject.start + T.subject.duration * t) })),
      ];
