// THE LAND AND THE STATIONS IN ONE COORDINATE SPACE, computed once in node from the frozen files.
//
// PROJECTION. Lambert azimuthal equal-area centred on 52°N 10°E — the projection the sibling map beats measure
// in and the one the static plate's window was sized in. A dot map is read as density, points per area of
// land; an equal-area camera is the one on which that reading is honest everywhere in the frame.
//
 ...
 */
export function dotGeometry(geo, points, { window: win, width }) {
  const border = [];
  for (let i = 0; i <= 120; i++) {
    const t = i / 120;
