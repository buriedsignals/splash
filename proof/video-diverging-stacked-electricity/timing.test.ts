...
import { DIVERGING_STACKED_VIDEO_TIMING as T } from "./timing-contract";

const broken = (patch: Partial<BeatTiming>): BeatTiming => ({ ...T, ...patch });

describe("the shipped diverging stacked bar video timing", () => {
  ...
  it("should refuse a subject that starts before the reveal has finished", () => {
    expect(
      checkTiming(broken({ subject: { start: 250, duration: 180 } })).join(" "),
