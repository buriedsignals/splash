// « En France, le nucléaire pèse plus que le fossile et le renouvelable réunis » — rendered as video once per filed direction: the story told
...
//
// Usage:  bun proof/video-diverging-stacked-electricity/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = "video-diverging-stacked-electricity-landscape";
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(DivergingStackFrame, { ...props, at: frame })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const envDir = await mkdtemp(join(tmpdir(), "video-diverging-stacked-env-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · row ${report.pitch}px · ${report.unit}px a percentage point`,
    );
    ...
    if (lookDir) {
      ...
      const frames = [
        ...
        { name: "first", frame: 0 },
        { name: "reference-growing", frame: Math.round(T.reference.start + T.reference.duration * 0.6) },
        { name: "reveal-sliding", frame: Math.round(T.reveal.start + T.reveal.duration * 0.5) },
        { name: "subject-parting", frame: Math.round(T.subject.start + T.subject.duration * 0.3) },
        { name: "subject-carrying", frame: Math.round(T.subject.start + T.subject.duration * 0.62) },
      ];
