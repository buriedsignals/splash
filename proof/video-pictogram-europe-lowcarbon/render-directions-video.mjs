// « L'Europe électrique est aux deux bouts : 6 pays seulement au milieu » — rendered as video once per filed direction: forty
// squares on the low-carbon axis, parted at the cuts and settled into three counted blocks (BRIEF.md). Everything is measured and asserted in Bun (`build.mjs`); the markup of every event's last
...
//
// Usage:  bun proof/video-pictogram-europe-lowcarbon/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = "video-pictogram-europe-lowcarbon-landscape";
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(PictogramFrame, { ...props, at: frame })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const envDir = await mkdtemp(join(tmpdir(), "video-pictogram-env-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · ${report.cell}px a cell · columns of ${report.rows}`,
    );
    ...
    if (lookDir) {
      ...
      const frames = [
        ...
        { name: "first", frame: 0 },
        { name: "reference-sweep", frame: Math.round(T.reference.start + T.reference.duration * 0.6) },
        { name: "reveal-split", frame: Math.round(T.reveal.start + T.reveal.duration * 0.6) },
        { name: "subject-gather", frame: Math.round(T.subject.start + T.subject.duration * 0.4) },
        { name: "conclusion-close", frame: Math.round(T.conclusion.start + T.conclusion.duration * 0.3) },
      ];
