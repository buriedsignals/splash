// « Les 6 pays au-dessus de 20 t de CO₂ par personne pèsent 0,6 % de l’humanité » — rendered as video once per filed
...
//
// Usage:  bun proof/video-beeswarm-co2-per-person/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = `video-beeswarm-co2-per-person-${SIZE}`;
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(BeeswarmFrame, { ...props, at: frame })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const envDir = await mkdtemp(join(tmpdir(), "video-beeswarm-env-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · largest radius ${report.maxRadius}px in a ${report.band}px band · names ${report.naming} · first tick « ${report.unit} »`,
    );
    ...
    if (lookDir) {
      ...
      const frames = [
        ...
        { name: "subject-ghost", frame: mid("subject", WINDOWS.subject.ghost) },
        { name: "subject-gather", frame: mid("subject", WINDOWS.subject.gather) },
