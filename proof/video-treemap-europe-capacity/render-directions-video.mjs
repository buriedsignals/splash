// « L’eau et l’atome portent encore 77 % du bas-carbone européen, mais 10 pays ont basculé » — rendered as video once per filed
// direction: the treemap argued in time (BRIEF.md). Everything is measured and asserted in Bun (`build.mjs`); the markup of every event's last
...
//
// Usage:  bun proof/video-treemap-europe-capacity/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = "video-treemap-europe-capacity-landscape";
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(TreemapFrame, { ...props, at: frame })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const envDir = await mkdtemp(join(tmpdir(), "video-treemap-env-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · ${report.count} countries, ${report.drawn} cells, ${report.named} named · thread = ${report.into} of the largest`,
    );
    ...
    if (lookDir) {
      ...
      const frames = [
        ...
        { name: "first", frame: 0 },
        { name: "reference-whole", frame: mid("reference", WINDOWS.reference.whole) },
        { name: "reveal-split", frame: mid("reveal", WINDOWS.reveal.split) },
        ...["fill", "flood", "gather"].map((f) => ({ name: `subject-${f}`, frame: mid("subject", WINDOWS.subject[f]) })),
        { name: "subject-filled", frame: Math.round(T.subject.start + T.subject.duration * WINDOWS.subject.fill[1]) },
