/**
 * The Remotion composition: `TreemapFrame` at the current frame, drawn only once the embedded faces are in,
...
import { TreemapFrame, type TreemapFrameProps } from "./TreemapFrame";

export type DirectedTreemapVideoProps = TreemapFrameProps & {
  faces: EmbeddedFace[];
};

export function DirectedTreemapVideo(props: DirectedTreemapVideoProps) {
  const frame = useCurrentFrame();
  const { ready, ref } = useEmbeddedFaces<SVGSVGElement>(props.faces);
  return (
    <AbsoluteFill style={{ backgroundColor: props.colours?.ground }}>
      {ready ? <TreemapFrame {...props} at={frame} svgRef={ref} /> : null}
    </AbsoluteFill>
  );
}
