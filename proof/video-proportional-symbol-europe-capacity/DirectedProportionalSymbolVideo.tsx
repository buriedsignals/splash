/**
 * The Remotion composition: the live MapTiler map driven by the frame, and `SymbolFrame`'s overlay over it, drawn only once
...
import { SymbolFrame, type SymbolFrameProps } from "./SymbolFrame";
...
export type DirectedProportionalSymbolVideoProps = SymbolFrameProps & {
  faces: EmbeddedFace[];
...
function LiveSymbolMapOn(
  props: DirectedProportionalSymbolVideoProps & { plan: ProxiedPlan; at: number },
) {
  const { plan } = props;
...
function LiveSymbolMap(props: DirectedProportionalSymbolVideoProps & { at: number }) {
  if (!props.mapPlanProxied) return null;
  return <LiveSymbolMapOn {...props} plan={props.mapPlanProxied} />;
}

export function DirectedProportionalSymbolVideo(props: DirectedProportionalSymbolVideoProps) {
    ...
    <AbsoluteFill style={{ backgroundColor: props.colours?.ground }}>
      {ready ? (
        <SymbolFrame
          {...props}
          at={frame}
          liveMap={(at) => <LiveSymbolMap {...props} at={at} />}
          svgRef={ref}
        />
      ) : null}
    </AbsoluteFill>
