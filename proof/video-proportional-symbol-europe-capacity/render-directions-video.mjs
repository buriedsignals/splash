// « Un centième des sites porte plus d’un tiers de la puissance » — rendered as video once per filed direction: the scrolly's picture told
...
//
// Usage:  bun proof/video-proportional-symbol-europe-capacity/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(SymbolFrame, { ...props, at: frame })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const envDir = await mkdtemp(join(tmpdir(), "video-proportional-symbol-env-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · key over ${report.keyLand.toFixed(2)} land · top ${report.shareTop.toFixed(1)} %`,
    );
