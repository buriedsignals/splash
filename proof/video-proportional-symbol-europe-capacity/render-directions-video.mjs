// « Un centième des sites porte plus d’un tiers de la puissance » — rendered as video once per filed direction: the scrolly's picture
...
//
// Usage:  set -a && . ./.env && set +a && bun proof/video-proportional-symbol-europe-capacity/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = "video-proportional-symbol-europe-capacity-landscape";
 ...
 *  every event, and the circles arriving. */
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(SymbolFrame, { ...props, at: frame, liveMap: () => null })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const scratch = await mkdtemp(join(tmpdir(), "video-proportional-symbol-props-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source « ${report.sourceText} » · k ${report.k.toFixed(3)} · key over ${(100 * report.keyLand).toFixed(1)} % land · top ${report.shareTop.toFixed(1)} % · ${report.layers} map layers`,
    );
    ...
    if (lookDir) {
      ...
      const frames = [
        ...
        { name: "reveal-arriving", frame: mid("reveal", WINDOWS.reveal.top) },
      ];
