...
describe("statesFor", () => {
  it("should open on the title card alone, then Europe and the key", () => {
    expect(establish).toEqual({ title: 1, furniture: 0, top: 0, rest: 0, source: 0 });
    expect(reference).toEqual({ ...establish, title: 0, furniture: 1 });
  });

  it("should land the hundred largest at reveal, and the rest at subject", () => {
    expect(reveal).toEqual({ ...reference, top: 1 });
    expect(subject).toEqual({ ...reveal, rest: 1 });
  });
