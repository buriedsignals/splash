// Usage: set -a && . ./.env && set +a && bun proof/video-proportional-symbol-europe-capacity/measure.mjs [--size landscape|portrait|square]
//
...
//
// THE PROPORTIONAL SYMBOL VIDEO'S STILL CAMERA, MEASURED ON THE REAL MAP ONCE AND FROZEN: where MapLibre draws the seats
// the overlay is checked against (the Atlantic, the westmost, northmost and eastmost stations) and the colour it paints
// under every cell, for each direction. `build.mjs` reads the result offline; a plan that changes after it is refused
// there (`planDigest`).
//
// The camera never moves, so it is measured once, in the picture the video ends on (`whole`, the last frame: the
// hundred circles and the rest in) — the sea the key and the credit stand on, the offshore stations counted as not sea.
