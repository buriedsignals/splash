// « Le plancher européen est monté de 30 points, le plafond de 2 » — rendered as video once per filed
// direction: the argument of BRIEF.md told in time. Everything is measured and asserted in Bun (`build.mjs`); the markup of every event's last
...
//
// Usage:  bun proof/video-dot-strip-lowcarbon-spread/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = `video-dot-strip-lowcarbon-spread-${SIZE}`;
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(DotStripFrame, { ...props, at: frame })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const envDir = await mkdtemp(join(tmpdir(), "video-dot-strip-env-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · chip rows ${report.rows} · corridor ${report.corridor}px` +
        // A rung that fires is a decision, and a decision nobody chose does not happen silently: the field it drew and
        // the scale it stopped writing out are reported beside the numbers. With the whole field and both scales
        // written, there is nothing to report and the line is the one landscape has always printed.
        (report.drawn < report.field ? ` · field ${report.drawn} of ${report.field}` : "") +
        (report.labelUpper ? "" : " · upper scale unlabelled"),
    );
    ...
    if (lookDir) {
      ...
      const frames = [
        ...
        { name: "first", frame: 0 },
        { name: "reveal-travel", frame: Math.round(T.reveal.start + T.reveal.duration * 0.5) },
        { name: "subject-span", frame: mid("subject", WINDOWS.subject.span) },
        { name: "subject-drop", frame: mid("subject", WINDOWS.subject.drop) },
        { name: "conclusion-settle", frame: mid("conclusion", WINDOWS.conclusion.settle) },
      ];
