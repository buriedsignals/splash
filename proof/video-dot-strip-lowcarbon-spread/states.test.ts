import { describe, expect, it } from "bun:test";
import { statesFor } from "./states.mjs";

/** The choreography of BRIEF.md as numbers, hand-copied from its table. */

const [establish, reference, reveal, subject, conclusion, hold] = statesFor();

describe("statesFor", () => {
  it("should open on the title card alone, then both rails and the 2000 chips", () => {
    expect(establish).toEqual({
      title: 1,
      furniture: 0,
      travel: 0,
      focus: 0,
      changes: 0,
      span: 0,
      drop: 0,
      cut: 0,
      settle: 0,
      source: 0,
    });
    expect(reference).toEqual({ ...establish, title: 0, furniture: 1 });
  });

  it("should travel the chips at reveal, then focus, name, trace, drop and cut the span at subject", () => {
    expect(reveal).toEqual({ ...reference, travel: 1 });
    expect(subject).toEqual({
      ...reveal,
      focus: 1,
      changes: 1,
      span: 1,
      drop: 1,
      cut: 1,
    });
  });

  it("should settle with the credit at conclusion, and hold it exactly", () => {
    expect(conclusion).toEqual({ ...subject, settle: 1, source: 1 });
    expect(hold).toEqual(conclusion);
  });
});
