// « L’Espagne a ajouté plus d’électricité bas-carbone que la France depuis 2000 » — rendered as video once per filed direction: the
// stack argued in time (BRIEF.md). Everything is measured and asserted in Bun (`build.mjs`); the markup of every event's last
...
//
// Usage:  bun proof/video-stacked-bar-lowcarbon-growth/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = "video-stacked-bar-lowcarbon-growth-landscape";
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(StackedBarFrame, { ...props, at: frame })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const envDir = await mkdtemp(join(tmpdir(), "video-stacked-bar-env-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · × ${report.copies} copies · row ${report.pitch}px`,
    );
    ...
    if (lookDir) {
      ...
      const frames = [
        ...
        { name: "subject-copies-laid", frame: Math.round(T.subject.start + T.subject.duration * WINDOWS.subject.detach[0]) },
        ...["copies", "detach", "reorder"].map((f) => ({ name: `subject-${f}`, frame: mid("subject", WINDOWS.subject[f]) })),
