/**
 * The Remotion composition: the live MapTiler map driven by the frame, and `HexFrame`'s overlay over it, drawn only
...
export type DirectedHexGridVideoProps = HexFrameProps & {
  faces: EmbeddedFace[];
...
function LiveHexGridMapOn(
  props: DirectedHexGridVideoProps & { plan: ProxiedPlan; at: number },
) {
  const { plan } = props;
...
function LiveHexGridMap(props: DirectedHexGridVideoProps & { at: number }) {
  if (!props.mapPlanProxied) return null;
  return <LiveHexGridMapOn {...props} plan={props.mapPlanProxied} />;
}
    ...
    <AbsoluteFill style={{ backgroundColor: props.colours?.ground }}>
      {ready ? (
        <HexFrame
          {...props}
          at={frame}
          liveMap={(at) => <LiveHexGridMap {...props} at={at} />}
          svgRef={ref}
        />
      ) : null}
    </AbsoluteFill>
