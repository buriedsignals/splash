...
//   furniture  the key column (and, under it, the live map of the hosts)                         0..1
//   morph      every country from its shape on the map (0) to its hexagon (1), the basemap leaving  0..1
//   codes      every cell's code set on it                                                       0..1
...
export function statesFor() {
  const blank = { title: 0, furniture: 0, morph: 0, codes: 0, count: 0, largest: 0, rate: 0, leader: 0, source: 0 };
  const establish = { ...blank, title: 1 };
  const reference = { ...establish, title: 0, furniture: 1, morph: 1, codes: 1 };
