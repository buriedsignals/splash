// « Par habitant, la Tchéquie accueille 36,1 Ukrainiens pour 1 000 habitants » — rendered as video once per filed
// direction (BRIEF.md), its geography on the live MapTiler map until the countries leave it for their cells.
//
...
//
// Usage:  set -a && . ./.env && set +a && bun proof/video-hex-grid-europe-protection/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = "video-hex-grid-europe-protection-landscape";
 ...
 *  last frame of every event, the map before the handover, the handover, the travel, the counting and the re-classing. */
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(HexFrame, { ...props, at: frame, liveMap: () => null })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const scratch = await mkdtemp(join(tmpdir(), "video-hex-grid-props-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · ` +
        `key over ${report.keyLand} % land · hexagon R ${report.R} · codes at ${report.codeSize}px`,
    );
    ...
    if (lookDir) {
      ...
      const frames = [
        ...
        { name: "map", frame: Math.floor(T.reference.start + T.reference.duration * HANDOVER[0]) },
        { name: "handover", frame: mid("reference", HANDOVER) },
        ...[0.3, 0.7].map((t) => ({ name: `morph-${t}`, frame: Math.round(T.reference.start + T.reference.duration * (WINDOWS.reference.morph[0] + t * (WINDOWS.reference.morph[1] - WINDOWS.reference.morph[0]))) })),
        { name: "reveal-counting", frame: mid("reveal", WINDOWS.reveal.count) },
        { name: "subject-reclassing", frame: mid("subject", WINDOWS.subject.rate) },
      ];
