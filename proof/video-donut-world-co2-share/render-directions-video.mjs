// « En 2000 les États-Unis émettaient un quart du CO₂ mondial et la Chine un septième ; en 2023, c’est l’inverse » — rendered as video once per filed
...
//
// Usage:  bun proof/video-donut-world-co2-share/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = `video-donut-world-co2-share-${SIZE}`;
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(DonutFrame, { ...props, at: frame })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const envDir = await mkdtemp(join(tmpdir(), "video-donut-env-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · tonnes form ${report.tonnesForm} · ${report.rings} of ${report.ofRings} rings · k ${report.k.toFixed(3)} · world ring r=${report.r1}px · country ring ${report.edge}px · the past is ${report.past}`,
    );
    ...
    if (lookDir) {
      ...
      const frames = [
        ...
        { name: "first", frame: 0 },
        { name: "reference-trace", frame: mid("reference", WINDOWS.reference.world) },
        { name: "reveal-grow", frame: mid("reveal", WINDOWS.reveal.grow) },
        { name: "subject-split", frame: mid("subject", WINDOWS.subject.split) },
      ];
