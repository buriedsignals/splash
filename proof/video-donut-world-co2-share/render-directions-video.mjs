...
const COMPOSITION = "video-donut-world-co2-share-landscape";
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · tonnes form ${report.tonnesForm} · k ${report.k.toFixed(3)} · world ring r=${report.r1}px · country ring ${report.edge}px · the past is ${report.past}`,
    );
