...
viz1: the value is in white inside the segment, so *"no axis reading is required to know that Sweden
went 13 → 15"*.
...
A number inside a segment is measured against that segment's own fill, and the fills of one stack run
from pale to full. `adjustToContrast(ink, segmentFill, TEXT_CONTRAST_MIN)` per segment — the same
lesson the calendar heatmap's ramp taught, met again where the ramp is only two steps long.
