// « Le nucléaire de ces six pays est français à 84 % » — rendered as video once per filed
// direction: the sankey argued in time (BRIEF.md). Everything is measured and asserted in Bun (`build.mjs`); the markup of every event's last
...
//
// Usage:  bun proof/video-sankey-electricity-sources/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = "video-sankey-electricity-sources-landscape";
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(SankeyFrame, { ...props, at: frame })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const envDir = await mkdtemp(join(tmpdir(), "video-sankey-env-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · ${report.scale} px/TWh · share ${report.share} %`,
    );
    ...
    if (lookDir) {
      ...
      const frames = [
        ...
        { name: "reveal-pour", frame: mid("reveal", WINDOWS.reveal.pour) },
        ...["trace", "slide"].map((f) => ({ name: `subject-${f}`, frame: mid("subject", WINDOWS.subject[f]) })),
