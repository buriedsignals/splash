...
| `reveal` | 3 + 4 | above 94 %, only seven remain — six are north or west | **filter + count**, then **name** | every country under the floor steps back to bare land while the counter counts 0 → 7; once the filter has landed, Islande, Suède, Norvège, Finlande, France, Suisse are named | stepped back = 33; above = 7; named = 6 (`above` minus Albania), each north or west of Albania |
| `subject` | 5 | the seventh is Albania, 100 %, and its three measured neighbours are all under 60 % | **zoom + name** | the six names leave; every class returns as the camera travels, eased, onto the Balkans, Albania at the centre of the close-up; once the camera has settled: Albania ringed and named « Albanie · 100 % », then « Monténégro · 59 % », « Macédoine du Nord · 39 % », « Grèce · 49 % », and « Kosovo, hors données » | Albania ∈ `above` at 100 %; measured neighbours = 3, the highest Monténégro 59.5 % < 60; Kosovo = the one ring-neighbour without a row |
