...
//
// Usage:  set -a && . ./.env && set +a && bun proof/video-choropleth-europe-lowcarbon/render-directions-video.mjs [--size <size>] [--only <id>] [--still] [--check] [--look <dir>]
...
const COMPOSITION = `video-choropleth-europe-lowcarbon-${SIZE}`;
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(ChoroplethFrame, { ...props, at: frame, liveMap: () => null })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  // WHAT THIS RUN WRITES IS WHAT THIS RUN MAY REMOVE — and every name it writes is this one. Spelled `${id}`
  // it was the LANDSCAPE name: a refusal at another size deleted a delivered landscape mp4 that had nothing to
  // do with it, and a SUCCESS at another size overwrote the delivered landscape still and props with its own.
  const at = nameAtSize(id, SIZE);
  const outputs = [`${at}.mp4`, `${at}-final-frame.png`, `${at}-props.json`].map((f) => join(OUT, f));
  const scratch = await mkdtemp(join(tmpdir(), "video-choropleth-props-"));
  try {
    ...
    if (lookDir) {
      ...
      for (const { name, frame } of frames) await renderVideoMap({ ...common, outDir: lookDir, name: `${at}-${String(frame).padStart(3, "0")}-${name}`, mode: "still", frame });
