// « Les émissions de CO2 par personne en France ont culminé dans les années 1970 » — rendered as video once per filed direction: the argument
...
//
// Usage:  bun proof/video-box-plot-france-co2-decades/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = `video-box-plot-france-co2-decades-${SIZE}`;
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(BoxplotFrame, { ...props, at: frame })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const envDir = await mkdtemp(join(tmpdir(), "video-box-plot-env-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · name form ${report.nameForm + 1} · k ${report.k.toFixed(3)} · medians ${report.valueSeats.join(" / ")} · slot ${report.slot}px · plot right ${report.right}px`,
    );
    ...
    if (lookDir) {
      ...
      const frames = [
        ...
        { name: "first", frame: 0 },
        { name: "reference-readings", frame: Math.round(T.reference.start + T.reference.duration * 0.6) },
        ...[0.2, 0.5, 0.8].map((t) => ({ name: `reveal-${t}`, frame: Math.round(T.reveal.start + T.reveal.duration * t) })),
        { name: "subject-walk", frame: Math.round(T.subject.start + T.subject.duration * 0.4) },
        { name: "conclusion-release", frame: Math.round(T.conclusion.start + T.conclusion.duration * 0.3) },
      ];
