...
const COMPOSITION = "video-box-plot-france-co2-decades-landscape";
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · slot ${report.slot}px · plot right ${report.right}px`,
    );
