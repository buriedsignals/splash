// « 4,5 millions d’Ukrainiens sous protection temporaire » — rendered as video once per filed direction: the scrolly's picture told
...
//
// Usage:  bun proof/video-flow-map-ukraine-protection/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(FlowFrame, { ...props, at: frame })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const envDir = await mkdtemp(join(tmpdir(), "video-flow-map-env-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · key over ${report.keyLand.toFixed(2)} land · ${report.drawn} bands, ${report.named} named`,
    );
    ...
    if (lookDir) {
      ...
      const frames = [
        ...
        { name: "first", frame: 0 },
        ...[0.2, 0.5].map((t) => ({ name: `reveal-trace-${t}`, frame: Math.round(T.reveal.start + T.reveal.duration * (WINDOWS.reveal.trace[0] + t * (WINDOWS.reveal.trace[1] - WINDOWS.reveal.trace[0]))) })),
      ];
