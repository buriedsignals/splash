// « 4,5 millions d’Ukrainiens sous protection temporaire » — rendered as video once per filed direction: the scrolly's picture
...
//
// Usage:  set -a && . ./.env && set +a && bun proof/video-flow-map-ukraine-protection/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = "video-flow-map-ukraine-protection-landscape";
 ...
 *  every event, and the bands halfway through their trace. */
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(FlowFrame, { ...props, at: frame, liveMap: () => null })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const scratch = await mkdtemp(join(tmpdir(), "video-flow-map-props-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source « ${report.sourceText} » · k ${report.k.toFixed(3)} · key over ${(100 * report.keyLand).toFixed(1)} % land · ${report.drawn} bands, ${report.named} named`,
    );
    ...
    if (lookDir) {
      ...
      const frames = [
        ...
        // The two largest bands halfway out of the node: the trace cut on the map.
        { name: "reveal-tracing", frame: Math.round(T.reveal.start + T.reveal.duration * (WINDOWS.reveal.trace[0] + 0.045 * (WINDOWS.reveal.trace[1] - WINDOWS.reveal.trace[0]))) },
      ];
