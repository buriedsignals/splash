// « L’Allemagne a produit 143 TWh d’électricité de moins en 2024 qu’en 2015 » — rendered as video once per filed direction: the bridge built
// from the two mixes (BRIEF.md). Everything is measured and asserted in Bun (`build.mjs`); the markup of every event's last
...
//
// Usage:  bun proof/video-waterfall-germany-electricity-bridge/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = "video-waterfall-germany-electricity-bridge-landscape";
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(WaterfallFrame, { ...props, at: frame })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const envDir = await mkdtemp(join(tmpdir(), "video-waterfall-env-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · slot ${report.slotW}px · ${report.unit}px a TWh`,
    );
    ...
    if (lookDir) {
      ...
      const frames = [
        ...
        { name: "first", frame: 0 },
        { name: "reference-carry", frame: Math.round(T.reference.start + T.reference.duration * 0.78) },
        { name: "reveal-morphing", frame: Math.round(T.reveal.start + T.reveal.duration * 0.6) },
        { name: "subject-sliding", frame: Math.round(T.subject.start + T.subject.duration * 0.45) },
        { name: "subject-landing", frame: Math.round(T.subject.start + T.subject.duration * 0.75) },
        { name: "conclusion-bracket", frame: Math.round(T.conclusion.start + T.conclusion.duration * 0.4) },
      ];
