// « La population mondiale a dépassé 8 milliards en 2022 » — rendered as video once per filed direction: the scrolly's picture told
...
//
// Usage:  bun proof/video-cold-world-population/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = "video-cold-world-population-landscape";
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(PopulationFrame, { ...props, at: frame })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const envDir = await mkdtemp(join(tmpdir(), "video-population-env-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · ×${report.multiple.toFixed(3)} · 8 billion at ${report.crossingYear.toFixed(2)}`,
    );
    ...
    if (lookDir) {
      ...
      const frames = [
        ...
        { name: "subject-level", frame: mid("subject", WINDOWS.subject.level) },
        { name: "subject-zoom", frame: mid("subject", WINDOWS.subject.zoom) },
        { name: "conclusion-untint", frame: mid("conclusion", WINDOWS.conclusion.tint) },
      ];
