// « Les seize pays ont tous gagné du bas-carbone — un seul a doublé la France » — rendered as video once per filed direction: the scrolly's picture told
...
//
// Usage:  bun proof/video-slope-europe-lowcarbon/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = "video-slope-europe-lowcarbon-landscape";
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(SlopeFrame, { ...props, at: frame })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const envDir = await mkdtemp(join(tmpdir(), "video-slope-env-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · largest push ${report.pushed}px`,
    );
    ...
    if (lookDir) {
      ...
      const frames = [
        ...
        ...[0.4, 0.8].map((t) => ({ name: `subject-check-${t}`, frame: Math.round(T.subject.start + T.subject.duration * t) })),
      ];
