// « La Croatie est le seul pays de l’UE à émettre plus de CO₂ par personne qu’en 1990 » — rendered as video once per filed direction: the scrolly's picture told
...
//
// Usage:  bun proof/video-diverging-bar-eu-per-capita/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = `video-diverging-bar-eu-per-capita-${SIZE}`;
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(DivergingBarFrame, { ...props, at: frame })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const envDir = await mkdtemp(join(tmpdir(), "video-diverging-bar-env-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · row ${report.pitch}px · ${report.unit}px a tonne · zoom ×${report.zoomBy}${report.reduced ? ` · R8: ${report.drawn} of ${report.members} rows drawn, said on the plate in form ${report.scopeForm + 1}` : ""}`,
    );
    ...
    if (lookDir) {
      ...
      const frames = [
        ...
        { name: "first", frame: 0 },
        { name: "reference-levels", frame: Math.round(T.reference.start + T.reference.duration * 0.7) },
        { name: "reveal-shrinking", frame: Math.round(T.reveal.start + T.reveal.duration * 0.5) },
        { name: "subject-flipping", frame: Math.round(T.subject.start + T.subject.duration * 0.18) },
        { name: "subject-zooming", frame: Math.round(T.subject.start + T.subject.duration * 0.62) },
      ];
