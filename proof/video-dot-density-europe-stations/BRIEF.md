...
after the other on the same dots — the gesture only a video has is **the dots growing into their weight**.
...
| `reveal` | 72 of them are nuclear: 0,8 % of the sites | **filter + name** | every other dot steps back; the 72 are ringed; « 72 nucléaires, 0,8 % des sites » | 72 sites, share < 1 % |
| `subject` | and they carry a third of the power | **rescale + count up** | the others come back; every dot grows to an area proportional to its capacity — the rings swell, the solar field stays dust; « 34,4 % de la puissance » counts up; a size reference lands in the key | share > 30 %; nuclear the highest MW per site |
