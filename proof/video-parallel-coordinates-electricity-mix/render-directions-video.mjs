// « 2 pays sur 16 ont plus de 25 % de nucléaire et plus de 20 % d’éolien » — rendered as video once per filed
// direction: the parallel coordinates argued in time (BRIEF.md). Everything is measured and asserted in Bun (`build.mjs`); the markup of every event's last
...
//
// Usage:  bun proof/video-parallel-coordinates-electricity-mix/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = "video-parallel-coordinates-electricity-mix-landscape";
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(ParallelFrame, { ...props, at: frame })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const envDir = await mkdtemp(join(tmpdir(), "video-parallel-env-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · ${report.scale} px/pt · seats on rails ${report.seated}`,
    );
    ...
    if (lookDir) {
      ...
      const frames = [
        ...
        { name: "first", frame: 0 },
        ...[["reference", "stand"], ["reveal", "draw"], ["subject", "zoom"], ["subject", "nuclear"], ["subject", "wind"], ["conclusion", "back"]].map(([e, f]) => ({ name: `${e}-${f}`, frame: mid(e, WINDOWS[e][f]) })),
      ];
