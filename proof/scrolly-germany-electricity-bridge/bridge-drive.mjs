...
export function applyBridgeState(root, state, context) {
  ...
  let noteOn = 0;
  for (const [key, node] of Object.entries(c.notes)) {
    node.style.opacity = String(notes[key]);
    noteOn = Math.max(noteOn, notes[key]);
  }
  c.running.style.opacity = String(1 - noteOn);
}
