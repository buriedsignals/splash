// « La Chine a émis plus de CO₂ que les cinq pays suivants réunis » — rendered as video once per filed direction: the scrolly's picture told
...
//
// Usage:  bun proof/video-bar-top-emitters-2024/render-directions-video.mjs [--only <id>] [--still] [--look <dir>]
...
const COMPOSITION = `video-bar-top-emitters-2024-${SIZE}`;
...
export function assertEventFramesReadable(props, id) {
  for (const event of EVENT_ORDER) {
    ...
    assertTypeFloor(renderToStaticMarkup(createElement(BarFrame, { ...props, at: frame })), SIZE, { what: `${id} at the end of ${event} (frame ${frame})` });
  }
}
...
for (const id of filedIds.filter((i) => only === null || i === only)) {
  ...
  const envDir = await mkdtemp(join(tmpdir(), "video-bar-env-"));
  try {
    ...
    console.log(
      `  title form ${report.titleForm + 1} · source form ${report.sourceForm + 1} · k ${report.k.toFixed(3)} · row pitch ${report.pitch} · pile on ${report.pileLines} line(s) · ${report.leaders} leaders`,
    );
