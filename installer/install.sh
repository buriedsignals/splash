...
if [ -z "$ROOT" ]; then
  if [ "$PROVISIONED" = 1 ] || [ "$EXISTING_CHECKOUT" = 1 ]; then ROOT="$SOURCE"; else ROOT="$HOME/Splash"; fi
fi
...
if [ "$PROVISIONED" = 1 ] || [ "$EXISTING_CHECKOUT" = 1 ]; then
  if [ "$PROVISIONED" = 1 ]; then
    say "2/6  Using the checkout the signed bundle created"
  else
    say "2/6  Using the existing Splash checkout (private integration)"
  fi
...
if [ "$SKIP_CONFIGURE" = 1 ]; then
  ...
  if [ "$HEADLESS" = 1 ]; then
    bun "$ROOT/installer/configure.mjs" --root "$ROOT" --skill-namespace "$SKILL_NAMESPACE" --headless
  else
    bun "$ROOT/installer/configure.mjs" --root "$ROOT" --skill-namespace "$SKILL_NAMESPACE"
  fi
fi

say "5/6  Wiring the doors your AI hosts read"
run bun "$ROOT/installer/place-skills.mjs" --root "$ROOT" --namespace "$SKILL_NAMESPACE"

say "6/6  Writing splash-doctor, then running it"
...
BIN="$HOME/.local/bin"
if [ "$DRY_RUN" = 1 ]; then
  echo "   would write $BIN/splash-doctor  →  bun $ROOT/installer/doctor.mjs --root $ROOT --skill-namespace $SKILL_NAMESPACE"
else
  mkdir -p "$BIN"
  cat > "$BIN/splash-doctor" <<EOF
#!/usr/bin/env bash
...
exec bun "$ROOT/installer/doctor.mjs" --root "$ROOT" --skill-namespace "$SKILL_NAMESPACE" "\$@"
EOF
  chmod +x "$BIN/splash-doctor"
  echo "   wrote $BIN/splash-doctor"
  case ":$PATH:" in
    *":$BIN:"*) ;;
...
echo
bun "$ROOT/installer/doctor.mjs" --root "$ROOT" --skill-namespace "$SKILL_NAMESPACE" || true

cat <<EOF

Splash is installed at $ROOT.

  Ask your AI assistant:   "use splash to make a chart from this article"
  Check it is still well:  splash-doctor
