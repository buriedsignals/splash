...
const BEAT_ID = "vidy-histogram-life-expectancy";
...
// `--size <name>` renders one of the OTHER two into `sizes/` so all three can be compared. It is
// deliberately not a way to change what this beat DELIVERS.
...
const stem = sizeFlag === -1 ? "histogram" : `histogram-${size}`;
if (sizeFlag !== -1)
...
assertAnnotationReadsOverMarks(
  ...
  [BEAT.ground],
  // The floor is asked for at the size this beat SHIPS at: WCAG 1.4.3's large-text threshold moves
  // with the drawn font size, so a contrast claim made against an unscaled 1080-frame token is a
  // claim about a render that no longer happens.
  textContrastFloor(noteFor(size)),
);
...
// And the DELIVERED mp4, out of the container itself — the one reading the code that wrote the file
