 ...
 * bake uses to lay out its search rings, so a marker's measured distance and the ring that reaches
...
/**
 * The projected search ring at `km`, interpolated between the two baked rings that bracket it.
 *
 * The bake projects a real great-circle locus at each of its own radii; this only ever moves
 * BETWEEN two of them, vertex by vertex, in the same bearing order. It never scales a circle by an
 * assumed metres-per-pixel, which is the shape a projection lie takes on a map: at 0.25 km steps the
 * two brackets differ by about ten pixels at this beat's scale, and a straight line between two
 * concentric loci ten pixels apart is inside the width of the stroke that draws it.
 */
export function ringAtKm(
  rings: [number, number][][],
  radiiKm: number[],
  ...
  if (!first || !last || radiiKm.length !== rings.length)
    throw new Error(
      `the plate carries ${rings.length} rings and ${radiiKm.length} radii — they must agree`,
    );
  const lerp = (
    a: [number, number],
    b: [number, number],
  ...
  if (km <= 0) return first.map(() => centre);
  if (km <= radiiKm[0]!)
    return first.map((point) => lerp(centre, point, km / radiiKm[0]!));
  if (km >= radiiKm[radiiKm.length - 1]!) return last;

  let i = 0;
  while (i < radiiKm.length - 2 && km >= radiiKm[i + 1]!) i++;
  const t = (km - radiiKm[i]!) / (radiiKm[i + 1]! - radiiKm[i]!);
  return rings[i]!.map((point, index) => lerp(point, rings[i + 1]![index]!, t));
}
