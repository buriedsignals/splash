...
// segments vary in on-screen length after projection.
 ...
 *  territory crossed." Orange is held back for the route's own accent (see
 *  `FlowMapVideo.tsx`/`FlowMapStill.tsx`), so none of these nine overlaps it. */
...
// ── The travelled sub-path, by real arc-length fraction ───────────────────────────────────────────

/**
 * The pixel-space point list for the FIRST `fraction` of the route's real geographic length —
 * `cumKm` is `cumulativeKm(route)`, and `pixelRoute` is the same route already projected to pixel
 * space by the bake (same length, same order, index-for-index). Interpolates the exact cut point on
...
export function travelledPath(
  pixelRoute: [number, number][],
  cumKm: number[],
  fraction: number,
): [number, number][] {
  if (pixelRoute.length !== cumKm.length)
    throw new Error(
      `travelledPath: pixelRoute (${pixelRoute.length}) and cumKm (${cumKm.length}) must be the same length — they must be the same route, index for index`,
    );
  const clamped = Math.max(0, Math.min(1, fraction));
  if (pixelRoute.length === 0) return [];
  const total = cumKm[cumKm.length - 1]!;
  const targetKm = total * clamped;
  if (clamped >= 1) return pixelRoute;
  if (clamped <= 0) return [pixelRoute[0]!];

  let hi = 1;
  while (hi < cumKm.length && cumKm[hi]! < targetKm) hi++;
  if (hi >= cumKm.length) return pixelRoute;
  const lo = hi - 1;
  const segStart = cumKm[lo]!;
  const segEnd = cumKm[hi]!;
  const t = segEnd > segStart ? (targetKm - segStart) / (segEnd - segStart) : 0;
  const [ax, ay] = pixelRoute[lo]!;
  const [bx, by] = pixelRoute[hi]!;
  const cut: [number, number] = [ax + (bx - ax) * t, ay + (by - ay) * t];
  return [...pixelRoute.slice(0, hi), cut];
}
