...
export function BoxplotVideo({
  ...
  // Furniture: title, source, axis unit — one fade, together, then still forever. The title is
  ...
  return (
    ...
    >
      ...
      <g opacity={furnitureOpacity}>
        ...
        {/* The value axis: sparse ticks, established once with the rest of the furniture, never
            moving again — a box plot needs its unit labelled (`boxplot.md`), but this genre asks
            for the sparsest version of that rule, not the static genre's dense grid. */}
        {ticks.map((t) => (
          <g key={t}>
            <line
              x1={g.plot.left}
