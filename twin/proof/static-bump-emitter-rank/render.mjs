...
async function main() {
  ...
  const name = flag === -1 ? "static-bump-emitter-rank-still" : `static-bump-emitter-rank-${size}`;
  if (flag !== -1)
    console.log(`LOOKING at ${size}; the pinned size stays ${pinned} -> ${outDir}`);
  // …and whether this TYPE may enter that size at all.
  const form = assertTypeMayEnter(TYPE, size, { what: "static-bump-emitter-rank" });
  ...
  assertTypeFloor(svg, size, { what: "static-bump-emitter-rank" });
  assertWithinStage(svg, size, { what: "static-bump-emitter-rank" });
