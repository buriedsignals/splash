...
grounded conventions (renewables, fossil, water, heat), and this beat's subject — the DISTRIBUTION
of per-capita CO₂ footprints across the world's countries — fires none of them. When no convention
applies, the house theme wins.

One accent, because a histogram is one series: every bin is the same quantity counted in a different
place on the axis, so a second hue would encode a distinction the data does not carry. The median
marker and its label are ink and `muted`, both derived by `deriveFurniture` from `ground`, never the
bin colour.
