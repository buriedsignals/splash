...
The `renderStill` function in `#shared/twin-chart-beat/render-still.mjs` enforces this by wrapping the component in a proper SVG element with xmlns before passing it to the rasteriser. If you are testing SVG rendering outside of `renderStill`, you must ensure the xmlns is present yourself.
