...
Six sheets, one per map type, each a page you read before writing that type's beat by hand — not
...
choropleth · proportional-symbol · locator · flow-map · dot-density · hex-grid.

## What is not here

The source engine builds one more 2D type this set skips: **cartogram** (area distorted to
encode a value instead of true geography) — real, built, and guarded in the source engine, just
not sheeted tonight. Also not covered: **contour / isoline** (designed in the source engine's
own notes but never built there either) and **3D terrain flyover** (a wholly separate engine —
Cesium, not MapTiler — with its own camera and timing model, out of scope for a 2D map-beat
sheet set). Do not treat this set as complete. If your story genuinely needs one of these, the
same harvesting method applies: read the source engine's conformance rules and component for that
type, and write the sheet before you write the beat.
