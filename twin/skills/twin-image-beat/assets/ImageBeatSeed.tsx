 ...
 * prevent — the same rule `twin-chart-beat/assets/ChartSeed.tsx` states for a `variant` prop.
 *
 ...
 *  `twin-chart-beat/assets/ChartSeed.tsx`'s own `wrap`, this skill's own copy because a skill never
 ...
 * the drawing is rewritten, the same promise `twin-chart-beat`'s `lineGeometry` makes.
 *
  ...
  return (
    ...
    >
      ...
      {blocks.map((block, i) => (
        ...
        // `role="img"` + `<desc>` pairing `twin-chart-beat/assets/ChartSeed.tsx` uses at its own
