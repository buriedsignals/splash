 ...
 * agree (`skills/splash/test/helper-parity.test.ts` is the pattern this file follows, and
 * `skills/splash/test/no-cross-skill-imports.test.ts` excludes every skill's own `test/`
 * directory from the "no import leaves the skill" guard for exactly this reason).
 *
 * `storyboard/scripts/capability-gap.mjs` is a byte-for-byte-intended duplicate of
 * `splash/scripts/preflight.mjs`'s own `capabilityGap` — carried, not imported, so
 * `storyboard` stays copy-pasteable on its own. This file is the guard against the risk that
...
import { capabilityGap as preflightCapabilityGap } from "../../splash/scripts/preflight.mjs";
...
describe("capabilityGap — storyboard's copy agrees with splash's original", () => {
