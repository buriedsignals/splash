...
skill's other runners) read it with `readPalette`, exactly as a beat does — so the static and video map seed
...
`doctrine`: the copy carries its own answer and renders on its own.
...
`skills/splash/assets/root-template/NEWSROOM.example.md` (`brandColor`, `ground`) — the
same pair, recorded the same way, as `proof/webz-bump-emitter-rank/PALETTE.md`. They are NOT a
subject convention: `palette`'s `SUBJECT_CONVENTIONS` table holds green `#1B7F4B` for
