...
// The frozen profile shape `twin-intake`'s `profileTable` produces, with the run's own numbers:
  ...
  it("should keep an unrendered type's proven genres empty rather than inventing one", () => {
    const rows = readTypeSurvey(
      [
        ...
        "|---|---|---|---|",
        "| **Beeswarm** | Every raw observation on one axis. | — none rendered here yet | `twin-chart-beat/references/types/beeswarm.md` |",
        "| **Bar and column** | One value per category. | static, web | `twin-chart-beat/references/types/bar-and-column.md` |",
      ].join("\n"),
    );
...
describe("medium, then genre, then size — each verified before it is offered", () => {
  ...
  it("should offer scrolly for every medium that has a producer for it", () => {
    for (const medium of ["chart", "map", "image"]) {
      ...
      expect(scrolly?.producer).toBe("twin-scrolly");
    }
  });
