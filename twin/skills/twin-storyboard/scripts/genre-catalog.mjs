...
// that renders it (`producerSkill`, a directory under `skills/`) and `twin-deliver`'s own
...
// producer. A `medium: map` + `genre: web` slot therefore passed by naming `twin-chart-web` — the
// wrong producer for that medium — and `twin-map-beat`, `twin-map-web`, `twin-image-beat` and
// `twin-scrolly` were unreachable through the table entirely, while `MATRIX.md` recorded a real
...
// `delivered: true` matches a real key in twin-deliver's `FORMS_BY_GENRE` — in both directions, so
