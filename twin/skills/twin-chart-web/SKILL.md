...
- `scripts/render-preview.mjs` — renders THIS skill's seed from THIS skill's sample data (never a
  ...
  same division `render-web.mjs`'s `renderWeb` uses for a real beat, so the seed component itself never
  imports the rasteriser. `--check` re-renders and fails non-zero if the committed PNG no longer matches
  a fresh render of the seed.
