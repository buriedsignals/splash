 ...
 *
 * WHAT THIS IS A COPY OF, AND WHY IT IS A COPY. `twin-chart-video/assets/timing.ts` already knows
 ...
 * (`splash-twin/test/no-cross-skill-imports.test.ts`) — a skill directory has to build after being
 * copied, on its own, into a journalist's root, and no copy of THIS directory carries
 * `twin-chart-video` with it. What keeps the two honest is
 * `splash-twin/test/web-entrance-parity.test.ts`, which walks both files, compares the shared
 ...
 *     motion added for energy (`twin-doctrine/references/motion-grammar.md`, first anti-pattern).
 ...
 *      `splash-twin/test/web-entrance-is-an-addition.test.ts`), and it is a knob.
 *
 ...
 * (`splash-twin/test/web-entrance-is-an-addition.test.ts`) asserts every `clipPath` id in a page is
