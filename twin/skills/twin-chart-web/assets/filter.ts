// twin/skills/twin-chart-web/assets/filter.ts
//
...
//
//   - `twin-map-web` derived a filter from whether its points happened to carry more than one
...
//   - `twin-chart-web` hard-wired ONE story's dimension into the genre's own stylesheet:
...
// file is not trusted past its reach: `splash-twin/test/filters-are-declared-or-absent.test.ts`
...
// are held in step by `splash-twin/test/filter-vocabulary-parity.test.ts`. Nothing in it names a
// class, an id prefix or a genre: the scope selector and the id prefix are arguments, because
// `twin-chart-web` scopes on `.chart-figure` and `twin-map-web` on `.map-web-page`.
