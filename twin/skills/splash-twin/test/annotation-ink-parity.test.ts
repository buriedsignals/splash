 ...
 * in `twin-chart-beat/scripts/`, the live `twin/shared/twin-chart-beat/` copy a `proof/` beat
 ...
 * it does, and `twin-image-beat`'s adds six functions of its own. These do not. All three are
 ...
 * `twin/shared/twin-chart-beat/` copy only:
 *
 *   error: expect(received).toEqual(expected)
 *     "shared/twin-chart-beat/annotation-ink.mjs",
 *   - "identical",
 *   + "differs from skills/twin-chart-beat/scripts/annotation-ink.mjs",
 *   (fail) annotation-ink.mjs — every copy in the tree, discovered rather than listed >
 *          shared/twin-chart-beat/annotation-ink.mjs should be byte-identical to the canonical copy
 *   3 pass · 1 fail
 */
...
describe("annotation-ink.mjs — every copy in the tree, discovered rather than listed", () => {
  ...
  it("should find the two vendored copies that exist today, and any later one", () => {
    expect(copies.map((p) => relative(TWIN, p)).sort()).toEqual([
      "shared/twin-chart-beat/annotation-ink.mjs",
      "skills/splash-twin/assets/root-template/shared/twin-chart-beat/annotation-ink.mjs",
    ]);
  });
