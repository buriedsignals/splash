 ...
 *
 * `skills/twin-map-web/test/the-live-layer-is-in-the-artifact.test.ts` closed that for map × web.
 ...
 *               where R1 requires them; see twin-scrolly/references/scrolly-discipline.md.
 ...
 * line `twin-map-web/assets/live-map.mjs` carries and which R1 requires THERE.
 */
      ...
      if (found.length > 0)
        throw new Error(
          `this page constructs ${found.join(", ")}. On a scrolly the scroll drives the camera, so a ` +
            ...
            `twin-scrolly/references/scrolly-discipline.md.`,
        );
