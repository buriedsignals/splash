 ...
 *
 * B1.3. `twin-newsroom-charter` MEASURES a newsroom's typefaces off its own site,
 ...
 * vendored — `twin-image-beat`, `twin-map-web`, `twin-scrolly` — are outside it by that rule
 ...
 *         `twin-chart-beat/scripts/render-preview.mjs`:
 *           (fail) … twin-chart-beat/scripts/render-preview.mjs should put a recorded typeface in
 *           force before it rasterises
 *           ["skills/twin-chart-beat/scripts/render-preview.mjs", ["it never calls readTypeface(",
 *            "it never calls useTypeface("]]
 *
 *   M-T2  delete `skills/twin-map-beat/TYPEFACE.md`:
 *           (fail) … twin-map-beat should hold a TYPEFACE.md its own parser accepts
 *           "no TYPEFACE.md at skills/twin-map-beat"
 *
 *   M-T3  corrupt `origin:` to `house` in `twin-chart-beat/TYPEFACE.md` — caught through the real
 ...
 *           ["skills/twin-chart-web/scripts/render-preview.mjs", ["it never calls
...
import { parseTypeface } from "../../twin-chart-beat/scripts/render-still.mjs";
...
describe("a rasterising runner puts a RECORDED typeface in force, discovered rather than listed", () => {
  it("should find the skills the mechanism has been vendored into", () => {
    ...
    expect(SKILLS_IN_SCOPE).toContain("twin-chart-beat");
