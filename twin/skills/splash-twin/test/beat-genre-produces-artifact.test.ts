 ...
 *
 * The failure this defends against: a beat directory under `proof/` declares a genre — the
 * component and script shapes that mean "this is a video beat" or "this is a web beat" — and
 * never actually produced the artifact that genre implies. Five beats hit exactly this in one
 ...
 * genre is inferred from filenames alone, independently — a beat can trip more than one:
 *   - a `timing-contract.ts` or a `*Video.tsx` component  → genre VIDEO, requires an `.mp4`.
 *   - a `*Web*.tsx` component or a `render-web.mjs` script → genre WEB, requires an `.html`.
 *   - the beat directory's own name, or any filename in it, containing "scrolly"
 *                                                          → genre SCROLLY, also requires `.html`.
 *   - ONLY when none of the three signals above fired      → fallback genre STATIC, requires a
 ...
 *     its declared genre — asserting the artifact each DECLARED genre requires, not an exact file
 ...
 *   - Genre detection is filename-only, and can miss a real genre entirely. A beat that ships a
 ...
 *   - "Self-contained" for the WEB/SCROLLY genre is asserted by size floor only — never verified.
 ...
 *     file the genre implies — as opposed to some unrelated file that happens to share the
...
type Genre = "video" | "web" | "scrolly" | "static";
...
function detectGenres(
  beatDirName: string,
  files: string[],
): { genre: Genre; reason: string }[] {
  const basenames = files.map((f) => basename(f));
  const results: { genre: Genre; reason: string }[] = [];
  ...
  if (webComponent || renderWebScript) {
    results.push({ genre: "web", reason: webComponent ?? "render-web.mjs" });
  }
...
describe("every beat under proof/ produced the artifact its declared genre implies", () => {
  for (const beatDirName of beatDirNames) {
    const beatDir = join(PROOF_ROOT, beatDirName);
    const files = walk(beatDir);
    const genres = detectGenres(beatDirName, files);

    for (const { genre, reason } of genres) {
      const req = REQUIREMENT[genre];

      it(`should find ${req.label} under proof/${beatDirName}, which declares genre "${genre}" via ${reason}`, () => {
        ...
        expect(
          passing.length,
          `proof/${beatDirName} declares genre "${genre}" (via ${reason}) but has no ${req.ext} ` +
