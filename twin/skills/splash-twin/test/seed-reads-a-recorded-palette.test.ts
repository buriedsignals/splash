 ...
 *   - Nothing outside `skills/*&#47;scripts/render-*.mjs` is scanned at all. `twin-palette`'s own
 ...
 * 4. **`twin-dw-beat`.** Its colour arrives through a Datawrapper spec field, not a ground/accent
...
import { parsePalette } from "../../twin-chart-beat/scripts/render-still.mjs";
...
describe("a craft skill's seed reads a recorded palette, discovered rather than listed", () => {
  ...
  for (const { skill, path, label } of RUNNERS) {
    ...
    it(`${label} should call readPalette if it names ground or accent at all`, () => {
      ...
      // A runner that names neither is fine and needs nothing: twin-map-web's own
