...
describe("state and next are genuinely read-only", () => {
  ...
  // version behind it must always be — so a re-introduced hardcoded literal reddens whichever
  ...
  it("a run exactly one version behind CURRENT_SCHEMA_VERSION is refused as stale-schema", () => {
    const dir = emptyDir();
    writeFileSync(
      join(dir, "run.json"),
      JSON.stringify({
