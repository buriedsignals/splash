// legend-format — bin-boundary labels, shared by every binned legend AND by the scrolly
// orchestrator that drives those maps. It lived in skills/map-native/src/core/ and scrolly reached
// into it across the engine boundary (registry E16) — which the import guard forbids for exactly
// the reason it exists: a primitive two engines read is not one engine's to own.
//
// Moving it cost nothing it was feared to cost. Its one dependency, `./locale`, was ALREADY a
// re-export shim of lib/core/locale — measured, after a first reading claimed this would "drag
// map-native's whole locale layer" and was wrong. That claim is corrected in the registry.
//
