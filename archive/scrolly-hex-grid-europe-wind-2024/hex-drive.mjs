...
// SCAFFOLD: paint — the fields this driver reads and writes below are proof/scrolly-hex-grid-europe-protection's own. Adapt them to
// this beat's own state fields; keep the seat/paint split and the initScrollyMap/applyScrollyMap wiring.
export function applyEuropeWindHexState(root, state, context) {
  const carrier = root.querySelector("[data-hex]");
  if (!carrier) return;
  if (!root.__hex) seatEuropeWindHex(root, carrier);
  const c = root.__hex;
  ...
  // For the pair, the camera closes on the two cells and centres them.
  ...
  const toY = (y) => lerp(y, (narrow ? bandH : SH) * 0.5 + (y - pmy) * 1.5, pairE);
  for (const t of c.tiles) {
    ...
    const target = t.rateClass === null ? c.colours.originFill : mixEuropeWindHex(c.colours.fills[t.countClass], c.colours.fills[t.rateClass], rate);
    const colour = mixEuropeWindHex(c.colours.empty, target, t.rateClass === null ? 1 : fill);
...
function mixEuropeWindHex(a, b, t) {
...
function seatEuropeWindHex(root, carrier) {
  ...
  const repaint = () => {
    if (root.dataset.state) applyEuropeWindHexState(root, JSON.parse(root.dataset.state));
  };
