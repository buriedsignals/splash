...
gorge.forEach((p, i) => { const d = havKm(p, START); if (d < best) { best = d; s0 = i; } });
...
const resample = (coords) => {
  ...
  let carry = 0, from = coords[0];
  for (let i = 1; i < coords.length; i++) {
    let segLen = havKm(from, coords[i]);
    while (carry + segLen >= STEP_KM) {
      const t = (STEP_KM - carry) / segLen;
      const np = [from[0] + (coords[i][0] - from[0]) * t, from[1] + (coords[i][1] - from[1]) * t];
      out.push(np);
      from = np; segLen = havKm(from, coords[i]); carry = 0;
    }
    carry += segLen; from = coords[i];
  }
  return out;
};
...
const smoothMA = (coords, w, passes) => {
  ...
  for (let p = 0; p < passes; p++) {
    c = c.map((_, i) => {
      let sx = 0, sy = 0, n = 0;
      for (let j = Math.max(0, i - w); j <= Math.min(c.length - 1, i + w); j++) { sx += c[j][0]; sy += c[j][1]; n++; }
      return [sx / n, sy / n];
    });
  }
  return c;
...
const A = toXY(sm[0]), B = toXY(sm[sm.length - 1]);
const AB = [B[0] - A[0], B[1] - A[1]], len2 = AB[0] ** 2 + AB[1] ** 2;
const path = sm.map((p) => {
  const P = toXY(p);
  const t = ((P[0] - A[0]) * AB[0] + (P[1] - A[1]) * AB[1]) / len2;
  const proj = [A[0] + t * AB[0], A[1] + t * AB[1]];
  return toLL([proj[0] + (P[0] - proj[0]) * DAMP, proj[1] + (P[1] - proj[1]) * DAMP]);
});

mkdirSync(dirname(OUT), { recursive: true });
writeFileSync(OUT, JSON.stringify(path));

let len = 0; for (let i = 1; i < path.length; i++) len += havKm(path[i - 1], path[i]);
console.log(`cesium-path: clip ${clip.length} → resample ${even.length} → smooth → ${path.length} pts · ${len.toFixed(1)} km`);
const bear = (a, b) => { const r = Math.PI / 180; const y = Math.sin((b[0] - a[0]) * r) * Math.cos(b[1] * r); const x = Math.cos(a[1] * r) * Math.sin(b[1] * r) - Math.sin(a[1] * r) * Math.cos(b[1] * r) * Math.cos((b[0] - a[0]) * r); return (Math.atan2(y, x) * 180) / Math.PI; };
...
let prev = null, hs = [];
for (let i = 0; i + stepPts < path.length; i += stepPts) { const h = bear(path[i], path[i + stepPts]); if (prev !== null) { let d = h - prev; while (d > 180) d -= 360; while (d < -180) d += 360; hs.push(d.toFixed(0)); } prev = h; }
