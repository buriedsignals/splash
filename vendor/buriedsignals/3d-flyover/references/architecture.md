...
- **Bank (roll)** = lean *into* the turn — the helicopter tell. Measure turn rate as the bearing change
...
```bash
npx remotion still   src/index.ts <Comp> out.png --frame=N --gl=angle --timeout=180000   # validate framing/bank first
