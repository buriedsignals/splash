...
const WATER_FILL = flag("--water", "#aac9e0");
...
const gate = await page.evaluate(
  async ({ key, style, padding, styleDefinition, bounds, settleMs, width, height, waterFill }) => {
    ...
    // THIS BEAT'S OWN WATER, DERIVED FROM ITS PALETTE — not the literal the skill's copy carries.
    // Measured 2026-08-23 on this beat's first bake: the skill's `#aac9e0` has relative luminance
    // 0.557 against a ground of 0.009 and an accent of 0.426, so the SEA WAS BRIGHTER THAN EVERY
    // DATA MARK ON A MAP ABOUT LAND, and an accent circle over it measured 1.27:1 — below the 3:1
    // non-text floor the palette refuses at, on the only mark this beat has. `--water` takes the
    // colour instead, defaulting to the skill's literal so every existing caller bakes unchanged.
