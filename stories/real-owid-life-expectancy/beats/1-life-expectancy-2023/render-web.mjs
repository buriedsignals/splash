...
/* B5.1, and the conflict that dissolves with the ruling. The viewport keeps the PLATE's aspect,
   ...
   AND THIS BEAT PUTS THE ASPECT BACK, for the one case the reasoning above does not cover: a study
   set that is THE WHOLE WORLD. Driven with a real key at 1600x900, the live canvas came out
   1566 x 608 — an aspect of 2.58 against the world's own 1.47 — so the fit was height-bound and
   left 800px of horizontal margin, which MapLibre filled with a SECOND and a THIRD painted world.
   Turning renderWorldCopies off instead removed the repeats and cropped the claim: the camera
   clamped to make one world fill the width and cut everything below 20.8°S, taking Lesotho — one of
   the six countries the title names — off the screen. Neither is acceptable, and both come from the
   same cause: a container wider than the world is. Holding the live viewport to the plate's own
   aspect makes the fit width-bound, so exactly one world fills exactly one box and nothing is
   repeated or cut. A beat whose study set fits inside one world should delete this rule. */
...
async function ensurePlate(plateDir) {
  ...
  const result = spawnSync(
    "bun",
    [join(HERE, "bake-plate.mjs"), "--width", String(PLATE_SIZE), "--out", plateDir],
