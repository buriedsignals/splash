 ...
 *
 *  THE WORLD, and that is why this file's frame is no longer square. Rule 12 says pick the frame
 *  from the geography, and the geography here is every inhabited country: in Web Mercator, 66°S to
 *  84°N over a full turn of longitude is close to 2:1, so a square frame would spend half its
 *  pixels on empty ocean above and below the world and shrink every country by a factor of two.
 *  The format's own bake takes ONE `--size` and applies it to both axes (`width: size, height:
 *  size`); this copy takes `--width` and `--height`, and the height is DERIVED from the width and
 *  the latitude range by `minFrameHeightPx`, the function already in this file.
 *
 *  84°N is Natural Earth's own northern limit and the top of the Web Mercator square. 60°S cuts
 *  below the southernmost inhabited land (Puerto Williams, 55°S) and above the Antarctic shapes
 *  this beat's geography does not carry.
 *
 *  `dataviz-dark`, not `dataviz-light`, because this story's recorded ground is `#16191B`. A light
 *  plate under a dark ground is the exact disagreement `plateFollowsGround` measures. */
const BEAT = {
  bounds: [
    [-180, -60],
    [180, 84],
  ...
  style: "dataviz-dark",
  anchors: {
    // Where each named region's own direct label hangs — projected here, once, rather than guessed
    // as a pixel offset (rule 4). Nigeria is the lowest reading on the map and the argument's own
    // end; Japan is the high reference a reader can actually see (the three readings above it —
    // Monaco, San Marino, Hong Kong — are all under a pixel at this camera).
    subject: [2.0, 9.1],
    comparison: [147.0, 39.0],
  },
};
...
const size = Number(flag("--width", flag("--size", "1200")));
// DERIVED, never typed: the least height that holds this beat's own latitude range at this width
// without cropping it. `assertCameraReachesBounds` throws with this same number in its message when
// a frame is too short, so taking the number from the function that computes it is the one way the
// two cannot disagree.
const frameHeight = Number(flag("--height", "0")) || minFrameHeightPx(size, BEAT.bounds[0][1], BEAT.bounds[1][1]);
...
// THE KEY HAS ONE HOME, and it is not a fixed number of `..` segments. The file this was copied
// from lives two levels under the repository root and hard-coded `../../.env` for it; a beat
// under `stories/<slug>/beats/<id>/` is FOUR levels down, so that same literal resolves to the
// story's own folder, where no `.env` has ever existed. `splashEnvPath` walks up to the nearest
// ancestor that declares `#shared/*`, which is the root by definition wherever a beat sits.
...
const missingShapes = LIFE_EXPECTANCY_STUDY.filter((code) => !byKey.has(code));
...
const payload = LIFE_EXPECTANCY_STUDY.map((code) => {
