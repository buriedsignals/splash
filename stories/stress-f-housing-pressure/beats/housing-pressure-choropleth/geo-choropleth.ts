 ...
 *
 * Eight of the nine codes `source/data.csv` carries. Kosovo IS declared here, under `KOS` — the
 * tree's own mapping convention (Natural Earth's `ADM0_A3`) — never under `XKX`, the statistics
 * office's own code the frozen csv actually uses; `render-web.mjs`'s own `alias` maps one to the
 * other at the join, exactly the way the article's own second sentence says to. Greenland (`GRL`,
 * a value the csv carries) is deliberately NOT declared: a self-governing territory of the Kingdom
 * of Denmark, at 56,000 inhabitants, is not a plausible member of a "housing pressure across
 * Europe" study set, the article gives it no sentence the way it gives Kosovo and Malta one, and there is no
 * shape for it in this beat's own `countries.geojson` either — `render-web.mjs` declares it an
 * `expectedExtraValues` stray instead of aliasing it in, which is the honest way to say "the source
 * covers ground this study does not claim."
 */
export const HOUSING_STUDY = [
  "DEU",
  "ESP",
  "FRA",
  "ITA",
...
/** Class boundaries, in percentage points of income spent on housing. Round numbers in the data's
 *  own unit, not quantiles — `geo-discipline.md`'s own rule for a legible ramp. Six breaks (seven
 *  classes) rather than the five a purely comparable 3-18% range would need, because Malta's own
 *  143% (flagged by the article as drawn from a different survey, kept because the office
 *  publishes it in the same table) has to sit in a class of its own rather than share the top one
 *  with Kosovo's fully-comparable 18% — see `render-web.mjs`'s own `caveat` for the sentence that
 *  states why. */
export const PRESSURE_BREAKS = [5, 8, 11, 14, 17, 20];

// ── Reading the source ─────────────────────────────────────────────────────────────────────────

/** This beat's own frozen csv: `code,country,pressure` (`stories/stress-f-housing-pressure/source/data.csv`,
 *  read directly — nothing here is a second copy of a journalist's frozen material). Every
 *  `pressure` cell carries its own unit INSIDE it ("12 %", not 12) — the profiler that ran at
 *  intake typed the column `text` with `min`/`max`/`sum` all null over exactly this, and recorded
 *  no reason why. `stripUnit` below is this beat's own explicit decision about it, made once, here,
 *  never inside a skill.
...
/** Trap 1: every cell in `source/data.csv` reads "12 %", the unit INSIDE the cell — this is the
 *  one, explicit place that unit is stripped. A cell that carries no trailing "%" (or carries a
 *  different one) is left alone and falls through to `readHonestNumber`'s own refusal, named by the
 *  raw string rather than silently coerced to `NaN`. */
function stripPercentUnit(raw: string): string {
...
export function valuesFromCsv(csv: string): Map<string, number> {
  ...
  const valueAt = columns.indexOf("pressure");
  if (codeAt < 0 || valueAt < 0)
    throw new Error(`csv has no code / pressure column, got: ${header}`);
  ...
  for (const row of rows) {
    ...
    if (raw === "") continue;
    const value = readHonestNumber(stripPercentUnit(raw));
    if (value === null)
      throw new Error(
        `${cells[codeAt]}: "${raw}" is not a value this join can read honestly — neither a plain number nor a thousands-grouped one, once its unit is stripped`,
      );
