// stories/stress-t-europe-recycling/beats/europe-recycling-map/bake-plate.mjs
//
// A COPY of `skills/map-beat/scripts/bake-plate.mjs`, not an import — a story workspace may not
// reach into a skill at runtime. Every function below carries the source's own `@parity` tag and is
// byte-identical to it; what differs is this beat's own BEAT block (camera, style, anchors), its
// study set, and its default shapes path, each tagged where it differs.
//
 ...
 *
 * @parity-exempt: a beat's camera is the one thing that cannot be shared. The bounds are the seed's
 * own Europe, unchanged, because this story's geography IS the seed's — Iceland to the Arctic
 * circle, Gibraltar to the Black Sea, near-square on purpose (geo-discipline rule 12).
 *
 * The STYLE is this beat's own and is not a taste decision: `PALETTE.md` records a charcoal ground
 * (#16191B), and `plateFollowsGround` refuses a beat whose plate sits on the other side of the
 * theme from its declared ground — the delivered-route defect (white labels over a pale basemap)
 * that guard was written for. A dark ground takes a dark basemap.
 */
const BEAT = {
  bounds: [
    [-26, 36],
  ...
  style: "dataviz-dark",
  anchors: {
    // Germany, the subject: the centre its outline is pointed at, and where its label hangs — west
    // of it, over the North Sea, where no reporting country's shape is.
    subject: [10.45, 51.2],
    label: [1.6, 55.4],
    // Macedonia, the other end of the gap the takeaway names. Its label hangs south-east of it,
    // over the Aegean, clear of Greece's own shape.
    comparison: [21.7, 41.6],
    comparisonLabel: [17.2, 39.4],
  },
};
...
const outDir = flag("--out", join(import.meta.dirname, `plate-${size}`));
const shapesPath = flag("--shapes", join(import.meta.dirname, "countries.geojson"));
...
const missingShapes = EUROPE_STUDY.filter((code) => !byKey.has(code));
...
const payload = EUROPE_STUDY.map((code) => {
...
// construction. This is the same question asked of the STUDY SET, which is what `EUROPE_STUDY` is.
const empty = shapes.filter((s) => s.rings.length === 0).map((s) => s.key);
if (empty.length > 0)
  throw new Error(
    `this camera crops ${empty.length} of the ${EUROPE_STUDY.length} shapes this beat declares, entirely: ` +
