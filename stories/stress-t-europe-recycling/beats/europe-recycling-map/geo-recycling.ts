/**
 * The pure half of THIS beat: the join, the classes, the ramp, the ring arithmetic.
 *
 * A COPY of `skills/map-beat/assets/geo.ts`, not an import — a story workspace is not a skill
 * and may not reach into one at runtime. Every function below is byte-identical to its source
 * and carries the source's own `@parity` tag; only the DATA block (the study set, the alias
 * table, the declared no-data list, the class breaks) and the csv reader are this beat's own,
 * and each says so where it differs.
 *
 ...
 *
 * This is the beat DECLARING what it claims to show. The article asks for "the whole continent",
 * so the study set is all forty-two European shapes this beat's frozen geography carries — not the
 * eleven that happen to have a reading. Every key here must find a shape, and every one must find
 * a value or be declared below; both are checked, loudly. Thirty-one are declared, which is the
 * honest shape of this source: a reader sees where the survey does not reach instead of seeing a
 * continent that appears to have been surveyed.
 */
export const EUROPE_STUDY = [
  "ALB",
  "AND",
  "AUT",
  "BEL",
 ...
 *
 * THE WHOLE DIRT OF THIS STORY LIVES HERE, DELIBERATELY. The frozen csv keys its rows by a country
 * NAME a person typed, not by a code: `Holland` for the shape Natural Earth calls `NLD`,
 * `Macedonia` for `MKD`, `Czech Republic` for `CZE`, and `Belgiumm` — a plain typo — for `BEL`.
 * Nothing in this toolchain maps a name to a shape key, so this table is written by hand, once,
 * where a reader of the beat can see every judgement it makes.
 *
 * It is written as shape → SOURCE STRING rather than as a cleaned csv on purpose: keyed this way,
 * `unmatchedValues` measures the RAW names the journalist's file actually carries, so a name this
 * table forgets is refused BY THAT NAME instead of vanishing from the map. A build step that
 * silently rewrote `Belgiumm` to `BEL` upstream would have taken that guard away.
 */
export const RECYCLING_ALIAS: Record<string, string> = {
  AUT: "Austria",
  BEL: "Belgiumm",
  CZE: "Czech Republic",
  DEU: "Germany",
...
/**
 * The shapes this source genuinely does not report: the thirty-one European countries whose
 * national agencies are not in the March 2025 collection at all. Declared, so that any OTHER shape
 * arriving without a value is a bug and throws — which is what catches a name this beat's own alias
 * table gets wrong.
 */
export const RECYCLING_EXPECTED_NO_DATA = [
  "ALB",
  "AND",
  "BGR",
  "BIH",
...
/** Class boundaries in per cent of municipal waste recycled. Six classes on round decades: under
 *  20, then 20s, 30s, 40s, 50s, and 60 and over. The 20–29 class is EMPTY in this survey and is
 *  still drawn on the legend — that gap between Macedonia at 18.4 and Spain at 36.9 is the shape of
 *  the argument, and collapsing the scale onto the values present would hide it. */
export const RECYCLING_BREAKS = [20, 30, 40, 50, 60];
...
/**
 * The frozen survey, keyed by the source's own COUNTRY NAME — dirt and all.
 *
 * @parity-exempt: the seed reads an OWID csv keyed by code and filtered to a year; this source is
 * keyed by a hand-typed name and carries no usable year (its `survey_date` column mixes
 * `2025-03-01`, `01/03/2025` and `March 2025`, which is why `intake` typed it `text`). The join
 * below, the guard below it, and everything after are the source file's, unchanged.
 *
 * TWO THINGS THIS DOES THAT THE SEED DOES NOT, both because the toolchain does not do them:
 *
 * 1. **It refuses a duplicated row instead of collapsing it.** `intake`'s profiler already found
 *    this one — `source/profile.json` says `duplicates.count: 1`, rows 10 and 11, Sweden, byte for
 *    byte — and NOTHING downstream of the profile reads that field. A `Map.set` would have taken
 *    the second Sweden silently and the map would have been right by luck. So the count is
 *    recomputed here, at the join, and an EXACT repeat is dropped with its indices named while a
 *    repeat carrying a DIFFERENT value throws: two disagreeing readings for one country is not a
 *    duplicate, it is a source this beat cannot draw.
 * 2. **It returns what it dropped**, so the render can print it rather than the author having to
 *    trust that it happened.
 */
export function ratesFromCsv(csv: string): {
  values: Map<string, number>;
  droppedDuplicates: { country: string; atRows: number[] }[];
} {
  const [header, ...rows] = parseCsvRows(csv.trim());
  const columns = header ?? [];
  const countryAt = columns.indexOf("country");
  const rateAt = columns.indexOf("recycling_rate");
  if (countryAt < 0 || rateAt < 0)
    throw new Error(`csv has no country / recycling_rate column, got: ${header}`);

  const values = new Map<string, number>();
  const seenAt = new Map<string, number[]>();
  const droppedDuplicates: { country: string; atRows: number[] }[] = [];

  rows.forEach((cells, index) => {
    const country = (cells[countryAt] ?? "").trim();
    if (!country) return;
    ...
    if (value === null)
      throw new Error(
        `${country}: "${raw}" is not a value this join can read honestly — neither a plain number nor a thousands-grouped one`,
      );
    const already = values.get(country);
    const rowNumbers = seenAt.get(country) ?? [];
    rowNumbers.push(index);
    seenAt.set(country, rowNumbers);
    ...
    if (already !== value)
      throw new Error(
        `${country} appears twice in the frozen source with different readings (${already} and ${value}, ` +
          `rows ${rowNumbers.join(" and ")}). Two disagreeing readings for one country is not a duplicate row — ` +
          `nothing here may choose between them.`,
      );
    droppedDuplicates.push({ country, atRows: [...rowNumbers] });
  });

  return { values, droppedDuplicates };
}
