...
- **`still.png`** — a raster copy, for a system that cannot take the vector
- **`still.svg`** — the vector file — this is the one to give the CMS, and it stays sharp at any size

## Where it goes in the article

replaces the headline's claim, before the raw counts are cited
...
> Source: municipal safety incident report and district population estimates
...
> residents is a single population figure per district, not adjusted to the same year the incidents were logged
