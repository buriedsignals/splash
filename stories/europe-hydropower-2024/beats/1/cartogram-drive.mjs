...
// SCAFFOLD: paint — the fields this driver reads and writes below are proof/scrolly-cartogram-europe-lowcarbon's own. Adapt them to
// this beat's own state fields; keep the seat/paint split and the initScrollyMap/applyScrollyMap wiring.
export function applyEuropeHydroState(root, state) {
  if (!root.__carto) seatEuropeHydro(root);
...
function seatEuropeHydro(root) {
  ...
  const repaint = () => {
    if (root.dataset.state) applyEuropeHydroState(root, JSON.parse(root.dataset.state));
  };
