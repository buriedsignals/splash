/**
 * The pure half of this map beat: the release reader, the join, the classes, the ramp, the ring
 * arithmetic.
 *
 * Copied from `skills/map-beat/assets/geo.ts` — the seed — and adapted to this story. Every
 * function kept verbatim carries the `@parity` tag it was copied with, so
 * `splash/test/geo-parity.test.ts` compares it against every other copy in the tree; the two
 * things that are genuinely this beat's own (`yieldsFromRelease`, `STATE_BY_NAME`) are new names
 * and belong to nobody else.
 *
 * Nothing here knows about a browser, a tile, a key or a camera. That is what makes the join
 * testable at all. It also means this file does NOT derive furniture colours: `deriveFurniture`
 * owns the ink/muted/grid rule in `#shared/chart-beat/render-still.mjs`, and the render script
 * calls it in node and passes the result in.
 */
...
/**
 * The study set, as Natural Earth admin-1 `postal` keys — the two-letter USPS abbreviation, which
 * is the one field in `ne_50m_admin_1_states_provinces` that is a stable join key for the United
 * States. (`iso_3166_2` carries the same information as `US-XX`; `adm1_code` is Natural Earth's
 * own and matches nothing a publisher uses.)
 *
 * Forty-nine shapes: the lower forty-eight states and the District of Columbia. Alaska and Hawaii
 * are OUT of the study set and out of the frame, and `STORYBOARD.md`'s `limits` says so — both are
 * among the states USDA does not report separately, so nothing is being hidden by the frame that
 * the frame could have shown.
 *
 ...
 *
 * EMPTY, and that is a fact worth stating rather than a line worth deleting. The seed's own alias
 * table exists because Our World in Data codes Kosovo `OWID_KOS` where Natural Earth says `KOS`.
 * Here the two sides do not share a key at all: USDA writes state NAMES in prose ("North Dakota"),
 * Natural Earth carries postal codes. The bridge is `STATE_BY_NAME` below, which is a NAME table
 * and not an alias table, and it is checked the same way — a name it cannot read is refused, never
 * skipped.
 */
export const HONEY_ALIAS: Record<string, string> = {};

/**
 * The shapes this source genuinely does not report — twenty-nine of the forty-nine in frame.
 *
 * They are not missing by accident and they are not suppressed one by one: USDA publishes twenty
 * states individually and folds every other state into ONE line, "Other States 5/ 6/", whose
 * footnote 5 reads *"Includes data for States not published in this table."* So a state on this
 * list has a real value that exists and is pooled somewhere the map cannot reach — which is a
 * different sentence from "nobody measured it", and the beat's caveat has to say which.
 *
 * Declared, so that any OTHER shape arriving without a value is a bug and throws.
 */
export const HONEY_EXPECTED_NO_DATA = [
  "AL",
  "AR",
  "AZ",
  "CO",
...
/** Class boundaries in pounds of honey per colony. Six classes, the top open.
 *
 *  `48` is not a round number chosen for the scale's sake: it is the United States average USDA
 *  reports in its own first paragraph ("Yield per colony averaged 48.0 pounds"). Putting it on a
 *  class boundary is what makes "above the national average" something a reader SEES on the map
 *  rather than something the caption asserts — eight of the twenty published states sit at or
 *  above that line and twelve below it. */
export const HONEY_BREAKS = [35, 42, 48, 56, 67];
...
/**
 * Natural Earth's `postal` code for a state, keyed by the name USDA writes in its own release.
 *
 * A NAME table, not an alias table: the publisher and the shapes share no key at all. It carries
 * every one of the fifty states and the District of Columbia rather than only the twenty USDA
 * publishes, so a release that starts publishing a twenty-first state is joined rather than
 * refused — and a name outside it is refused by `yieldsFromRelease` rather than skipped.
 */
...
/** A NASS release row whose "State" cell is an AGGREGATE of other rows rather than a place.
 *
 *  Two of them sit inside this table — "Other States 5/ 6/" and "United States 6/ 7/" — and both
 *  carry their footnote markers INSIDE the cell, which is why this reads the cell rather than the
 *  row's position. The seed had no case for this because OWID ships one row per entity; a national
 *  statistics release does not, and a reader that took every `"d"` row as a place would have put
 *  the United States total on the map as a state. */
const AGGREGATE_STATE_RE = /^(Other States|United States)\b/;

/** Everything a NASS release table is, read once.
 *
 *  `values` is postal code → pounds per colony for the states published individually. `aggregates`
 *  is the rows that are TOTALS of other rows, kept rather than dropped, so the render can print
 *  them and the caveat can quote them. `year` is read off the table's own title row and returned,
 *  so a beat that claims a year can be checked against the file rather than trusted.
 *
 *  Every refusal here is loud. A "d" row whose state name is not in `STATE_BY_NAME` and is not an
 *  aggregate throws; a non-blank yield cell that is not a number a human wrote throws. A cell
 *  silently dropped as if absent is a value that vanished, not one that was never there.
 *
 *  @parity-exempt: the seed reads a tidy one-row-per-entity OWID csv keyed by ISO code; this reads
 *  a USDA release table whose first nine rows are titles, headers and units, whose entity column
 *  is a prose name, and two of whose rows are aggregates of the others. The join below is the
 *  same; only the shape of the file differs. */
export function yieldsFromRelease(csv: string): {
  values: Map<string, number>;
  aggregates: { label: string; yield: number | null }[];
  year: number;
  dataRows: number;
} {
  const rows = parseCsvRows(csv.replace(/\r\n?/g, "\n").trim());
  const titles = rows.filter((r) => r[1] === "t").map((r) => r[2] ?? "");
  const titled = titles.map((t) => /:\s*(\d{4})\s*$/.exec(t)).find(Boolean);
  if (!titled)
    throw new Error(
      `this release table names no year in any of its title rows (${titles.length} found), so ` +
        `nothing can check the year the beat claims against the file it read`,
    );
  const year = Number(titled[1]);

  // The units row states what column 4 is measured in. Reading it rather than assuming it is what
  // stops this beat from drawing pounds of honey on a scale labelled colonies: the table has six
  // numeric columns and five different units, declared in one row that is not a header.
  const units = rows.find((r) => r[1] === "u");
  if (!units) throw new Error("this release table carries no units row");
  if ((units[4] ?? "").trim() !== "(pounds)")
    throw new Error(
      `column 4 of this release is measured in ${JSON.stringify(units[4])}, not "(pounds)" — the ` +
        `beat draws pounds per colony and will not draw a column it cannot name`,
    );

  const values = new Map<string, number>();
  const aggregates: { label: string; yield: number | null }[] = [];
  let dataRows = 0;
  for (const row of rows) {
    if (row[1] !== "d") continue;
    const label = (row[2] ?? "").trim();
    if (label === "") continue; // the blank spacer rows the release lays out its table with
    dataRows += 1;
    const raw = (row[4] ?? "").trim();
    const value = raw === "" ? null : readHonestNumber(raw);
    if (raw !== "" && value === null)
      throw new Error(
        `${label}: "${raw}" is not a value this join can read honestly — neither a plain number ` +
          `nor a thousands-grouped one`,
      );
    if (AGGREGATE_STATE_RE.test(label)) {
      aggregates.push({ label, yield: value });
      continue;
    }
    const postal = STATE_BY_NAME[label];
    if (!postal)
      throw new Error(
        `this release names ${JSON.stringify(label)} and no shape key answers to it — it is ` +
          `neither one of the fifty states and the District of Columbia nor one of the two ` +
          `aggregate rows this table carries. A name skipped here is a state that would have ` +
          `rendered as no-data and looked entirely legitimate.`,
      );
    if (value !== null) values.set(postal, value);
  }
  return { values, aggregates, year, dataRows };
}
...
// ── Rule 7 on a DARK plate: the water tint this beat has to derive for itself ──────────────────
//
// `geo-discipline.md` rule 7 fixes two hexes: water `#aac9e0`, no-data `#b9b9b9`. Both are LIGHT
// constants stated as absolutes, and this story's recorded ground is `#16191B`. Measured on the
// plate this camera actually captured, sampled at four ocean points and three inland points:
//
//   * MapTiler `dataviz-dark` serves water `#141414` against land `#292929` — both PURE NEUTRAL
//     GREYS, which is the one thing rule 7 says water may never be, and 10.27 ΔE76 apart against
//     the 23.77 the same rule's own light-plate pair (`#F7F7F7` / `#aac9e0`) achieves.
//   * `skills/map-beat/scripts/bake-plate.mjs` — the seed this beat's bake is copied from — applies
//     NO water override at all, where `map-web`'s and `scrolly`'s bakes both set `#aac9e0` before
//     capture. One rule, three copies of one bake, and the copy whose skill owns the doctrine is
//     the one that does not carry it.
//   * Rule 7's own no-data `#b9b9b9` has relative luminance 0.486 here, BRIGHTER than every class
//     of this beat's ramp (top class 0.208). Painted on this plate it would make a state nobody
//     measured the brightest thing on the map.
//
// So the tint is DERIVED rather than taken. The convention's own hue is kept — `#aad3df`, OSM
// Carto's water, which rule 7 names first — and the land is walked toward it by the smallest step
// that reaches the separation the light plate already has. One anchor, one target, no free
// parameter: on a light plate the convention DARKENS the land toward that blue; on a dark plate the
// only direction with anywhere to go is up.
