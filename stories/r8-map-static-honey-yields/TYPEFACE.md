...
Space Grotesk does not resolve on the machine that renders this beat, and Courier New sets every digit on one grid, which is wrong for a map whose numbers are read off labels. The stated fallback is recorded as a choice with the gap named.
