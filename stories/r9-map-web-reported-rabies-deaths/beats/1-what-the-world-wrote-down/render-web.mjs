...
// to `RABIES_2024_STUDY` and the map drew 42. The two country NAMES stay in the wording because
...
const SEED = {
  ...
  source: "World Health Organization, Global Health Observatory — NTD_RAB2, \u201cReported number of human rabies deaths\u201d (retrieved 23 August 2026)",
  basemapCredit: "shapes: Natural Earth 1:50m Admin 0 Countries · basemap © MapTiler, © OpenStreetMap",
  // The caveat is NOT typed here. Every number in it — how many filed nothing, how many reported
  // zero, what the reported total is — is a product of the join this script performs, so it is
  // built in `render()` from the joined rows (`claimSentences`). Typed here, it would keep its
  // wording after a code moved between the three states.
...
const PLATE_BOX_ASPECTS = "0.877,2.375";
...
const DEFAULT_VALUES_PATH = join(HERE, "rabies-deaths-2024.csv");
...
const OUTPUT_NAME = "reported-rabies-deaths-2024.html";
// The subject is the country that reported the most; the comparison is the largest of the
// countries that reported NOTHING, which is the other half of the claim and has no value at all.
const SUBJECT_KEY = "AFG";
const COMPARISON_KEY = "IND";
// Every country in WHO's own 2024 row whose cell reads "No data" — declared, per shape, so the join
// refuses rather than paints a silence as a legitimate reading. Written by `prepare-inputs.mjs`
// into JOIN.json and read back here, never typed.
 ...
 */
export function checkClaim(values, { drawn, noData }) {
  const violations = [];
  ...
  // The comparison is a country with NO reading, and that is checked as such: if India ever files,
  // this beat's second half is about a different country and the sentence must be rewritten.
  if (values.has(COMPARISON_KEY))
    violations.push(
      `the claimed comparison is ${COMPARISON_KEY}, named because it filed NOTHING for this year — but the source now reports ${values.get(COMPARISON_KEY)} for it`,
    );
  if (!noData.includes(COMPARISON_KEY))
    violations.push(`${COMPARISON_KEY} is not among the ${noData.length} shapes this join left with no reading`);

  const reported = entries.reduce((sum, [, v]) => sum + v, 0);
  if (reported !== JOIN_RECORD.totalDrawn)
    violations.push(
      `the joined values sum to ${reported}, but JOIN.json recorded ${JOIN_RECORD.totalDrawn} as drawable — one of the two is stale`,
    );
  // The register's own total and the total this map can draw are two numbers, and the gap between
  // them is exactly the readings Natural Earth gives no shape to. If that stops being true, a
  // reading has gone missing somewhere else and the caveat no longer accounts for it.
  const undrawn = JOIN_RECORD.readingsThatLandOnNoShape.reduce((sum, r) => sum + r.value, 0);
  if (JOIN_RECORD.totalReported - JOIN_RECORD.totalDrawn !== undrawn)
    violations.push(
      `${JOIN_RECORD.totalReported} reported less ${JOIN_RECORD.totalDrawn} drawn is ` +
        `${JOIN_RECORD.totalReported - JOIN_RECORD.totalDrawn}, but the readings with no shape sum to ${undrawn}`,
    );
  if (drawn !== JOIN_RECORD.drawn)
    violations.push(`this render draws ${drawn} shapes, JOIN.json recorded ${JOIN_RECORD.drawn}`);
  const zeros = entries.filter(([, v]) => v === 0).length;
  if (zeros !== JOIN_RECORD.reportedZero)
    violations.push(`${zeros} countries report zero here, JOIN.json recorded ${JOIN_RECORD.reportedZero}`);
  ...
  return {
    ...
    comparison: { key: COMPARISON_KEY, value: null },
    reported,
    registered: JOIN_RECORD.totalReported,
    undrawn: JOIN_RECORD.readingsThatLandOnNoShape,
    readings: entries.length,
    zeros,
    silent: noData.length,
    drawn,
  };
}

export function claimSentences({ breaks, claim, year, names }) {
  const nameOf = (key) => names.get(key) ?? key;
  const subjectName = nameOf(claim.subject.key);
  const comparisonName = nameOf(claim.comparison.key) ?? "India";
  const title =
    `WHO estimates 59 000 rabies deaths a year. For ${year} the world's health ministries ` +
    `wrote down ${claim.registered.toLocaleString("en-GB").replace(/,/g, " ")} — and ${claim.silent} of ` +
    `the ${claim.drawn} countries WHO asks, ${comparisonName} among them, filed nothing at all`;
  const legendCaption = `Human rabies deaths reported to WHO, ${year}`;
  const caveat =
    `REPORTED deaths, not deaths. Grey is not zero: ${claim.silent} countries filed nothing for ` +
    `${year}, ${comparisonName} and China and Pakistan among them, and are drawn as no reading. ` +
    `${claim.zeros} countries did file, and reported zero — those are shaded as the lowest class. ` +
    `Raw counts of people, never a rate: this file carries no population. ` +
    claim.undrawn
      .map(
        (r) =>
          `${r.name} reported ${r.value} and has no shape of its own in Natural Earth, so those ` +
          `${r.value} are drawn nowhere on this map — ${claim.reported.toLocaleString("en-GB").replace(/,/g, " ")} of ` +
          `the ${claim.registered.toLocaleString("en-GB").replace(/,/g, " ")} are.`,
      )
      .join(" ");
  const alt =
    `A world choropleth of ${claim.drawn} countries shaded by the number of human rabies deaths each ` +
    `reported to WHO for ${year}, in ${breaks.length + 1} classes from 0 to ${breaks[breaks.length - 1]} ` +
    `and over. ${claim.silent} countries filed nothing and are painted in a neutral no-reading fill, ` +
    `including ${comparisonName}, China and Pakistan. The darkest countries are ${subjectName}, which ` +
    `reported ${claim.subject.value}, the highest of any country, together with the Philippines, ` +
    `Ghana and South Sudan. The ${claim.readings} countries that did file reported ` +
    `${claim.registered.toLocaleString("en-GB").replace(/,/g, " ")} deaths between them, against WHO's ` +
    `standing estimate of about 59 000 a year.`;
  return { title, legendCaption, alt, caveat };
}
...
async function ensurePlate(plateDir) {
  ...
  const result = spawnSync(
    ...
    { cwd: splashRoot(HERE), stdio: "inherit" },
  );
...
async function render({ valuesPath, shapesPath, plateDir, outDir, name = OUTPUT_NAME }) {
  ...
  const shapes = joinShapes(RABIES_2024_STUDY, geometry.shapes);
  ...
  // Every silence declared, per shape: `expectedNoData` is the list of countries whose own 2024
  // cell in WHO's file reads "No data". A shape in that list with a value, or a shape with no value
  // that is not in it, both throw — which is what stops a silence being painted as a reading.
  const { rows, noData } = joinValues(RABIES_2024_STUDY, values, {
    alias: {},
    expectedNoData: SILENT_SHAPES,
  });
  ...
  const { names: csvNames, year } = labelsFromCsv(csv);
  // The csv carries a name only for a country that FILED. This beat draws 94 that did not, and a
  // reader hovers those too — so the reader-facing spelling comes from `region-names.json`, written
  // by `prepare-inputs.mjs` out of WHO's own COUNTRY dimension, for every shape in the study set.
  const names = new Map(
    Object.entries(JSON.parse(readFileSyncSync(join(HERE, "region-names.json"), "utf8"))),
  );
  for (const [key, spelled] of csvNames)
    if (names.get(key) !== spelled)
      throw new Error(
        `two spellings for ${key}: the csv says "${spelled}", region-names.json says "${names.get(key)}"`,
      );
  ...
  const claim = checkClaim(values, { drawn: named.length, noData });

  const { title, legendCaption, alt, caveat } = claimSentences({
    breaks: RABIES_BREAKS,
    claim,
    year,
    names,
  });
  ...
  const { outPath } = await renderMapWeb({
    ...
    table: RegionTable,
    props: {
      geometry: { frame: geometry.frame, shapes: named, cannotCover: geometry.cannotCover ?? null, studySet: geometry.studySet ?? null, labelFrame: geometry.labelFrame ?? null, boxAspects: geometry.boxAspects ?? null },
      rows: named,
      breaks: RABIES_BREAKS,
      plate,
      title,
...
if (import.meta.main) {
  ...
  console.log(
    `choropleth-web beat → ${outPath}  [${regions} regions]\n` +
      `claim: ${claim.readings} readings summing to ${claim.reported}, top ${claim.subject.key} (${claim.subject.value}), ` +
      `${claim.zeros} reported zero, ${claim.silent} filed nothing (${claim.comparison.key} among them)` +
      (plan
