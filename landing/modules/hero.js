...
void main(){
  ...
  outColor = vec4(col, 1.0);
    ...
    /* ------------------------------------------------------------- tuning
     * A live panel over the scene, off unless asked for: "?tune" in the URL,
     * or the T key. Values that only shape the render are written straight to
     * P; values the plates are seeded from reseed the field when they move.
     * "copy" puts the whole set on the clipboard as JSON, so a session of
     * fiddling can come back as numbers rather than as a description.
     */
    const TUNE = [
      [
        "l'intro",
        [
          ["IN_HOLD", 0, 5, 0.05, () => IN_HOLD, (v) => (IN_HOLD = v)],
          ["IN_DONE", 0.1, 8, 0.05, () => IN_DONE, (v) => (IN_DONE = v)],
        ],
      ],
      [
        "le couloir",
        [
          ["FLOW_LAPS", 0.2, 6, 0.1, () => FLOW_LAPS, (v) => (FLOW_LAPS = v)],
          ["FLOW_DIR", -1, 1, 2, () => FLOW_DIR, (v) => (FLOW_DIR = v)],
        ],
      ],
      [
        "la rupture",
        [
          ...
          ["FIRE", 0.05, 1, 0.01, () => FIRE, (v) => (FIRE = v)],
          ["SQUASH", 0, 1.5, 0.02, () => SQUASH, (v) => (SQUASH = v)],
          ["FLY", 0.4, 8, 0.1, () => FLY, (v) => (FLY = v)],
          ["RETURN", 0.3, 4, 0.1, () => RETURN, (v) => (RETURN = v)],
          ["HOME_TRIP", 0.2, 3, 0.05, () => HOME_TRIP, (v) => (HOME_TRIP = v)],
        ],
      ],
      [
        "la sortie blanche",
        [
          ["CARD_OUT", 0.5, 0.98, 0.01, () => CARD_OUT, (v) => (CARD_OUT = v)],
          ["TUMBLE", 0, 1.5, 0.05, () => TUMBLE, (v) => (TUMBLE = v)],
          ["TUMBLE_T", 0.6, 6, 0.1, () => TUMBLE_T, (v) => (TUMBLE_T = v)],
          ["BURN_AT", 0, 2, 0.05, () => BURN_AT, (v) => (BURN_AT = v)],
          ["BURN_D", 0.1, 2, 0.05, () => BURN_D, (v) => (BURN_D = v)],
          ["FLOOD_AT", 0, 3, 0.05, () => FLOOD_AT, (v) => (FLOOD_AT = v)],
          ["FLOOD_D", 0.1, 2, 0.05, () => FLOOD_D, (v) => (FLOOD_D = v)],
          ["EXIT_END", 0.5, 4, 0.05, () => EXIT_END, (v) => (EXIT_END = v)],
        ],
      ],
      [
        "dans le verre",
        [
          ["inside", 0, 2, 0.05],
          ["insideBend", 0, 0.4, 0.005],
          ["insideDisp", 0, 0.12, 0.002],
          ["insideRim", 0, 3, 0.05],
          ["insideAmp", 0, 1, 0.02],
          ["insideFreq", 0.2, 4, 0.05],
          ["insideSpeed", 0, 1, 0.02],
          ["insideBand", 0, 1.2, 0.02],
          ["insideSheen", 0, 3, 0.05],
        ],
      ],
      [
        "le champ (resème)",
        [
          ["spreadMin", 0.4, 5, 0.05, 0, 0, 1],
          ["spreadMax", 0.2, 7, 0.05, 0, 0, 1],
          ["plateMin", 0.06, 0.9, 0.01, 0, 0, 1],
          ["plateMax", 0, 0.8, 0.01, 0, 0, 1],
          ["depthZ", 0, 1, 0.02, 0, 0, 1],
        ],
      ],
      [
        "le papier",
        [
          ["sheetS", 0.15, 1.2, 0.01],
          ["bend", 0, 0.6, 0.005],
          ["curl", 0, 1.5, 0.01],
          ["paper", 0.2, 2.5, 0.02],
          ["ink", 0.2, 2.5, 0.02],
    ...
    function buildTuner() {
      ...
      box.id = "hero-tune";
      box.style.cssText =
        "position:fixed;top:12px;right:12px;z-index:99998;width:238px;max-height:calc(100vh - 24px);" +
        "overflow:auto;padding:10px 12px 12px;background:rgba(14,14,18,.88);color:#e8e8e8;" +
        "border:1px solid rgba(255,255,255,.14);border-radius:10px;" +
        "font:11px/1.4 ui-monospace,SFMono-Regular,Menlo,monospace;" +
        "-webkit-backdrop-filter:blur(10px);backdrop-filter:blur(10px)";
      const st = document.createElement("style");
      st.textContent =
        "#hero-tune .g{margin:11px 0 3px;font-size:9px;letter-spacing:.13em;text-transform:uppercase;opacity:.36}" +
        "#hero-tune .r{display:flex;align-items:center;gap:7px;margin:4px 0}" +
        "#hero-tune .r label{flex:0 0 74px;opacity:.62;overflow:hidden;text-overflow:ellipsis}" +
        "#hero-tune .r .v{flex:0 0 42px;text-align:right;opacity:.45;font-variant-numeric:tabular-nums}" +
        "#hero-tune input{flex:1;min-width:0;height:14px;-webkit-appearance:none;appearance:none;background:transparent}" +
        "#hero-tune input::-webkit-slider-runnable-track{height:2px;background:rgba(255,255,255,.2)}" +
        "#hero-tune input::-webkit-slider-thumb{-webkit-appearance:none;width:10px;height:10px;margin-top:-4px;" +
        "border-radius:50%;background:#f2b13c;cursor:pointer}" +
        "#hero-tune button{width:100%;margin-top:10px;padding:6px 0;font:inherit;color:#e8e8e8;cursor:pointer;" +
        "background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.16);border-radius:6px}" +
        "#hero-tune button:hover{background:#f2b13c;color:#14141c;border-color:#f2b13c}";
      document.head.appendChild(st);
      for (const [group, rows] of TUNE) {
        const g = document.createElement("div");
        g.className = "g";
        g.textContent = group;
        box.appendChild(g);
        for (const [key, min, max, step, get, set, reseeds] of rows) {
          const r = document.createElement("div");
          r.className = "r";
          r.innerHTML = "<label>" + key + "</label>";
          const inp = document.createElement("input");
          inp.type = "range";
          inp.min = min;
          inp.max = max;
          inp.step = step;
          inp.value = get ? get() : P[key];
          const val = document.createElement("span");
          val.className = "v";
          const show = () =>
            (val.textContent = (+inp.value).toFixed(step < 0.01 ? 4 : 2));
          inp.addEventListener("input", () => {
            const v = parseFloat(inp.value);
            if (set) set(v);
            else P[key] = v;
            if (reseeds) seedPlates();
            show();
          });
          show();
          r.appendChild(inp);
      ...
      btn.onclick = () => {
        ...
        for (const [, rows] of TUNE)
          for (const [k, , , , get] of rows) if (!get) out[k] = P[k];
        ...
        setTimeout(() => (btn.textContent = "copier les valeurs"), 1200);
      };
      box.appendChild(btn);
    ...
    return {
      ...
      init(api) {
         ...
         */
        {
          let panel = null;
          const toggle = () => {
            if (!panel) panel = buildTuner();
            else
              panel.style.display =
                panel.style.display === "none" ? "" : "none";
          };
          if (/[?&]tune\b/.test(location.search)) toggle();
