...
void main(){
  ...
  outColor = vec4(col, 1.0);
    ...
    /* ------------------------------------------------------------ réglages
     * Un panneau vivant au-dessus de la scène, éteint sauf demande : "?word"
     * dans l'URL, ou la touche W. Même langage que celui du hero, posé à
     * gauche pour que les deux puissent être ouverts ensemble. "copier" rend
     * le jeu complet en JSON : une séance de curseurs revient en chiffres. */
    const TUNE = [
      [
        "la lentille",
        [
      ...
      [
        ...
        [
          ...
          ["swirl", 0, 1.5, 0.01],
          ["flow", 0, 2.5, 0.01],
          ["grain", 0.001, 0.02, 0.0001],
          ["speed", 0, 3, 0.05],
        ],
      ],
      [
        "au repos, et la couleur",
        [
          ["amb", 0, 12, 0.1],
          ["split", 0, 3, 0.02],
        ],
    ...
    function buildTuner() {
      ...
      box.id = "word-tune";
      box.style.cssText =
        "position:fixed;top:12px;left:12px;z-index:99998;width:236px;" +
        "max-height:calc(100vh - 24px);overflow:auto;padding:10px 12px 12px;" +
        "background:rgba(14,14,18,.88);color:#e8e8e8;" +
        "border:1px solid rgba(255,255,255,.14);border-radius:10px;" +
        "font:11px/1.4 ui-monospace,SFMono-Regular,Menlo,monospace;" +
        "-webkit-backdrop-filter:blur(10px);backdrop-filter:blur(10px)";
      const st = document.createElement("style");
      st.textContent =
        "#word-tune .t{font-size:9px;letter-spacing:.13em;text-transform:uppercase;opacity:.5}" +
        "#word-tune .g{margin:11px 0 3px;font-size:9px;letter-spacing:.13em;text-transform:uppercase;opacity:.36}" +
        "#word-tune .r{display:flex;align-items:center;gap:7px;margin:4px 0}" +
        "#word-tune .r label{flex:0 0 46px;opacity:.62}" +
        "#word-tune .r .v{flex:0 0 48px;text-align:right;opacity:.45;font-variant-numeric:tabular-nums}" +
        "#word-tune input{flex:1;min-width:0;height:14px;-webkit-appearance:none;appearance:none;background:transparent}" +
        "#word-tune input::-webkit-slider-runnable-track{height:2px;background:rgba(255,255,255,.2)}" +
        "#word-tune input::-webkit-slider-thumb{-webkit-appearance:none;width:10px;height:10px;margin-top:-4px;" +
        "border-radius:50%;background:#f2b13c;cursor:pointer}" +
        "#word-tune button{width:100%;margin-top:10px;padding:6px 0;font:inherit;color:#e8e8e8;cursor:pointer;" +
        "background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.16);border-radius:6px}" +
        "#word-tune button:hover{background:#f2b13c;color:#14141c;border-color:#f2b13c}";
      document.head.appendChild(st);

      const head = document.createElement("div");
      head.className = "t";
      head.textContent = "le mot · déformation";
    ...
    return {
      ...
      init(a) {
        ...
        // W, et non T : le hero tient déjà T, et la page ses post-it
        let panel = null;
        const toggle = () => {
        ...
        if (/[?&]word\b/.test(location.search)) toggle();
