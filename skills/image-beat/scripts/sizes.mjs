// twin/skills/image-beat/scripts/sizes.mjs
//
...
// which is the reference this project has already accepted and rendered — and this skill produces
// the static genre only (SP1), so the article reading is the whole of its question. `map-beat`'s
// copy takes 30 for the opposite reason: that skill produces both genres from one table.
//
// WHY THIS FILE EXISTS AT ALL — round-five finding W3, closed in round six. This skill shipped no
// size table, so gate 2c's `size:` reached nothing here. `stories/stress-w-quay-photographs` says
// so in its own maintainer notes: "This beat had to vendor chart-beat/scripts/sizes.mjs into its
// own directory to learn that landscape is 1920x1080 with a 26px type floor." A story vendoring
// another format's table is the defect, not the workaround. The alternative considered and REFUSED
// was to withdraw the size from gate 2c for this medium, the way
// `storyboard/scripts/format-gate.mjs`'s `treatmentFormatGap` withdraws one treatment from one
// format. The tree refutes it: that same beat delivers 1920x1080, measured off the file's own bytes
// in `image-beat/test/size-table.test.ts`. What that beat's own portrait probe found is a different
// refusal and it already works: three letterboxed photographs do not fit the platform's safe band,
// and `assertWithinStage` below says so AT RENDER TIME, per beat. A beat that cannot fit the band
// is not a medium that cannot be drawn at portrait — one photograph fits it comfortably.
//
...
//
//     this seed's base tokens: TITLE 26 / CAPTION 15 / CREDIT 13  <- smallest is 13
//     portrait & square  36 / 13 = 2.77 -> 3.0 (the chart format's own number, kept: it clears the
//                        floor with room, and two formats disagreeing on a multiplier for no
//                        measured reason is drift wearing a justification)
//     landscape          26 / 13 = 2.0 -> 2.2 (the same, and for the same reason)
//
// The chart format's previous landscape value of 2.1 put its 12px gap note at 25.2 px, a fifth of a
// pixel under its own floor. It is 2.2 rather than waived, because a floor with an exception in it
// is a preference — and this skill's smallest token is 13, so the same multiplier clears it by
// 2.6 px rather than by nothing.
//
