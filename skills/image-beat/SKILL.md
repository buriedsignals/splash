...
**This format does not read the story's recorded typeface, and that is a gap, not a design.** `scripts/render-still.mjs:29` holds `export const FONT_FAMILY = "Helvetica, Arial, sans-serif"` as a **const**. This format has no `readTypeface`/`useTypeface` at all (measured 2026-08-21 — 0 calls), so the journalist's own caption, the alt text's visible twin and every credit line are set in a face nobody chose.
