...
**This is what makes the scroll vehicle worth having.** `scrolly` carries any beat's own
...
(`scrolly/SKILL.md`, "When to use"). A scrolly that moves from a photograph to a chart could
not have been anything else. This skill exists so that step is a real photograph the journalist
took, not a stand-in drawn in flat shapes — the exact gap `scrolly/scripts/build-sample-photo.mjs`
...
2. **The type sheet becomes a frame, never a choreography.** `choreographyFrame`
   ...
   id. It returns no rows, no cards and no shots: a static beat's choreography — the reading order through one composed frame of photographs — is
...
**Not yet vendored.** `chart-beat/scripts/render-still.mjs` is physically copied into every
fresh Splash root at `<root>/shared/chart-beat/` by `splash`'s own root template
...
bun skills/image-beat/scripts/render-preview.mjs --out /tmp/image-beat-preview
...
import { renderStill } from "./render-still.mjs"; // the carried copy every craft skill holds
import { readImageMeta, checkOrientation, checkWeight, toDataUri } from "./image-raster.mjs"; // this skill's own photograph side
...
| The combined raw-byte ceiling for one beat's embedded photographs | `20 MB` (`WEIGHT_LIMIT_BYTES`) | `image-raster.mjs` |
...
- `scripts/render-still.mjs`, `scripts/colour.mjs` — carried verbatim from `chart-beat` (line 1
  names the canonical; `splash/test/carried-copies.test.ts` holds them).
- `scripts/image-raster.mjs` — this skill's own photograph side: `readImageMeta`,
  `readOrientation`/`checkOrientation`, `fitBox`, `toDataUri`, `WEIGHT_LIMIT_BYTES`/`checkWeight`.
