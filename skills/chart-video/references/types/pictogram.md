...
- `no-scale-icon-carry` — scale an icon to carry a value — count is the encoding, so a bigger icon is a broken one
- `no-let-blocks-count` — let the blocks count at different paces: the counts are only comparable if the j-th icon of every block lands on the same frame
