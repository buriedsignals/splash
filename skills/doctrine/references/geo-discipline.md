...
(`map-beat`, the CO₂ choropleth) and say which defect produced them.
...
`extentFacts` (`map-beat/assets/geo.ts`), which the seed's bake now writes into every
...
(`map-beat/output-proof/extent-range/`, both pictures committed).
...
(`map-beat/output-proof/extent-range/RANGE.md`) — **nothing in the machinery is per-rung. What
