...
describe("reconcileBrandViolations — policy (b) brand-first bypass", () => {
  it("downgrades a CVD violation to a concern when the colour is brand-explicit", () => {
    const r = reconcileBrandViolations([CVD], ["#E30613"]);
    expect(r.violations).toEqual([]);
    expect(r.concerns).toHaveLength(1);
    expect(r.concerns[0]).toContain("#E30613");
    expect(r.concerns[0]).toContain("house style");
  });

  it("downgrades a contrast violation to a concern when the colour is brand-explicit", () => {
    const r = reconcileBrandViolations([CONTRAST], ["#F5A623"]);
    expect(r.violations).toEqual([]);
    expect(r.concerns).toHaveLength(1);
    expect(r.concerns[0]).toContain("#F5A623");
    expect(r.concerns[0]).toContain("2.03:1");
    expect(r.concerns[0]).toMatch(/WCAG|4\.5/);
  });
