...
import { GALLERY_API, searchInspiration } from "../scripts/search.mjs";

// A real search spends one of the day's anonymous searches for this address, so it runs only when
// asked for explicitly.
describe(`against the real gallery at ${GALLERY_API}`, () => {
  ...
  it.skipIf(!enabled)(
    ...
    async () => {
      const result = await searchInspiration({ query: "floods", apiBase: GALLERY_API });
      if (!result.ok) {
