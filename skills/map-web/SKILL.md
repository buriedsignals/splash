...
| Render | `scripts/render-web.mjs` | `renderMapWeb({ component, table, props, outDir, name, regionTable })` — SSRs the one fluid map render, plus the table only when the beat opted in, inlines the interaction script, writes one self-contained HTML file. `assertDistinctSlugs` refuses a filter vocabulary that cannot work. `repeatWorlds` is the wrap: a camera that spans the world fills its box by painting an odd number of `<use>` copies of its plate side by side, each with its own marks and hit targets, only the middle one in the accessibility tree. Also this skill's own seed runner (`ensurePlate`, `render`) behind a labelled CONFIG seam |
| Wrap proof | `scripts/verify-wraps-the-world.mjs` | `everyPaintedWorldAnswersAPointer` — opens the delivered page at four widths (including 2990×1718) and counts, per VISIBLE copy, how many marks answer a pointer sent to their own position, with the script off and again live through the canvas's own `queryRenderedFeatures`. Refuses a copy that answers for fewer marks than the world it is a copy of — the defect the wrap ruling was given with. See the discipline file, "A world camera wraps" |
