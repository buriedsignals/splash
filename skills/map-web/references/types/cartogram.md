...
- pile the cells on their true centroids: squares on true centroids overlap catastrophically, and a well-drawn pile is still a pile — keep each cell's area EXACTLY and move only the centres, by the smallest push that separates them
- treat this as a live map: there is no basemap and no geography to tile here, so there is no plan, no plate, no fallback and none of `live-map.mjs`'s radius behaviours — and the sheet says so rather than pretending
