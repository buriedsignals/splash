...
function countryLayers(d: LiveLocatorDeclaration): Record<string, unknown>[] {
...
export function liveLocatorPlan(d: LiveLocatorDeclaration): Record<string, unknown> {
  ...
  const layers: Record<string, unknown>[] = [
    ...countryLayers(d),
    // THE BORDERS USED TO BE DRAWN HERE, as a bare line over the provider's own land. They are now
    // `mw-countries-edge` above, over the beat's OWN land fill — the choropleth's mechanism, same
    // tileset, same level-0 filter, same generalisation per zoom (a frozen outline good enough for
    // 260 km is a smear at 5 000, and one good enough for 5 000 is a straight line through a river
    // at 260). A line with no land under it left the map a pale provider blob with a wire frame on
    // it; `countryGround` gives it the land the marks were already measured against.
