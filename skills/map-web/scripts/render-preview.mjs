...
try {
  ...
  if (process.argv.includes("--check")) {
    ...
    // No --out override: this IS the canonical regenerate, so the proof a reader opens is written
    // from the SAME screenshot buffer in the SAME run — never a second Chrome launch (not
    // byte-reproducible across launches, see compare-png.mjs's own header) and never a second
    // command (the step three regenerations in a row forgot: bc308ab8, 97293519, and the state this
    // branch found).
    let proofNote = "";
    if (outDirArg === -1) {
