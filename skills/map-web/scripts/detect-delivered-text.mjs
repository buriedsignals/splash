...
/** The credit line a rendered DRAFT prints, read off the page's own `.mw-source` run. A draft has no
 *  `HANDOVER.md` — that is written at delivery — so the credit that can still be corrected is the
 *  one drawn on the page. Returns null when the page carries no such run, which is itself an answer
 *  a caller reports rather than a silence. */
export function draftCredit(html) {
  const found = /class="mw-source"[^>]*>([\s\S]*?)<\//.exec(html);
