...
`;
  ...
  // THE LIMIT IS SAID OUT LOUD ON THE VERDICT A PRODUCER READS. A page whose plate cannot be given
  // the margin its own delivery range asks for is laid out the old way — the box keeps the plate's
  // aspect and sits centred — and the one thing that must never happen is that this reads as a
  // page which simply chose not to fill its container. `splash/test/...` and this format's own
  // `test/frame-fills-window.test.ts` assert the sentence, not just the layout.
