 ...
 *  only `map-beat/test/geo.test.ts` with label stacks built by hand. `placeLabels` beside it in
 ...
 *  the beat says it is drawing, and the journalist's own table. This skill's seed is a symbol map
 *  with neither, and giving it one means inventing a study set to ask about — a fixture, which is
 *  what `guard-wired-to-run` refuses. `real-owid-life-expectancy`'s own web runner calls it for
 *  real, on the 255 entity names the downloaded OWID file actually carries.
 *
