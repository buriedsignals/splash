...
- **Annotations are clamped inward deterministically.** You do NOT hand-nudge near-edge callouts any more. The mapper reads each point's fractional position in the plot and auto-picks `align` + inward `dx/dy` (right-edge → right-anchor + negative dx; near top/bottom similarly), adds a connector line, and offsets two annotations that land on the same point. A manual near-edge `align/dx` is augmented by the inward pull, never trusted to prevent a clip. `align`/`dx`/`dy` in the spec are honoured only as interior hints.
- **The label-safety guardrail cannot be bypassed silently.** `produceChart` loads the published chart headless (`src/label-safety.ts`), enumerates every text rect (series labels, annotations, axis ticks, title/subtitle) and THROWS if any rect extends beyond the content box (clipped) or intersects another (overlap). A clipping/overlapping chart cannot pass produce. `findLabelViolations` is a pure, deterministic geometry check (unit-tested).
