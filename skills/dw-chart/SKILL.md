...
- **Horizontal-bar value labels are printed directly ON the bars by default (FT best-practice #3), with the value axis kept on as a redundant reading path.** `d3-bars`/`d3-bars-grouped`/`d3-bars-split` set `show-value-labels:true` (ON unless `valueLabels:false`) + `value-label-format` from `numberFormat` — the value prints at the bar end (inside long bars, outside short ones), matching chart-native so the reader never has to estimate off the gridlines. The label **colour** is Datawrapper's own auto-pick, which it offers **no override for** (verified against the live `d3-bars` option schema — `GET /v3/visualizations/d3-bars`; the only value-label keys are `show-value-labels`/`value-label-format`/`value-label-alignment`/`value-label-visibility`; none control inside/outside or colour). That auto-pick is a **YIQ perceived-brightness threshold (~160), NOT WCAG luminance**: DW renders WHITE text on darker fills and DARK ink on lighter ones — verified across the Okabe-Ito palette (the discriminator: sky `#56B4E9` and amber `#E69F00` have near-identical relative luminance, 0.405 vs 0.416, yet DW picks white on sky and dark on amber; their YIQ, 158 vs 162, straddles the boundary). On a few mid-tone subject hues the white pick is below AA (green `#009E73` = 3.42:1, vermilion `#D55E00` = 3.87:1, pink `#CC79A7` = 3.06:1, sky `#56B4E9` = 2.31:1); light fills render safe dark ink (amber 7.9:1, grey 10:1, yellow 13:1). The mapper keeps the value axis on (`force-grid:true`) as a redundant reading path for the value, but — **unlike chart-native, which renders its own marks and recolours the label to max-contrast ink (`labelInkOnFill`) — dw-chart CANNOT recolour a DW label**, and a redundant axis does not cure the label text's own WCAG 1.4.3 failure. So `produceChart` runs `checkValueLabelContrast` before the first API call and treats a sub-AA white inside label as a **HARD failure (fail-loud)** on the auto path — caught before publish, never shipped silently as advisory (this was the `w8-krankenhaus-wartezeit-de` regression: a green `#009E73` highlight bar shipped a white "118" at 3.42:1 as a render-review concern). The guard models DW's YIQ auto-pick so it never false-flags a light fill's safe dark ink. The **only** keep-and-ship path is a journalist-chosen brand colour (policy b: `brandExplicit`), downgraded to a recorded concern. A chart that hits the hard failure resolves it by picking a fill DW renders a safe label on (Okabe-Ito blue/amber/grey/yellow), by opting the direct labels out (`valueLabels:false` → the value reads off the axis alone), or by declaring the colour brand-explicit. Stacked bars keep the DW default (their per-segment inside labels are a separate concern). See `src/value-label-safety.ts`.
