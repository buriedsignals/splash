...
- **Horizontal-bar value labels are printed directly ON the bars by default (FT best-practice #3), with the value axis kept as the accessible fallback.** `d3-bars`/`d3-bars-grouped`/`d3-bars-split` set `show-value-labels:true` (ON unless `valueLabels:false`) + `value-label-format` from `numberFormat` — the value prints at the bar end (inside long bars, outside short ones), matching chart-native so the reader never has to estimate off the gridlines. The label **colour** is Datawrapper's own auto-pick, which it offers **no override for** (verified live — the only value-label keys in the `d3-bars` schema are `show-value-labels`/`value-label-format`/`value-label-alignment`/`value-label-visibility`; none control inside/outside or colour). That auto-pick is a **YIQ perceived-brightness threshold (~160), NOT WCAG luminance**: DW renders WHITE text on darker fills and DARK ink on lighter ones — verified across the Okabe-Ito palette (the discriminator: sky `#56B4E9` and amber `#E69F00` have near-identical relative luminance, 0.405 vs 0.416, yet DW picks white on sky and dark on amber; their YIQ, 158 vs 162, straddles the boundary). On a few mid-tone subject hues the white pick is below AA (green `#009E73` = 3.42:1, vermilion `#D55E00` = 3.87:1, pink `#CC79A7` = 3.06:1, sky `#56B4E9` = 2.31:1); light fills render safe dark ink (amber 7.9:1, grey 10:1, yellow 13:1). So the mapper **also keeps the value axis on** (`force-grid:true`) as the contrast-clean redundant reading path for exactly that marginal case — the direct label reads for everyone else, the black-ink axis carries the value when the inside label is marginal. `produceChart` runs `checkValueLabelContrast` before the first API call: it models DW's YIQ auto-pick (so it never false-flags a light fill's safe dark ink) and records a marginal inside label as a **render-review concern** (kept, because the axis fallback is present) rather than throwing — a hard throw remains only when an unreadable inside label has NO axis fallback. `valueLabels:false` opts the direct labels out (cluttered case) and falls back to the axis alone. Stacked bars keep the DW default (their per-segment inside labels are a separate concern). See `src/value-label-safety.ts`.
