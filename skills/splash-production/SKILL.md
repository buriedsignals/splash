...
- **`resolvedPlaces`** (REQUIRED on a point map whose coordinates YOU resolved): one entry per
  ...
  "resolvedName", "categories", "elevationM", "shownToJournalist", "correctedFrom" }`. A lon/lat
  the machine geocoded was, until now, the one value in the chain nobody stood behind: the
  takeaway is confirmed verbatim, the source is compared against what the article named — and a
  coordinate was shown to no one and recorded nowhere. That is how `glaciers-requiem-2026` plotted
  "Cervin" on the Matterhorn GLACIER's centroid, 1063 m from the summit, under a beat naming the
  4478 m summit, **after the journalist had said it was wrong before production ran**. Show the
  journalist WHAT each coordinate resolved to (name, kind, elevation) and set
  `shownToJournalist`; when they correct one, put the corrected coordinate on **the marker
