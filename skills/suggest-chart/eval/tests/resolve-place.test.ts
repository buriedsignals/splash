...
// ── THE MERGE THAT NEARLY SHIPPED A WIRED-BUT-UNREACHABLE CAPABILITY ────────────────────────
//
// This script's `EXPECTED_KINDS` was hand-written as `["peak"]`. Three more kinds — lake, glacier,
// settlement — landed in lib/geo/geocode.ts the same day, on a different branch. Merging the two
// produced a tree where the module resolved four kinds and the only script allowed to call it
// refused three: the exact "a capability the engine computes and the delivery throws away" defect
// this repo names by hand elsewhere. The list is DERIVED now, and this is what keeps it derived.
import { EXPECTED_KINDS } from "../../scripts/resolve-place.mjs";
import { RESOLVABLE_PLACE_KINDS } from "../../../../lib/geo/geocode";

describe("the kinds this script accepts are the geocoder's own", () => {
  it("accepts every kind the geocoder can actually resolve, and no others", () => {
    expect([...EXPECTED_KINDS].sort()).toEqual([...RESOLVABLE_PLACE_KINDS].sort());
  });

  // A single-kind list is what the drift looked like, so the shape itself is asserted: a future
  // hand-edit back to ["peak"] fails here even if someone also edits the geocoder to match.
  it("names more than one kind", () => {
    expect(EXPECTED_KINDS.length).toBeGreaterThan(1);
    expect(EXPECTED_KINDS).toContain("peak");
    expect(EXPECTED_KINDS).toContain("lake");
  });
});
