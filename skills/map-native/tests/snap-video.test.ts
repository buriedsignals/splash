...
// SAME bundled ffmpeg the snap uses (scripts/lib/ffbin.mjs). MIRROR of chart-native's
// tests/snap-video.test.ts. Deliberate choice for map-native: a full map video render
// (Remotion+MapLibre+MapTiler network) is slow/flaky under gate contention, so this
// suite exercises the snap SCRIPT on synthetic mp4s only — fast, offline,
// deterministic — while the real-render path is covered by
// tests/produce-single-format.test.ts's video case, which now passes THROUGH this
// snap fail-hard inside produce.mjs. Adversarial cases: a deliberately WRONG still
// (an early frame passed off as the review still), a frozen video, a duration
// mismatch.
