...
- `scripts/measure-camera.mjs` — **diagnostic, not a gate**: reads the ZOOM of every beat off a rendered story mp4, solved from the pixel distance between marks of known lon/lat. Reach for it whenever a camera rule changes (`src/core/tour-box.ts`) instead of re-deriving numbers by hand. `--hue-histogram` when it finds 0 marks; `--fixture=` to re-cut the committed ground truth. Arithmetic, limits and unit tests in `lib/core/camera-measure.ts`; ground truth in `lib/core/camera-measure.fixture.json`.
