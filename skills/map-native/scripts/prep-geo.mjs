...
const flow = turf
  .simplify(turf.lineString(routed.slice(0, end)), { tolerance: RIVER_SIMPLIFY_TOL, highQuality: true })
...
const insideCountry = (pt, name) => polys[name].features.some((f) => turf.booleanPointInPolygon(pt, f));
const stops = {};
COUNTRIES.forEach((c, i) => { stops[c] = i === 0 ? 0 : 1; }); // 0 = headwaters; 1 = sentinel until entered
let acc = 0;
for (let i = 0; i < flow.length; i++) {
  if (i > 0) acc += havKm(flow[i - 1], flow[i]);
  const frac = acc / (flowKm || 1);
  const pt = turf.point(flow[i]);
  for (const c of COUNTRIES) if (stops[c] === 1 && insideCountry(pt, c)) stops[c] = frac;
}
...
const biggestPoly = (geom) => {
  ...
  for (const c of rings) { const p = turf.polygon(c); const a = turf.area(p); if (a > bestA) { bestA = a; best = p; } }
  return best;
};
...
const poleOfInaccessibility = (poly) => {
  ...
  for (let i = 0; i <= N; i++) for (let j = 0; j <= N; j++) {
    const lng = bb[0] + ((bb[2] - bb[0]) * i) / N, lat = bb[1] + ((bb[3] - bb[1]) * j) / N;
...
console.log("river:", flow.length, "pts ·", flowKm.toFixed(0), "km · entry stops", JSON.stringify(stops));
for (const n of COUNTRIES) {
  const km = countryMeta[n].border.reduce((s, seg) => s + turf.length(turf.lineString(seg)), 0);
  console.log(`  ${n}: anchor ${countryMeta[n].anchor.map((v) => v.toFixed(2)).join(",")} · border ${km.toFixed(0)} km`);
}
