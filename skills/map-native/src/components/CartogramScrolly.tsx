// CartogramScrolly — scrolly-as-video cartogram composition.
// Ports HexGridScrolly's step timeline (title → reveal x N), pinned ScrollyPanels, and
// per-step dim-emphasis; renders the cell-build + legend treatment of CartogramStory (cells tagged
// __id, __color, __value; sequential/diverging bin legend). Two variants: scaled (keeps basemap)
// and grid (neutral flat canvas via applyCartogramBasemap).
// Per-step dim-emphasis: on a reveal step the cells whose __id isn't the highlighted key dim to
...
const FULL_OPACITY = 0.85; // full opacity cap — mirrors CartogramStory
...
export const CartogramScrolly: React.FC<{ config: CartogramConfigShape }> = ({
  config,
}) => {
  ...
  const [mapState, setMapState] = useState<CGScrollyState | null>(null);
  const [legendState, setLegendState] = useState<CGLegend | null>(null);
  const [handle] = useState(() =>
    delayRender("cartogram-scrolly-init", { timeoutInMilliseconds: 120000 }),
  );
  ...
  useEffect(() => {
    ...
    const h = delayRender(`cartogram-scrolly-frame-${frame}`);
