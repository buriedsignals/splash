 ...
 * together by the eye; the thing that would actually hurt if they drifted — a reader unable to operate
 * the control — is held by neither, but by `verify-web.mjs` driving a real keyboard.
 *
...
${scope} .chart-fold legend { float: left; font-weight: 600; padding: 0; color: var(--ink); }
${scope} .chart-fold .options { display: inline-flex; flex-wrap: wrap; gap: 4px 12px; align-items: center; }
${scope} .chart-fold label { position: relative; display: inline-flex; align-items: center; gap: 4px; cursor: pointer; color: var(--muted); }
${scope} .chart-fold input { cursor: pointer; margin: 0; }
...
${scope} .fold-notes p { margin: 0; }

@supports selector(:has(*)) {
  ${scope} .chart-fold .options {
    gap: 0;
    padding: 2px;
    border: 1px solid var(--grid);
    border-radius: 999px;
  }
  ${scope} .chart-fold label {
    gap: 0;
    padding: 5px 10px;
    border-radius: 999px;
    line-height: 1.2;
  ...
  ${scope} .chart-fold label:hover { color: var(--ink); }
  /* ink-on-ground, never the accent: the accent is what the argument is drawn in on this page, and a
     ...
     here". */
  ${scope} .chart-fold label:has(input:checked) { background: var(--ink); color: var(--ground); }
  ${scope} .chart-fold label:has(input:focus-visible) { outline: 2px solid var(--ink); outline-offset: 2px; }
}
`.trim();
}
