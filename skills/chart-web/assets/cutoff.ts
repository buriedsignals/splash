 ...
 * `stack.ts` for `.chart-stack` and `level.ts` for `.chart-level`, and the cost is stated rather
 * than hidden: each is emitted only for a beat that declared ITS OWN control, so reaching one from
 * another would mean shipping a control the beat does not want.
 *
...
${scope} .chart-cutoff legend { float: left; font-weight: 600; padding: 0; color: var(--ink); }
${scope} .chart-cutoff .options { display: inline-flex; flex-wrap: wrap; gap: 4px 12px; align-items: center; }
${scope} .chart-cutoff label { position: relative; display: inline-flex; align-items: center; gap: 4px; cursor: pointer; color: var(--muted); }
${scope} .chart-cutoff input { cursor: pointer; margin: 0; }

/* THE SENTENCE THE CONTROL OWES THE READER. Its row is reserved whether or not a line is chosen, so
...
${scope} .cutoff-notes p { margin: 0; }

@supports selector(:has(*)) {
  ${scope} .chart-cutoff .options {
    gap: 0;
    padding: 2px;
    border: 1px solid var(--grid);
    border-radius: 999px;
  }
  ${scope} .chart-cutoff label {
    gap: 0;
    padding: 5px 10px;
    border-radius: 999px;
    line-height: 1.2;
  ...
  ${scope} .chart-cutoff label input {
    position: absolute;
    inset: 0;
    width: 100%;
    height: 100%;
  ...
  ${scope} .chart-cutoff label:hover { color: var(--ink); }
  /* ink-on-ground, never the ramp's ink: on any plate that needs a cutoff, colour is already
     carrying the whole quantity, and a control that borrowed a step of the ramp would make a colour
     that means "20 °C" also mean "you clicked here". */
  ${scope} .chart-cutoff label:has(input:checked) { background: var(--ink); color: var(--ground); }
  ${scope} .chart-cutoff label:has(input:focus-visible) { outline: 2px solid var(--ink); outline-offset: 2px; }
}
`.trim();
}
