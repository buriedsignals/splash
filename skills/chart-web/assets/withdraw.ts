 ...
 *
 * A DELIBERATE COPY of `stackChromeCss`, not an import, and the cost is stated rather than hidden —
 * the same trade that file records against `render-web.mjs`'s `.chart-filter` block. Reaching one of
 * those would mean either shipping a control this beat does not want or teaching another file's
 * stylesheet a class it cannot see the declaration for. The three blocks are held together by the
 * eye; the thing that would actually hurt if they drifted — a reader unable to operate the control —
 * is held by neither, but by `verify-web.mjs` driving a real keyboard.
 *
...
${scope} .chart-withdraw legend { float: left; font-weight: 600; padding: 0; color: var(--ink); }
${scope} .chart-withdraw .options { display: inline-flex; flex-wrap: wrap; gap: 4px 12px; align-items: center; }
${scope} .chart-withdraw label { position: relative; display: inline-flex; align-items: center; gap: 4px; cursor: pointer; color: var(--muted); }
${scope} .chart-withdraw input { cursor: pointer; margin: 0; }
...
${scope} .withdraw-notes p { margin: 0; }

@supports selector(:has(*)) {
  ${scope} .chart-withdraw .options {
    gap: 0;
    padding: 2px;
    border: 1px solid var(--grid);
    border-radius: 999px;
  }
  ${scope} .chart-withdraw label {
    gap: 0;
    padding: 5px 10px;
    border-radius: 999px;
    line-height: 1.2;
  ...
  ${scope} .chart-withdraw label input {
    position: absolute;
    inset: 0;
    width: 100%;
    height: 100%;
  ...
  ${scope} .chart-withdraw label:hover { color: var(--ink); }
  /* ink-on-ground, never the accent: the accent is what the argument is drawn in on this page, and a
     ...
     here". */
  ${scope} .chart-withdraw label:has(input:checked) { background: var(--ink); color: var(--ground); }
  ${scope} .chart-withdraw label:has(input:focus-visible) { outline: 2px solid var(--ink); outline-offset: 2px; }
}
`.trim();
}
