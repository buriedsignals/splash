...
/**
 * The control's own chrome, emitted ONLY for a beat that declared a ladder.
 *
 * A DELIBERATE COPY of the segmented treatment `render-web.mjs` gives `.chart-filter`, and the cost
 * is stated rather than hidden, exactly as `floor.ts`, `stack.ts` and `weigh.ts` state it: the
 * format's own block is emitted only for a beat that declared a FILTER, which this beat has not and
 * must not — nothing leaves its picture in any rung. The two blocks are held together by the eye;
 * the thing that would actually hurt if they drifted, a reader unable to operate the control, is
 * held by `verify-web.mjs` driving a real keyboard.
 *
...
${scope} .chart-qualify legend { float: left; font-weight: 600; padding: 0; color: var(--ink); }
/* THE GROUP'S FRAME HUGS ITS PILLS AND STILL WRAPS: flex 0 1 auto, which is the owner's own
   arbitration as he stated it — may not GROW (that is the 1240 px of border around 490 px of
   content he refused), may SHRINK. weigh.ts copied it as 0 0 auto, and a no-shrink row of four
   pills is a row that cannot wrap: measured here at 610 px of pills in a 375 px window, reported by
   the format's own fit check as "document 634px in a 375px window". */
${scope} .chart-qualify .options { display: inline-flex; flex: 0 1 auto; flex-wrap: wrap; gap: 4px 12px; align-items: center; }
${scope} .chart-qualify label { position: relative; display: inline-flex; align-items: center; gap: 4px; cursor: pointer; color: var(--muted); }
${scope} .chart-qualify input { cursor: pointer; margin: 0; }

/* THE SENTENCE THE CONTROL OWES THE READER. Its row is reserved whether or not a rung is chosen, so
...
${scope} .qualify-notes p { margin: 0; }

@supports selector(:has(*)) {
  ${scope} .chart-qualify .options {
    gap: 0;
    padding: 2px;
    border: 1px solid var(--grid);
    border-radius: 999px;
  }
  ${scope} .chart-qualify label {
    gap: 0;
    padding: 5px 12px;
    border-radius: 999px;
    line-height: 1.2;
  ...
  ${scope} .chart-qualify label input {
    position: absolute;
    inset: 0;
    width: 100%;
    height: 100%;
  ...
  ${scope} .chart-qualify label:hover { color: var(--ink); }
  ${scope} .chart-qualify label:has(input:checked) { background: var(--ink); color: var(--ground); }
  ${scope} .chart-qualify label:has(input:focus-visible) { outline: 2px solid var(--ink); outline-offset: 2px; }
}
`.trim();
}
