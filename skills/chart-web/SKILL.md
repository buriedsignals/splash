...
**So run `scripts/verify-web.mjs`, and know exactly what it can and cannot tell you.** Its last
section, `NOT ASKED`, is the answer to that second half, and it is not prose: the population is
derived from this skill's own `GUARDS` arrays, so every declared decision either has a measurement
printed above it or is named there with the argument for why one delivered page cannot answer it.
Before this it drove two of eighteen and printed nothing about the other sixteen — sixty-three green
checks on a real story that had never been asked whether a keyboard reaches a single mark. It
dispatches
...
| Render | `scripts/render-web.mjs` | Exports the format's generic `renderWeb({ component, props, outDir, name })` — SSRs the one component once, derives furniture/measures the y-axis gutter in node (this skill's OWN `scripts/render-still.mjs` copy — a skill never imports another skill), inlines the interaction script, writes one self-contained HTML file. It never imports a story's own numbers, and never a story's component; the caller hands both in. `accessibleTable`, also here, reads every mark's own `data-detail` back off the SSR'd markup and appends an opt-in, visually-hidden-until-focused `<table>` — one row per mark — so no story ever authors this by hand |
| Accessibility | `scripts/detect-accessible-table.mjs` | `tableCarriesTheMarks(html) → { rows, marks, missing }` — the capability this format carries (`same-facts-without-the-picture`, `doctrine/references/guard-catalogue.json`): every `data-detail` value against the accessible table's own cells, over the DELIVERED page, so a table that drifted from the picture fails as loudly as no table at all. Run in `verify-web.mjs`'s own CARGO check, on the artifact `render` just wrote |
...
| Verify | `scripts/verify-web.mjs` | The format's evidence, not its documentation: drives Chrome over a rendered beat — `checkFit` (the window fit at seven `VIEWPORTS`), `checkHover` (real `page.mouse.move` over marks discovered by `[data-detail]`, at each of `POINTER_VIEWPORTS`), `checkFilter` (real `page.mouse.click` on every option, with scripting on and with JavaScript disabled), `checkControlAffordance` (Tab reach, focus ring measured in pixels, checked-pill contrast). It also runs the format's own declared decisions against the file it was given: `CARGO` and `BEAT RECORD` before the browser, `CAPABILITIES` inside it, and `NOT ASKED` at the end. Every check is conditional on the beat's own shape and every skip is announced; `probe` rounds each coordinate. Exit 0 only when every check passed |
| Coverage | `scripts/verify-coverage.mjs` | `decisionsNotAsked(skillDir, asked) → [{ name, reason }]` — every decision this skill DECLARES that a run did not ask, derived from the `GUARDS` arrays it ships rather than typed beside them. `NOT_ABOUT_ONE_PAGE` records the argument for each one a single delivered page cannot answer; a name with no argument is a FAILURE on the run, not a line of prose |
| Population | `scripts/delivered-pages.mjs` | `deliveredPages(root) → string[]` — every delivered page this format has produced, from EVERY root a beat lives in (`stories/` as well as `proof/`), derived through `beatsCalling` rather than typed. The four capability walks in `test/` used to start at `proof/` and go no further, so six beats under `stories/` had never been put to any of them; the first run of the widened walk found a delivered page with no accessible table at all |
| Painted labels | `scripts/painted-labels.mjs` | `inlineSvgOf(html)`, `readPaintedGeometry(page)`, `paintedLabelSvg(painted)` — this format draws no SVG `<text>` and writes no `.svg` file, so two shared decisions (`mislabelledRows`, `rtlRunsAreIsolated`) were reading an empty page. Neither decision is changed; they are handed what this page actually paints, measured in the browser that lays it out |
