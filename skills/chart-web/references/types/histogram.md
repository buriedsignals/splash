...
- let the reader change the bin width here just because it is the type's own trap — the refusal is measured and written down, and a control whose every option returns the same reading at a different resolution changes the picture and not the reading
- carry the bin width as a bare literal: the three measurements that chose it are asserted in the render script
