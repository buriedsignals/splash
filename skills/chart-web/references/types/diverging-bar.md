...
- `no-draw-diverging-bar` — draw a diverging bar whose domain does not genuinely straddle the chosen zero — under every offered reference the page states who is above, on and below
- `no-lay-new-reference` — lay the new reference across the plot as a rule: a datum is SUBTRACTED from the data, so the reference is always at zero by construction and what moves is every mark
