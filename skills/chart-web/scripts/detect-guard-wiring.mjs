 ...
 *
 *  The web chart's own debt, and it is now one name. It was thirteen: `verify-web.mjs` — the one
 ...
 *  given rather than against a fixture. `framingMeasurement` is what remains, and
 *  `scripts/verify-coverage.mjs` records the argument by name, printed on every run so an
 *  unreachable decision stays visible instead of going quiet.
 */
export const RECORDED_UNWIRED_DEBT = [
  "framingMeasurement",
];

/** CATEGORY 2 — DECISIONS ONLY A BEAT CAN CALL. Empty here: nothing this format declares needs
 *  material its own seed cannot hold. An entry is `{ name, calledBy }`, `calledBy` a repository-
 *  relative path to a committed beat whose runner really calls the decision, and
 *  `beatSubstrateWithoutACaller` goes and reads it — a name in this array with no real caller
 *  anywhere is worse than debt, because it has stopped being counted as debt. */
