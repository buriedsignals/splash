...
async function realParent(target) {
  ...
  if (canonical !== parent || !info.isDirectory() || info.isSymbolicLink()) {
    throw new Error(
      "lock target parent must be a real directory without symlinks",
    );
  }
  return parent;
}
...
function parseOwner(bytes) {
  ...
  if (
    ...
    JSON.stringify(["createdAt", "host", "pid", "schemaVersion", "token"])
  )
    return null;
  if (
    owner.schemaVersion !== "splash-target-lock/v1" ||
  ...
  if (
    ...
    return null;
  if (!Number.isSafeInteger(owner.createdAt) || owner.createdAt <= 0)
    return null;
  return owner;
}
...
async function reclaimAbandoned(lockPath, { localHost, kill }) {
  ...
  if (
    !owner ||
    owner.host !== localHost ||
    !processIsProvablyGone(owner.pid, kill)
  )
    return false;
...
/**
 * Skill-local copy of Splash's portable adjacent lock. It reclaims only an unchanged same-host
 * owner whose PID the operating system proves absent; every ambiguous state fails closed.
 */
    ...
    try {
      ...
      if (now() - started >= timeoutMs) {
        const conflict = new Error(
          "target is locked by another live or unverifiable writer",
        );
