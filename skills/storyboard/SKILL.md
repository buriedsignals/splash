...
- `scripts/format-gate.mjs` — `formatPublicationFormatGate`, the complete G2b assistant turn, and
  `treatmentFormatGap` / `TREATMENT_FORMAT_GAPS`, the treatment-level half of the reachability
  answer that `formatGap` and `confirmFormatReachable` cannot see from a medium and a format alone.
