...
| Claim grounding | `scripts/ground-claim.mjs` | `groundTakeaway(takeaway, profile, { csv })` checks the confirmed takeaway's own numbers, comparisons and superlatives against the frozen data profile — and against the frozen `source/data.csv` itself, which is where ROW-level facts come from, since no `profile.json` in this tree carries rows. A number equal to a column's `sum` (a part-to-whole total) is `supported`, **read together with the comparator that governs it** ("more than 100" against a column summing to exactly 100 is `contradicted`, not confirmed); the frozen `rowCount`, a column's `missing` count and a column's `distinct` count are numerals' homes too — and where the sentence names no column at all, those counts are still tried, giving `consistent` when exactly one of them equals the numeral and a refusal naming all of them when several do; a number merely inside the range of the column **its own sentence names** is **`consistent`** — placed, not confirmed, and the detail says when it is exactly that column's min or max or is held verbatim in a row; a number it can place neither way is `unverifiable`, naming the column it was put to and the range it missed, never `contradicted`. A stated multiplier ("1.12 million") is read as a second reading beside the numeral as written, and a comma-grouped numeral the token reader calls ambiguous is settled by the frozen table when exactly one of its two readings is a number that table holds. A SPACE-grouped numeral — "59 000", how WHO, Eurostat, the ECDC and the EEA all print — is one numeral, never two claims: settled by its own shape where the shape can only be a grouping (more than one group, or a later group carrying a leading zero), then by the frozen table holding the grouped reading, and otherwise left as ONE ambiguous claim. `coverage.namedNoColumn` names the claims refused only because the sentence names no column of the table, so `resolveGrounding` can say that limit — the join is by column NAME — once rather than repeating it per claim. Not a fact-checker, not a conformance engine, one narrow class of error |
...
| What settles a SPACE-grouped numeral ("59 000", the SI/ISO 80000-1 grouping WHO, Eurostat, the ECDC and the EEA publish in) | its own shape first — more than one group, or a later group carrying a leading zero, can only be a grouping — then the frozen table holding the grouped reading. Unlike a comma, its two readings are one numeral against TWO side by side, and a real register holds every fragment of a grouped numeral (WHO's own table holds 59, 0, 3 and 21), so the table can only ever testify FOR the grouping. Nothing else settles it, and what is unsettled stays ONE claim | `scripts/ground-claim.mjs`'s `settleGroupedNumeral` |
