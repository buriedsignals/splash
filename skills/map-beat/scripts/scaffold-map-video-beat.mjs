// skills/map-beat/scripts/scaffold-map-video-beat.mjs
//
// THE PLUMBING OF A DIRECTED LIVE-MAPTILER MAP VIDEO, WRITTEN ONCE — AND NOTHING ELSE.
//
...
// plumbing copied from the worked example; this writes exactly those files, from `assets/video-beat-scaffold/`, with
// the beat's names in them. The subject-agnostic placement they re-wrote each time lives in
// `scripts/video-placement.mjs`, which the generated `build.mjs` and `map-plan.mjs` import.
//
// WHAT IT GENERATES — the files identical across map videos, modulo names:
//   index.ts, Root.tsx                                 the Remotion entry and root
//   Directed<Name>Video.tsx                            the live-map composition: `useLiveMap` with `mountPlan` and
//                                                      `transformStyle` injected, `mapStateAt` + bindings painted per frame
//   render-directions-video.mjs                        the runner on `renderVideoMap`, `buildProps(origin)` →
//                                                      `mapPlanProxied` via `throughProxy`: ONE composed direction by
//                                                      default (the design base's best candidate for NEWSROOM.md),
//                                                      `--candidates <N>` to choose, `--filed` for the demo directions
//                                                      only, `--only`, `--still`, `--look`; type floor at every event's end
//   measure.mjs                                        `measureLiveMap` per direction, merged into measured.json, a plan
//                                                      digest, refusing a camera measured with `tilesLoaded` false
//   map-plan.mjs                                       the dataviz style URL and the Countries source with the key
//                                                      PLACEHOLDER, `camera.view` fitted "meet" (`fitBoundsMeet`), an
//                                                      empty layers section
//   timing-contract.ts                                 a legal six-event ~20 s skeleton (45-frame title, 60-frame hold)
//   states.mjs, scene.mjs                              `assertEventStates` wiring; `fieldAt`/WINDOWS/LINEAR, `cameraAt`
//   build.mjs                                          direction → registers → `videoRegistersOf`, the map registers,
//                                                      `titleCardFor`, `sourceCreditFor(...CREDIT_ONE_LINE)` seated on the
...
//   the argument and the copy, the gestures (states' fields, WINDOWS, `mapStateAt`'s fields and camera travel), the
//   layout (build.mjs, THE LAYOUT: the key, the names, the leaders), the map's layers, bounds, other cameras and seats,
//   the scene maths (`sceneAt`), the story drawn (<Name>Frame.tsx, THE STORY), the claim's assertions (`assertClaim`),
//   and the gesture tests. Where a test must be written, the generated test FAILS until it is (its name carries
//   SCAFFOLD), and the runner refuses to render a beat whose copy or claim is still a placeholder.
//
...
const FILES = Object.freeze({
  ...
  "measure.mjs.tmpl": "measure.mjs",
  "map-plan.mjs.tmpl": "map-plan.mjs",
...
/** `video-choropleth-coal-share` of type `choropleth` → `CoalShare`. */
...
if (import.meta.main) {
  try {
    ...
    const written = scaffoldBeat({ root: workingRoot(process.cwd(), DEFAULT_ROOT), templates: TEMPLATES, files: FILES, skill: SKILL, medium: "map", type: args.type, beat: args.beat, staticBeat: args.static, component: args.component, filed: Boolean(args.filed), size: args.size ?? DEFAULT_SIZE });
    console.log(
