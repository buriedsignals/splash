...
/** `## The choreography`, empty: the event table's headers, the frame quoted, and no rows. */
export function renderChoreographySection(frame) {
  return [
    "## The choreography",
    "",
    frameComment(frame, [
      `     A video is the UNFOLDING IN TIME: the ladder ${EVENT_ORDER.join(" → ")}, each shot's`,
      "     gesture, and what it asserts. `start` and `duration` are not written here — they live in",
      "     this beat's own timing-contract.ts and are joined in from it.",
    ]),
    "",
