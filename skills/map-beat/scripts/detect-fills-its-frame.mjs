 ...
 *  skill's own scripts — the population `exampleRunnersFor` derives (a committed runner naming
 *  `skills/map-beat/scripts/` or `#shared/map-beat/`), restricted to the frames a beat delivers
 *  rather than the baked plates beside them. Four readings, which is the whole of it:
 *    proof/mapvid-hexgrid-quakes,  final frame, 1080x1080:  75.42%
 *    proof/mapvid-locator-geneva,  final frame, 1080x1350:  77.87%
 *    proof/mapvid-dot-population,  final frame, 1080x1440:  77.94%
 *    this skill's own preview,                   900x560:   82.90%
 *  A map beat sits HIGH in this measurement rather than low, and for a reason worth writing down:
 *  its plate is a square raster with furniture in a column beside it, so the drawing's own box
 *  reaches both margins by construction. The narrow population is stated rather than padded — most
 *  committed map beats reach the shared `chart-beat` renderer and are measured in that skill's own
 *  population, not this one. `MARGIN_FRACTION` is wider than chart-beat's because four readings
 *  from one camera shape is a thinner base than fifty. */
export const MEASURED_MIN_FRACTION = 0.7542;
