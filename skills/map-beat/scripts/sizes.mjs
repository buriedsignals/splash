...
//
// THE ROW THAT DIFFERS FROM `chart-beat`, and why THIS skill takes the video's number for it.
// A static landscape is read in a ~900 px article column and clears 12 CSS px at 26; a landscape
// VIDEO is watched on the phone turned sideways, ~800 dp, and needs 30. Every other craft skill
// produces ONE genre and answers this once. This one produces BOTH — `render-map.mjs`'s ladder is
// a still, then a video's last frame, then the mp4 — so one row has to hold for two reading
// distances, and it takes the stricter of the two. 30 is never wrong for the article reading (it
// is 14 CSS px there rather than 12); 26 would be wrong for the video one, and a floor that is
// right for one genre of the two is a preference.
//
...
//
// WHY THIS FILE EXISTS AT ALL — round-five finding T3, closed in round six. This skill shipped no
// size table, so gate 2c's `size:` reached nothing here: `stories/stress-t-europe-recycling` pinned
// `portrait`, and the only way its beat could learn what portrait means was to reach for
// `#shared/chart-video/sizes.mjs` — a table belonging to a format it is not. The alternative
// considered and REFUSED was to withdraw the size from the gate for this medium, the way
// `storyboard/scripts/format-gate.mjs`'s `treatmentFormatGap` withdraws one treatment from one
// format. The tree refutes it: that same beat delivers 1080x1920, measured off the file's own bytes
// in `map-beat/test/size-table.test.ts`. A gate row saying "a map cannot be drawn at portrait"
// would have been false against a beat already on disk.
//
