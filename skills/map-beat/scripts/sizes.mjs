// twin/skills/map-beat/scripts/sizes.mjs
//
// WHY THIS SKILL HAD NO COPY UNTIL NOW, AND WHAT THAT COST. Gate 2c asks a video beat for its
// export size and `checkStoryboard` refuses three ways on the answer; `where.mjs` mirrors those
// refusals string for string. All of that careful machinery validated the answer against a
// vocabulary the map producer never read: the slot recorded `size: landscape`, a 16:9 plate was
// baked to match, and `Root.tsx` rendered 1080x1080 anyway — a square mp4 from a landscape gate
// answer, with the plate letterboxed into it and the camera work thrown away. Nothing warned.
//
// `size-table-parity.test.ts` could not see it, and says so in its own header: it WALKS for files
// named `sizes.mjs`, so a skill with no copy at all is indistinguishable from a skill the guard has
// nothing to check. That walker now refuses an absent table for any skill `FORMAT_CATALOG` says can
// produce a sized format, so this file's absence would redden rather than pass.
//
// WHICH NUMBERS THESE ARE. Carried from `chart-video`, not from `chart-beat`, because the format
// this table was added FOR is the video: a landscape video is watched, and the mode 16:9 is
// designed for is the phone turned sideways (~800 dp), which is the 30px floor rather than the
// article column's 26.
//
// NAMED AND NOT ANSWERED, because a number nobody has looked at should not pretend otherwise: this
// skill ships a STATIC map as well as a video, from one table, and a static map read in an article
// column is the `chart-beat` reading distance. One `minTypePx` per row cannot be right for both.
// `assertTypeFloor` is therefore carried and NOT wired into this skill's static path today — the
// composition size is what this file was added to deliver, and it delivers exactly that. Wiring the
// floor means first deciding which distance a map beat's static is read at, which is a question for
// somebody who has opened one.
//
...
//
// THE ROW THAT DIFFERS FROM `chart-beat`, and why. A static landscape is read in a ~900 px
