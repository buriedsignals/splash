 ...
 *      `image-beat`'s says it carries the raster machinery a photograph needs; a `proof/`
 *      beat's says a beat never imports out of a skill. Those paragraphs are the opposite of drift.
 *   2. Copies legitimately differ in their SET of functions. `image-beat` adds six
 ...
 *      `map-web`, `scrolly` and `image-beat` gained them when their own seed runners
 ...
 * scan compared function bodies with whitespace collapsed, and reported `scrolly`'s `mix` and
...
// red guard nobody would keep. Measured on `scrolly`'s copies of `mix` and `measureText`,
