...
// threaded to the NATIVE producers only (chart-native, map-native) as SPLASH_CHANNEL
...
describe("channelEnvFor — the argv/env builder for channel threading", () => {
  it("threads SPLASH_CHANNEL for chart-native with an explicit channel", () => {
    expect(channelEnvFor("chart-native", "social-vertical")).toEqual({
      SPLASH_CHANNEL: "social-vertical",
    });
  });

  it("threads SPLASH_CHANNEL for map-native with an explicit channel", () => {
...
// process reading process.env.SPLASH_CHANNEL, mirroring how chart-native/map-native's
...
describe("runProducerScript — forwards the env param to the spawned process", () => {
  function setupEnvHarness() {
    const dir = mkdtempSync(join(tmpdir(), "splash-adapters-env-harness-"));
    const fakeProducer = join(dir, "fake-producer-env.mjs");
    writeFileSync(
      fakeProducer,
      [
        'console.log("channel seen: " + (process.env.SPLASH_CHANNEL ?? "none"));',
      ].join("\n"),
    );
  ...
  it("a producer script observes the passed SPLASH_CHANNEL", () => {
    ...
    const out = execFileSync(
      ...
      [
        ...
        dir,
        JSON.stringify({ SPLASH_CHANNEL: "social-vertical" }),
      ],
      { encoding: "utf8" },
    );
  ...
  it("an empty env is a no-op — the spawned process sees no SPLASH_CHANNEL", () => {
    const { fakeProducer, harness, dir } = setupEnvHarness();
    writeFileSync(
      fakeProducer,
      [
...
describe("runProducerScript — never lets a producer's stdout reach the parent's real stdout", () => {
  ...
  function setupHarness() {
    const dir = mkdtempSync(join(tmpdir(), "splash-adapters-harness-"));
...
describe("realDispatch — map-dw single-format gate (video/scrolly fail hard before any API call)", () => {
  ...
  for (const format of BAD_FORMATS) {
    it(`fails a map-dw proposal pinned to "${format}" with the map-dw message`, async () => {
      const dir = mkdtempSync(join(tmpdir(), "splash-mapdw-gate-"));
...
describe("freshOutDir — wipes a re-produce's outDir clean before the next dispatch writes into it", () => {
  it("removes a superseded attempt's stray artifacts, leaving only what the next dispatch writes", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "splash-freshoutdir-"));
    writeFileSync(
  ...
  it("does not disturb a same-format re-produce's own outputs (no unrelated stray files survive it either)", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "splash-freshoutdir-retry-"));
    writeFileSync(
      join(dir, "static.png"),
  ...
  it("is safe on a directory that doesn't exist yet (first-ever produce for this id)", () => {
    const parent = mkdtempSync(join(tmpdir(), "splash-freshoutdir-first-"));
