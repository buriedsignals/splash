...
formats reported it independently across two rounds (U, V, W, Y, AC and AD), which makes it the
most-reported defect in this project's history, and every one of them wrote the same sentence:
`whereIs` answered `production, missing: []` on a story that could not close. Each producer worked
around it by writing the file at delivery, from memory of a survey that had already happened —
the lives-in-a-conversation-and-dies-with-it failure the file exists to prevent, happening around
the file itself — and each reported it separately because nothing recorded that the others had.

**Gate 2 closes into TWO files.** `STORYBOARD.md` is what will be drawn; `SUBJECTS.md` is what was
found and not drawn. `surveyGap` is the one decision, carried byte-identically by
`splash/scripts/where.mjs` and `storyboard/scripts/storyboard.mjs` and walked by
`test/guard-copies-parity.test.ts` — because `checkStoryboard` is pure over the front matter and
structurally cannot read a directory, which is exactly the shape that let one gate close while the
other could not see the question at all. `whereIs` reports it as `G2-subjects`.
...
| Layer                | File                                                       | Role                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| -------------------- | ---------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Phase recovery       | `scripts/where.mjs`                                        | `whereIs(storyDir)` — the state machine; the sole source of truth for "what phase is this story in". `missingForGate2` applies the real Gate 2 condition (takeaway, all six hand fields, the recorded `grounding`, `reference` and `language` scalars, every slot's medium/format/size/`reachable`/`chosen`, and the conditional `producer`/`datawrapperType` decision) before ever reporting `production`. It reports missing treatment selection as `G2-treatment` and a missing eligible provider choice as `G2-producer`. Past Gate 2 it walks the beats: `beatsAwaitingApproval` first — **nothing about `export/` may shorten the walk past it** — then the durable editor-feedback trigger (a valid review must bind the current `FEEDBACK.md` and render digests; delivery must bind that feedback/review/render tuple), then `beatsAwaitingBoundReview` (the same `OUTPUT-REVIEW.json` requirement `deliver` enforces, minus the two values only a caller holds), then `beatsAwaitingDelivery`, per beat, into `export/<beat>/`, then `beatsWithStaleDelivery` — `done` asserts that the bytes in `export/` are the bytes that were approved, so the delivery manifest's `draftDigest` is read against the bound review's and a beat whose delivery no longer holds is reported as `revision: {reason: "stale-delivery"}`; a hand-over with no manifest beside it is the same answer for the smaller reason that nothing records what those bytes were built from — and finally its carried copy of `deliveryClosed` — both closing-offer receipts, per beat. Between the front-matter gate and the beats it runs `surveyGap`, gate 2's SECOND file (`SUBJECTS.md`), reported as `G2-subjects`. `REQUIRED_SCALARS` and `REQUIRED_SLOT_FIELDS` are exported for the parity test to generate fixtures from |
