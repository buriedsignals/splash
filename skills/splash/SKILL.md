...
- The component/producer for a visual type is a REGISTRY LOOKUP (`register-producers` / the producer registry), never a `src/` inspection — do not grep an engine's `src/` to discover or author a spec's shape. This is the reach the orchestrator actually has (`lib/core/registry.ts` + `register-producers.ts`, held by `import-guard.test.ts`); grepping producer source for that purpose has no legitimate reason to exist.
