...
// mirrored from dw-chart (skills/dw-chart/src/furniture-i18n.ts; both DW producers
// share the invariant, duplicated per-skill the same deliberate way SOURCE_LABELS is).
//
// The render is Datawrapper-hosted, so the mechanical seam is the METADATA: for a
// non-English deliverable, specToMapMetadata routes the source through annotate.notes
...
//
// Deliberately does NOT reuse specToMapMetadata's own sourceNotes/
// usesNativeSourceCaption helpers: a gate that recomputes with the very code it
// checks would inherit the regression it exists to catch. Only the LABEL TABLE
// (SOURCE_LABELS — pure data, the exact bytes the furniture must carry) is shared;
// the invariant logic is independent.
//
...
import { SOURCE_LABELS, type MapPatch } from "./spec-to-map-metadata";

/** The slice of the spec the gate needs (every MapSpec variant fits). */
  ...
  if (name !== "") {
    violations.push(
      `describe.source-name is "${name}" but must be BLANK for lang "${spec.lang}" — ` +
        ...
        `map must carry the source via annotate.notes only (else the footer ships a ` +
