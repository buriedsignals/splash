 ...
 *
 * THE DELIVERED KEY IS `MAPTILER_KEY` — the one key Splash asks a journalist for (the separate
 * origin-restricted "delivery key" was retired on 2026-09-10 to keep setup simple; anyone who wants
 * origin restrictions applies them to this key in MapTiler Cloud, allowing `?` for previews from
 * their own computer). Ruling R1 governs: *"la carte doit rester interactive tout le temps… On a le
 * droit d'utiliser pleinement MapTiler. Et garder l'export du HTML pas grave pour la clé."* The owner
 * was shown the cost — the key ships inside the delivered HTML, readable by anyone who opens the
 * article — and accepted it explicitly. R1b adds where the key may not go (the repository).
 *
 * So this function never blocks: the key is substituted, and `mapKeyState` names what went in, so
 * the hand-over can say plainly what the newsroom is shipping and what it costs them. **There is no
 * refusal in this path** — a rule that stops a journalist delivering is a rule they will route
 * around.
 *
 * The three states, and there is no fourth:
 *
 *   - no key slot in the file → `"none"`. Not a map delivery; nothing is substituted or said.
 *   - `MAPTILER_KEY` set      → `"live"`. Substituted, and SAID OUT LOUD: the delivery is live and
 *                               carries the newsroom's key. Ruling R1's own trade.
 *   - not set                 → `"unkeyed"`. The placeholder travels through untouched. Not a silent
 *                               failure: the delivered page renders its complete fallback layer,
 *                               exactly as it does offline, and the hand-over says so.
 */
 ...
 */
export const LIVE_TILE_STATES = ["none", "live", "unkeyed"];

export function mapKeyState(html, env = process.env) {
  if (!carriesMapKey(html)) return "none";
  if (env.MAPTILER_KEY) return "live";
  return "unkeyed";
}

// A delivery can write more than one file, so the hand-over states the state that COSTS THE MOST.
// A key in any delivered file is the thing the newsroom needs told, whatever the file beside it
// carries.
const STATE_COST = { none: 0, unkeyed: 1, live: 2 };
