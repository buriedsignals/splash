// THE CREDENTIAL SWEEP FOR THE PHASE THAT HANDS WORK TO A NEWSROOM.
//
// `credential-alias-reconciled` could not reach this skill until the catalogue's population was
// widened on 2026-08-23 — `reachable()` iterated the eight skills that DRAW, and this one ships.
// Asked for the first time, it found three files reading a provider credential by its canonical
// name with no alias list anywhere in the skill, and one of them was live: with the root's own
// `.env` holding the MapTiler key under the engine's names, every map delivered from this checkout
// shipped its placeholder and a dead tile layer.
//
// The decisions below are the catalogue's own, copied byte-identically from
...
// `check-credentials.mjs` beside this file is the command that runs them over this skill's source.
