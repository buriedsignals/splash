...
| Hand-over | `scripts/format-handover.mjs` — `formatHandover` | `export/<outputId>/HANDOVER.md`, **G4** — not an option: `materialise` refuses a delivery with no payload to read back. each delivered file with its role, the placement read back, the alt text, the credit line, the caveat. A CLOSED parameter set — there is no free-text field, and adding one is what this file exists to prevent — and it **throws** on any string naming one of our own paths or modules, so a maintainer-facing sentence cannot reach the journalist. A defect in this toolchain goes to `stories/<slug>/NOTES-FOR-MAINTAINER.md`. It **throws** a second time on an HTML entity (`&amp;`, `&#39;`, `&#x27;`) in any of those four fields: a journalist never typed one, so an entity is the mark of text lifted back out of a rendered artefact rather than read out of the record that holds it — the refusal names `beats/<outputId>/ALT.md` and the storyboard's hand fields instead of decoding it |
| The language it is written in | `scripts/journalist-language.mjs` — `resolveScaffoldLanguage`, `untranslatedNotice` | Ruling R4: the journalist's language follows the ARTICLE and is confirmed with them, so it is READ from `STORYBOARD.md`'s `language:` field — never detected from the prose, never defaulted. **Both Gate-2 readings now require that field** (round-four finding 9), so a story that reaches this skill has already answered the question; this module still owns what the answer MEANS. Every journalist-facing document this skill writes (the hand-over, both halves of the closing offer) is rendered from a copy table keyed by language. `en` and `fr` are written today; any other recorded language gets the English scaffold **and a line at the top of the document saying so** |
