...
- **INTERACTIVE or SCROLLY (a self-contained `interactive.html` / `scrolly.html`, article-web only):**
  atelier **PROPOSES three delivery forms and the journalist CHOOSES one — and ONLY the chosen form is built
  ...
  2. **THEN relay the emitted proposal VERBATIM and ASK which form the journalist wants (a / b / c) — an
     ...
     **★ WAIT means WAIT: after emitting the proposal, `--form` MUST NOT run until a journalist message
     ...
     NAMED VIOLATION — observed on a real two-element hosted-DW run, where atelier emitted both proposals,
     ...
     gate: the journalist confirms the single form; atelier never confirms in their place.
     **Multi-element delivery: the form choice is PER ELEMENT (forms may differ).** Either ask element by
     ...
     JOURNALIST's to give, never atelier's to presume: applying one element's answer (or no answer at all)
