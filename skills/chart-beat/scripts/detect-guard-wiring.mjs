 ...
 *
 *  The static chart's own debt. `revealDashInScreenSpace` lives in `verify-static.mjs`, which this
 *  skill ships as a LIBRARY with no command behind it; the rest are the tree-wide detectors
 *  distributed to all eight in rounds four and five and wired into none of them.
 */
