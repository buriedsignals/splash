...
export function verifyBeatFiles(file) {
  ...
  for (const name of readdirSync(beatDir)) {
    ...
    // a beat under `proof/` with none is NOTED rather than failed, the same contract the ground has.
    const palette = paletteAboveBeat(beatDir);
    ...
    if (ground == null || ink.length === 0)
      notes.push(`${where}: ${name}/ no ground or no ink recorded above this beat, so its plate's own fills have nothing to be measured against`);
    else {
      const surfaces = surfacesUnderMarks({ image, geometry, surfaces: plateSurfaces(image), ink });
      const offences = plateSurfacesYieldToInk({ ground, ink, surfaces });
      for (const offence of offences) failures.push(`${where}: ${name}/ ${offence}`);
      if (!offences.length)
        notes.push(
          `${where}: ${name}/ ${surfaces.length} fill(s) — ${surfaces.map((one) => `${one.hex} ${(one.share * 100).toFixed(1)}%`).join(", ")} — all yield to this beat's ink`,
        );
    }
  }
