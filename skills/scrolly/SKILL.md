...
2. **The type sheet becomes a frame, never a choreography.** `choreographyFrame`
   ...
   id. It returns no rows, no cards and no shots: a scrolly beat's choreography — the card-to-card travel: what changes between card n and card n+1, and by which gesture — is
