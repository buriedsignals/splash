...
| Seed | `assets/ScrollySeed.tsx` | `STEPS_META` (the beat's four-step arc: id, `frameKind`, prose-as-a-function), the four frame components (`ImageFrame`, `DrawnGraphicFrame`, `MapFrame` — drawn under the same `cartographic-rules`, `doctrine/references/geo-discipline.md`, a map-beat plate is baked against — and `ChartFrame` — composed under the same `static-discipline`, `chart-beat/references/static-discipline.md`, any format drawing its own geometry is held to), and the placement constants: `ASPECT_ENVELOPE`, `safeBand`, `SAFE_AREA`, `FRAME`, `CHART_LAYOUT` |
...
**This format does not read the story's recorded typeface, and that is a gap, not a design.** `scripts/render-still.mjs:25` holds `export const FONT_FAMILY = "Helvetica, Arial, sans-serif"` as a **const**, and `scripts/render-scrolly.mjs:263` writes the same stack into the delivered page's CSS — so the prose cards a reader actually reads are set in it. This format has no `readTypeface`/`useTypeface` at all (measured 2026-08-21 — 0 calls).
