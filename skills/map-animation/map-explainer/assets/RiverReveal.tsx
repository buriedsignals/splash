...
import flowCoords from "../geo/yarlung-flow.json";
import countryMeta from "../geo/country-meta.json";
...
const META = countryMeta as Record<Country, { stop: number; anchor: [number, number]; border: [number, number][][] }>;
...
const DRAW = Object.fromEntries(
  ORDER.map((c) => {
    ...
    const cum: number[] = [];
    let acc = 0;
    for (const L of segLen) { cum.push(acc); acc += L; }
    return [c, { segLines, segLen, cum, total: acc }];
  }),
) as Record<Country, { segLines: any[]; segLen: number[]; cum: number[]; total: number }>;

// Reveal the portion of the border between fromKm and toKm as a MultiLineString (no joins across gaps).
const sliceBorder = (d: (typeof DRAW)[Country], fromKm: number, toKm: number) => {
  const out: number[][][] = [];
  ...
  return { type: "Feature" as const, properties: {}, geometry: { type: "MultiLineString" as const, coordinates: out } };
};
const EMPTY = { type: "Feature" as const, properties: {}, geometry: { type: "MultiLineString" as const, coordinates: [] as number[][][] } };
...
const FILL_S = 1.0;
const LABEL_S = 0.7;
const trigger = (c: Country) => RIVER_START + META[c].stop * (RIVER_END - RIVER_START);
...
export const RiverReveal: React.FC = () => {
  ...
  useEffect(() => {
    ...
    m.on("load", () => {
      ...
      m.addSource("countries", { type: "geojson", data: staticFile("geo/borders.geojson") });
      ...
      for (const c of ORDER) {
        ...
        m.addLayer({
          id: `trail-${c}`, type: "line", source: `trail-${c}`,
          layout: { "line-cap": "round", "line-join": "round" },
          paint: { "line-color": COUNTRY_DARK[c], "line-width": 2, "line-opacity": 0.95 },
        });
      }

      const seed = turf.lineSliceAlong(line, 0, Math.max(0.001, lineKm * 0.001));
      ...
      m.addLayer({ id: "river-glow", type: "line", source: "river", layout: { "line-cap": "round", "line-join": "round" }, paint: { "line-color": "#49C6FF", "line-width": 11, "line-opacity": 0.32, "line-blur": 6 } });
      m.addLayer({ id: "river-line", type: "line", source: "river", layout: { "line-cap": "round", "line-join": "round" }, paint: { "line-color": COLORS.river, "line-width": 3 } });
  ...
  useEffect(() => {
    ...
    const tt = interpolate(frame, [0, durationInFrames - 1], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
    ...
    (map.getSource("river") as any)?.setData(turf.lineSliceAlong(line, 0, Math.max(0.001, riverDrawnKm)));
    ...
    (map.getSource("river-head") as any)?.setData(turf.lineSliceAlong(line, Math.max(0, riverDrawnKm - riverHeadKm), Math.max(0.001, riverDrawnKm)));
    let riverHeadFade = 0;
    if (reveal > 0.002 && reveal < 0.999) riverHeadFade = 1;
    else if (reveal >= 0.999) riverHeadFade = 1 - clamp01((t - RIVER_END) / 0.5);
    ...
    for (const c of ORDER) {
      ...
      (map.getSource(`trail-${c}`) as any)?.setData(bp <= 0 ? EMPTY : sliceBorder(d, 0, d.total * bp));
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: COLORS.bg }}>
      ...
      <AbsoluteFill style={{ pointerEvents: "none" }}>
        {ORDER.map((c) =>
          labels[c] ? (
            <CountryLabel key={c} name={c.toUpperCase()} color={COUNTRY[c]} reveal={labels[c].reveal} x={labels[c].x} y={labels[c].y} />
          ) : null,
        )}
      </AbsoluteFill>
    </AbsoluteFill>
