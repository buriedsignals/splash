...
### 5.4bis Two defects found while planning, amended in under the §6 ruling

*Correctness governs over the plan. Both were discovered mapping the attachment points on
2026-08-08, after §1–§8 were written.*

**A — story files import by absolute machine path.** `proof/EmissionsLine.tsx:30` and
`proof/RankBars.tsx:21` both import
`"/Users/rmdms/Sites/Professional/splash-twin/twin/skills/twin-chart-beat/scripts/render-still.mjs"`.
Hardcoded to one laptop: these files break on any other machine and on any relocated worktree. They
cannot be moved without deciding how they import, so the fix lands with the move.

The fix is the mechanism the architecture already specifies rather than a relative path. A Splash
root declares `"imports": {"#shared/*": "./shared/*"}` in its `package.json` and a beat writes
`import { renderStill } from "#shared/twin-chart-beat/render-still.mjs"` — quoted verbatim in
`twin-chart-beat/SKILL.md`'s Quick start. This repository has no live `shared/` and no imports map,
which is *why* the proof stories reached for an absolute path.

So: create `twin/shared/` and add the imports map to `twin/package.json`, making `proof/` behave
like the Splash root it stands in for. Story files then use the same specifier a real beat writes,
which has a second benefit — the proof stories start exercising the real import mechanism instead
of a repository-only shortcut that no journalist ever runs.

**B — no script generates `preview.png`.** `twin-chart-beat/assets/preview.png` was produced by
hand; no script in any craft skill emits it. §6's "regenerated by the skill's script and
byte-compared" therefore names a mechanism that does not exist. Each craft skill gains
`scripts/render-preview.mjs`, which renders that skill's seed from that skill's `sample-data` and
writes `assets/preview.png` — making the preview a *derived* artifact, so a seed that changes and a
preview that does not become a loud test failure rather than a stale image nobody re-renders.
