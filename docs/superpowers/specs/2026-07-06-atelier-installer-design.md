...
Let a non-technical journalist install and configure Atelier on their Mac with **zero
...
Atelier reuses the **procedure**, not Mycroft's page: Atelier ships its **own standalone install
page**, because Atelier is a Claude Code plugin (not a Goose pack) and we want full control.
...
- Porting the Atelier skill to non-Claude runtime formats (esp. Goose). Those are separate,
...
Data flow: journalist fills form → generator reads registry + form → emits `atelier-setup.command`
...
export const RUNTIMES = {
  claude: {
    label: "Claude Code",
    verified: true,                      // Atelier is built + tested on this
...
| **Generate** | Produces and downloads `atelier-setup.command`. |
...
1. **Preamble** — friendly `echo` progress; install Homebrew if missing; install **Bun**
   (Atelier producers run in Bun).
2. **Runtime install** — `brew install <formula>` for the chosen runtime.
3. **Clone** — clone the Atelier repo → `~/Atelier`.
...
7. **Final message** — how to launch (e.g. `cd ~/Atelier && claude --plugin-dir .`, then
   `/atelier` or a natural-language brief).
...
The installer must never offer a non-technical journalist a runtime where Atelier does not
actually load. Each non-Claude runtime has a gate before its card flips to `verified: true`:

- **Claude Code** — verified now (Atelier's native format).
- **Codex** — verify the Atelier skill/plugin loads (low risk; viznews already claims Codex).
- **Gemini CLI** — verify the skill loads (low risk; viznews already claims Gemini).
- **Goose** — **port** the Atelier skill to Goose's extension format first (heaviest; separate task).
...
- **Manual smoke:** run the Claude Code `.command` on a clean macOS user account → Atelier
...
- Repo clone target: public HTTPS clone URL of the Atelier GitHub repo (must be public by the
