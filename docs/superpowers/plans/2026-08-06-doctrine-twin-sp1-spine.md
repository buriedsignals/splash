...
describe("inspectSvg", () => {
  ...
  it("should mark every contrast entry unresolved when a <style> block is present anywhere", () => {
    ...
    const result = inspectSvg(
      svg("<style>text { fill: #AAAAAA }</style>" + '<text fill="#000000">x</text>'),
      { ground: "#FFFFFF" },
    );
    expect(result.contrast.length).toBe(1);
    expect(result.contrast[0].unresolved).toBe(true);
  ...
  it("should read a font-weight declared only in style as a keyword", () => {
    const result = inspectSvg(
      svg('<text font-size="20" style="font-weight:bold" fill="#949494">x</text>'),
      { ground: "#FFFFFF" },
    );
...
**AMENDED A THIRD TIME 2026-08-06 (coordinator review — three fresh Critical holes in the
resolution surface the second round itself introduced, plus the approach called out as the
recurring problem).** 12 more tests were added on top of the 26 above: two for a `<style>` block
anywhere marking every entry unresolved (including one whose local fill is `none`), one for a
`style` attribute that declares the same property twice (last must win), two for a font-size
declared only via `style` (one confirming it's read at all, one confirming an unparseable style
size does NOT inherit a large ancestor's allowance), two for the equivalent font-weight cases, and
five (`it.each`) explicitly pinning forms that were already unresolved before this round —
`rgb()` with percentages/space-syntax/slash-alpha, `rgba()`, `hsl()` — because the report had
twice gone silent about exactly this. Every test confirmed to fail when the line it targets is
broken, then to pass again once reverted — see the Step 3 note below.
...
**AMENDED A THIRD TIME 2026-08-06 (coordinator review — the resolution surface round 2 built to
close its five Criticals had three fresh Critical holes of its own, all pixel-verified). The
review's real point was about the APPROACH, not just these three: two rounds had each closed real
holes and each opened fresh ones of the same shape, because the tool kept trying to resolve MORE
of the CSS cascade from a regex-driven walker — a bottomless task where every attempt to resolve
more leans the reassuring way. This round inverts the default instead of chasing the next case.**

**The tool is now conservative BY CONSTRUCTION.** It enumerates the small set of forms it fully
understands, and everything else — including anything it merely SUSPECTS it cannot see — is
`unresolved`:

- a `fill` presentation attribute
- a single `fill` in a `style` attribute (style wins the cascade over the attribute; the LAST
  declaration wins when one `style` attribute declares the same property twice)
- inheritance of fill/font-size/font-weight through `<g>`/`<text>`/`<tspan>` ancestors
- the named colours in `NAMED_COLOURS`, 6- and 3-digit hex, and classic `rgb(r,g,b)`
...
Three fresh Criticals, each pixel-verified the same way as round 2 — construct the SVG, render it
with the project's own rasteriser, sample the actually-painted pixel:

1. **Critical — a `<style>` CSS block was invisible.** `<style>text { fill: #AAAAAA }</style>` is a
   real, common SVG pattern the project's own rasteriser applies (real paint: `#AAAAAA`), but the
   walker never parsed `<style>` at all, so a bare `<text>` fell through to "never declared →
   black" and reported `#000000`, 21:1, a confident pass — precisely the worst case: silently
   missed AND reported as passing. Fixed: a `<style>` element ANYWHERE in the document (CSS rules
   apply document-wide by selector, not by the `<style>` tag's own tree position) now marks EVERY
   contrast entry `unresolved`, not just the ones it happens to target — including an entry whose
   local fill is `none`, because a rule could un-hide it, and skipping it would be exactly the
   silent miss this tool exists to prevent.
2. **Critical — a `style` attribute declaring `fill` twice picked the cascade LOSER.**
   `style="fill:#000000; fill:#AAAAAA"` really paints `#AAAAAA` (later wins); `readStyleProperty`
   used `.exec()` without `/g`, returning the first match. Fixed: `matchAll` with a global regex,
   taking the LAST match.
3. **Critical — `font-size`/`font-weight` declared only via `style=` were never read.**
   `ownFontSizePx`/`ownFontWeight` never called `readStyleProperty`, so a child that shrinks
   itself with `style="font-size:8px"` under a large-attribute ancestor kept the ancestor's
   large-text 3:1 floor — verified by measuring the real glyph height: 28px without the style
   override, 8px with it, both reported pass under the old code. Fixed with the same
   style-then-attribute precedence as fill, plus a sharper rule for uncertainty: a font-size/weight
   that IS declared (attribute or style) but cannot be confidently parsed (a unit outside px, an
   unrecognised weight keyword) now returns `NaN`, not `undefined`. `NaN` is an OWNED value — it
   overrides an ancestor's context instead of falling through to inherit it — and `NaN` compares
   false against every threshold in `isLargeText`, so an uncertain declaration is held to the
   stricter 4.5:1 floor rather than silently inheriting a large ancestor's easier 3:1 allowance.
   Returning plain `undefined` here (as `undefined` correctly means for a font-size never declared
   at all) would have let exactly this case — a real declaration this file merely can't read the
   magnitude of — quietly borrow a large ancestor's leniency, the same failure shape as the colour
   holes, just in the size axis.

`rgb()` with percentages/space-syntax/slash-alpha, `rgba()`, and `hsl()` already returned
`unresolved` before this round (the strict `RGB` regex only matches classic `rgb(r,g,b)`) — kept
as-is, and now explicitly tested (`it.each`, 5 cases) and named here, because the report went
silent about exactly this twice.
