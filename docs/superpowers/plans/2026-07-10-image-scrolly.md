...
**Goal:** Add IMAGE-SCROLLY as splash's 4th engine — journalist-supplied photo/satellite sequences that splash *formats* (crop/canvas-frame + compress) and renders as static + crossfade-video + scroll-driven story, captions derived from the article, never generated.
...
```js
const TSC_DIRS = ["skills/splash", "skills/chart-native", "skills/map-native", "skills/scrolly", "skills/image-native", "install"];
```
