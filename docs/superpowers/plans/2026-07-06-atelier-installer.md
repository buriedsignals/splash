# Atelier Installer Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a static, client-side install page that generates a config-baked `atelier-setup.command` bash script, so a non-technical journalist installs Atelier on macOS with one double-click.
...
- No secret ever leaves the browser or gets committed. The Atelier repo's real `.env` stays gitignored.
...
Run: `cd ~/Sites/Professional/atelier && bun test docs/installer/runtimes.test.ts`
...
- Produces:
  ...
  - `export function scriptFilename(): string` — returns `"atelier-setup.command"`.
...
Expected: four runtime radios (Codex/Gemini/Goose disabled + "coming soon"); the AI-key label reads "(Anthropic API key)"; submitting downloads `atelier-setup.command`; opening it shows the baked keys.
...
- Produces: a public install URL (`https://<org>.github.io/atelier/installer/`) and a documented manual smoke test that gates release.
...
Expected: `https://<org>.github.io/atelier/installer/` serves the page.
