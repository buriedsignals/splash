...
This adapter surfaces Splash to **Goose** (Block's open-source, model-agnostic CLI agent) via its
...
- **Layer B — partial (see the dedicated section below).** Goose activated the `splash` skill and
...
`splash` skill** (`# Loaded Skill: splash`), read its full flow, and **drove the orchestration** —
