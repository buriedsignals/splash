...
| §3.10 échoue **ouvert** sur un profil malformé | `skills/splash/src/brand-profile.ts:125-129` → `tryLoadDecor` → `neutralDecor()` | Un `requiredSigner` sans signer enregistré fait throw le parseur ; le décor neutre n'a pas de `requiredSigners`, donc **le gate de sign-off disparaît en silence** (et source/crédit/langue avec lui). Forme pré-existante, devenue porteuse depuis que le fix C2 route l'enforcement par le décor. À traiter avec le sous-projet Verify. |
