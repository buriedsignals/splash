...
One fix landed alongside this registry: `reviewer-not-attributed`'s catalogue entry used to name
a `--reviewer-output <findings.json>` flag that `skills/splash/scripts/review-gate.mjs` never
implemented — the script only parses `--reviewer <name>@<version>`. The command above is the
corrected one; running it as written now actually works.
