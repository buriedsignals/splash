...
| `attestation-uncorroborated` | the run's own record (`skillsInvoked`) says it invoked sub-skills — and the run directory holds NOT ONE of the files those skills leave behind (`suggest-article` → `opportunities.json`, `suggest-chart` → `candidates.json`), so nothing shows they ever ran | actually invoke the skills this run says it invoked — or drop the claim, and say plainly that the step did not run | no command — the act is a skill invocation inside the session (and, on a host with no skill-invocation facility, reading that skill's own `SKILL.md` and following it); neither is a process this catalogue could name. The writers that leave each artifact ARE named, per skill, inside the refusal's own message | `skills/splash/src/attestation-corroboration.ts`'s `attestationRefusal()`, via `routed()`. Wired in `skills/splash/scripts/produce-all.mjs`, right after the `no-candidates-menu` precondition and before any engine runs — so a refused run leaves no half-built artifact. Fires only on TOTAL absence (one artifact present ⇒ a warning naming the other, never a stop) |
| `place-resolution-undeclared` | a point map cannot say where a plotted coordinate came from: either the run's own resolution receipt (`places.json`, written by `skills/suggest-chart/scripts/resolve-place.mjs`) holds a lookup the accepted entry carries no `resolvedPlaces` record for; or a record copies neither that resolution nor a declared `correctedFrom` of it; or it claims a non-`geocoder` origin (`data`, `journalist`) for a coordinate the receipt shows the machine chose, which would waive the showback G3 demands; or the element accounts for NOT ONE of its places and does not declare `coordinatesFromData: true` | look each place up for real, show the journalist what came back — the name, what kind of thing it is, its elevation — and record what they agreed to, so a correction they give has somewhere to land | `bun skills/suggest-chart/scripts/resolve-place.mjs <runDir> --place "<name>" [--expect peak] [--elevation <m>]` — the command performs the lookup AND writes the receipt; carrying the result onto the accepted entry stays an edit, and the refusal's own message names the field | `skills/splash/src/place-provenance.ts`'s `placeProvenanceRefusal()`, via `routed()`. Wired in `skills/splash/scripts/produce-all.mjs` as ③, after the attestation check and before any engine runs — so a refused run leaves no half-built artifact. The partial gap (some places accounted, some not) is a warning, never a stop |
| `source-hint-undeclared` | the citation the ARTICLE gave for these figures did not survive the journey: either the run's own analysis receipt (`opportunities.json`, written by `skills/suggest-article/scripts/save-opportunities.mjs`) records an attribution for the passage this element is anchored to and the accepted entry carries no `sourceHint`; or the entry names an organisation the analysis never recorded; or it bolts a URL onto a name the article gave bare; or the analysis attributed EVERY claim it recorded and NOT ONE accepted element carries an attribution | credit what the article credited — carry the source it named onto the element, or say plainly that the article named nobody for these figures | `bun skills/suggest-article/scripts/save-opportunities.mjs <runDir> --payload '<the ProposalSet JSON>'` — the command re-records the analysis (it is the writer that keeps the citation on disk); putting the hint onto the accepted entry stays an edit, and the refusal's own message names the field. A source the JOURNALIST supplied rather than the article is `sourceAnswer`, a different field | `skills/splash/src/source-provenance.ts`'s `sourceProvenanceRefusal()`, via `routed()`. Wired in `skills/splash/scripts/produce-all.mjs` as ④, after the place-resolution check and before any engine runs — so a refused run leaves no half-built artifact. The partial gap (some elements carry an attribution, some not) is a warning, never a stop |
