...
| Geography credit guard | a map built from a DECLARED geometry file (spec D7) with no `geoCredit` rendered into the artefact, or a blank one — a shipped basemap (Natural Earth, US Census) carries no such obligation and is exempt. The credit itself DOES render when present — every `MapFrame` call site forwards `geoCredit={config.geoCredit}` beside `source`, render-proven. **The OBLIGATION this guard exists to enforce is not reachable from a real run today**, though: it only fires when `geography.origin === "declared"`, and no production code ever constructs a geography object with that origin — `matchGeography` (`skills/map-native/src/geo-match.ts`) only ever returns `origin: "shipped"`, so every config this loop assembles takes the guard's exempt branch | `lib/geo/policy.ts` (`assertGeoCreditPresent`), called from `lib/geo/resolve-for-produce.ts` (`resolveGeometryForProduce`), wired into `skills/map-native/scripts/produce.mjs` before any build step |
