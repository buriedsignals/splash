...
| Unresolved geo-join refusal | a below-ADM1 map join whose ambiguous values were measured into candidates but never resolved to a decision — an unresolved value must not reach a build any more than an unwritten narrative beat does, so it is refused BEFORE dispatch to any producer. **Not reachable from a real run today**: `run.orient.geoJoin.pending` is READ here (and at `manifest.ts:634`/`:767`) but WRITTEN by no code in the tree — `orient()`'s own `OrientResult` (`lib/loop/orient.ts`) carries no `geoJoin` field at all, so this refusal can only fire against a hand-built manifest, never one this loop actually produces | `lib/loop/produce.ts:186` (`unresolvedGeoJoins`, `lib/geo/join.ts`) |
...
| Geography credit guard | a map built from a DECLARED geometry file (spec D7) with no `geoCredit` rendered into the artefact, or a blank one — a shipped basemap (Natural Earth, US Census) carries no such obligation and is exempt. On the **map-native** track the credit DOES render when present — every `MapFrame` call site forwards `geoCredit={config.geoCredit}` beside `source`, render-proven. On the **scrolly** track this guard is wired the same way (same shared resolver) but renders NOTHING when it passes: `Scrolly.tsx` shows `config.source` only and has no `geoCredit`-aware furniture at all — a build there refuses on a missing credit but a present one is silently dropped, never reaching the reader. **Neither obligation is reachable from a real run today**: the guard only fires when `geography.origin === "declared"`, and no production code ever constructs a geography object with that origin — `matchGeography` (`skills/map-native/src/geo-match.ts`) only ever returns `origin: "shipped"`, so every config either engine assembles takes the guard's exempt branch. The scrolly rendering gap is pinned, not fixed, by `skills/scrolly/tests/geo-credit-reachability.test.ts` so the day it becomes reachable, something fails | `lib/geo/policy.ts` (`assertGeoCreditPresent`), called from `lib/geo/resolve-for-produce.ts` (`resolveGeometryForProduce`), wired into `skills/map-native/scripts/produce.mjs` before any build step, and into `skills/scrolly/scripts/produce.mjs` the same way |
...
| Missing-geometry guard | a choropleth/cartogram/dot-density/route render (native or scrolly) building with no `config.geometry` injected — a loud, named failure instead of a blank map or a bare TypeError deep in an async load handler | identical `"config.geometry is required"` throw in 8 components: `skills/map-native/src/ChoroplethMap.tsx`, `skills/map-native/src/CartogramMap.tsx`, `skills/map-native/src/DotDensityMap.tsx`, `skills/map-native/src/RouteMap.tsx`, `skills/scrolly/src/ScrollyMap.tsx`, `skills/scrolly/src/ScrollyDotDensityMap.tsx`, `skills/scrolly/src/ScrollyCartogramMap.tsx`, `skills/scrolly/src/Scrolly.tsx` |
...
| Embed fail-fast | choosing the embed form with no `CLOUDFLARE_API_TOKEN`/`CLOUDFLARE_ACCOUNT_ID`/`SPLASH_EMBED_PROJECT` — fails before any Cloudflare Pages call, instead of stalling and marking delivered | `lib/delivery/adapters/cloudflare-pages.ts` (`embedTokenConfigured`), called from `skills/splash/scripts/deploy-embed.mjs` |
| Placement declared | once a run has read an article, an element that declares neither an `anchor` nor `freeStanding: true` — the journalist would receive a finished visual with no idea where it goes, and an omitted placement is indistinguishable from a forgotten one. A bare-topic run, and a direct-branch run that read no article, are untouched | `skills/splash/src/placement.ts`, `skills/splash/scripts/export-code.mjs` |
