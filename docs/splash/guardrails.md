...
| Claim-grounding (GUARD 4) | a title/`confirmedTakeaway` citing a year outside the chart's time axis, or a value ABOVE the plotted/joined data maximum, with no annotation or reference line backing it — the "70% target by 2035 on data that tops out at 48% in 2023" defect | `validate-gate.ts` (`claimGroundingErrors`; domain read from CSV `data` for charts + map-dw, from `rows[valueField]` for map-native; locator/route/symbol configs are a strict no-op) |
