...
| Missing-geometry guard | a choropleth/cartogram/dot-density/route render (native or scrolly) building with no `config.geometry` injected — a loud, named failure instead of a blank map or a bare TypeError deep in an async load handler | identical `"config.geometry is required"` throw in 8 components: `skills/map-native/src/{Choropleth,Cartogram,DotDensity,Route}Map.tsx` + `skills/scrolly/src/{ScrollyMap,ScrollyDotDensityMap,ScrollyCartogramMap,Scrolly}.tsx` |
