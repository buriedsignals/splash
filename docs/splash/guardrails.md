...
| Preflight (per-engine readiness) | a missing API key or uninstalled skill deps announced BEFORE production in journalist language (tri-state green/yellow/red persisted to `.splash-preflight.json`), instead of a lazy crash mid-PRODUCTION | `skills/splash/src/preflight.ts` (manifest + gate in `produce-all.ts`), `skills/splash/scripts/preflight.mjs` (PROPOSITION-time report) |
...
| Contrast snaps | WCAG text contrast measured on the REAL render (static and interactive passes, tooltip included), brand-explicit fills accounted for | `skills/chart-native/scripts/snap-contrast.mjs`, `snap-interactive-contrast.mjs`, `snap-tooltip-contrast.mjs`; `skills/map-native/scripts/snap-theme.mjs`, `snap-a11y.mjs` — all run fail-hard from `produce.mjs` |
