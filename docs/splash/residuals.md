...
|---|---|---|---|---|
| A1 | Un crédit contenant `{name}` sans `source.name` **ship le littéral `{name}`** dans `metadata.json` du paquet livré. Rien en aval ne l'attrape : `metadata.ts` ne fait que `trim()`, et la garde d'accolades de `snippet.ts` s'applique au gabarit, jamais aux valeurs substituées. | `lib/newsroom/decor.ts:133-136` | une ligne | La rédaction cesse de publier un gabarit non rempli. C'est un défaut visible du lecteur pour un `??` manquant. |
| A2 | **`install/preflight/client.ts:49` propose « Deutsch »** dans son sélecteur de langue alors que `install/preflight/copy.ts:203` ne connaît que `en`/`fr` et retombe sur l'anglais. Offrir un choix qu'on n'honore pas est pire que ne pas l'offrir. | `install/preflight/client.ts:49` · `install/preflight/copy.ts:203` | une ligne (retirer l'option) | Ferme le seul endroit du parcours d'install qui promet quelque chose de faux. La vraie fermeture (une entrée `de`) est en pile C : elle demande un locuteur. |
...
| A11 | **OUVERT — bloqué par une frontière de fichier (constaté 2026-07-27).** Vérifié réel, et le geste est bien d'une ligne : signature `propose(m, element, decor?)`, `element` requis, suppression de `?? m.elements[0]`. Mesuré à `tsc` : **22 erreurs, dont 21 dans `lib/loop` (`propose.test.ts`, `phrase.test.ts`, `driver.ts`) et UNE hors périmètre — `lib/delivery/adapters/wepublish-e2e.test.ts:106` (`propose(run)` → `propose(run, el)` ; `el` y est déjà lié ligne 104)**. Non fait pour ne pas écrire dans `lib/delivery`. À reprendre par qui possède ce répertoire, ou en un seul passage. ~~**`propose` garde le repli `m.elements[0]`.**~~ Le driver passe désormais l'élément vivant, mais le paramètre reste optionnel — deux sources pour « l'élément vivant », dont une atteignable. | `lib/loop/propose.ts:45-55` · `lib/loop/driver.ts:123` | une ligne (rendre le paramètre requis) | Une seule définition de l'élément vivant. Le fallback ne sert plus personne depuis que le seul appelant le passe. |
