...
| ~~A3~~ **FERMÉ (2026-07-27)** | ~~**`.splash-preflight.json` garde deux écrivains** : le script legacy écrit le fichier pendant que la page écrit `lastVerified` dans le décor. « Un champ, un domicile » est la règle §3.1 du Préflight, et c'est la classe de défaut que ce dépôt a déjà payée.~~ **Fermé dans l'ordre contraint — l'écrivain d'abord, la lecture conservée.** (1) **L'écrivain est retiré, pas déplacé** : `preflight.mjs` ne persiste plus rien. Écrire son tri-état dans `lastVerified` aurait été le *mauvais* déménagement — ce champ enregistre ce qu'un fournisseur a RÉPONDU (`verify.ts`, seule la page de setup le fait), et un `preflight.mjs` qui ne sonde que la présence d'une variable l'aurait écrasé à chaque run, effaçant un vrai verdict `rejected`. Mesuré sur la branche : le test de non-régression a capturé le CLI en train de récrire un tampon `green` en `yellow`. La moitié env/deps du fait, elle, est re-dérivée à chaque lecture par `lib/newsroom/readiness.ts` depuis le même manifeste, et le fichier persisté n'avait **aucun lecteur** hors l'absorption unique — un cache que rien ne lisait. Le flag `--project`, qui n'existait que pour choisir où écrire ce fichier, part avec lui (seuls ses propres tests le passaient). (2) **La lecture reste** (`migrate-decor.ts`) : une install jamais passée par la page n'a que ce fichier comme trace. (3) **Le fichier est retiré au seul moment où le décor tient démontrablement le même fait** : `install/preflight/server.ts` le supprime juste après avoir écrit le `newsroom.json` où ses tampons verts viennent d'être absorbés — exactement le geste déjà prouvé pour `.splash-runtime`, et testé dans les deux sens (le tampon `2026-07-01` survit dans `newsroom.json`, le fichier disparaît). Tests : `skills/splash/tests/preflight-cli.test.ts` (le CLI n'écrit aucun fichier ; un fichier legacy présent est laissé **byte-identique**), `install/preflight/server.test.ts` (absorbe puis retire). | `skills/splash/scripts/preflight.mjs` · `install/preflight/server.ts:167-190` · `lib/newsroom/migrate-decor.ts` | petit | Un fait, un domicile — et pas au prix d'un faux verdict de fournisseur. Écrivains de `.splash-preflight.json` : **zéro**. |
...
| A11 | **FERMÉ (2026-07-27), dans la tranche qui possède `lib/loop`** (`fix/res-a11`). Signature réordonnée en `propose(m, element, decor?)`, `element` **requis**, `?? m.elements[0]` retiré : l'élément vivant a une seule définition, l'argument. Le driver (`driver.ts:156`) garde déjà sur `live` avant l'appel, et un run à `elements` vide est routé vers `confirm-angle` — le repli ne couvrait donc aucun chemin réel. **Compte réel : 21 sites d'appel** touchés à la main (1 production + 20 en tests) : `propose.test.ts` ×14 · `phrase.test.ts` ×3 · `driver.ts` · `video-e2e.test.ts` · `delivery-genre-e2e.test.ts` · `lib/delivery/adapters/wepublish-e2e.test.ts`. **La mesure d'origine était juste sur le fond, trop haute sur le détail** : « une quinzaine » d'arguments-expressions → **10 réels**, dont **7 seulement** ont demandé de hoister un local (`propose(run([...]))`, `propose(run())`, `propose(angled(...))`) ; les 3 autres (`propose({...m, channel})`, `propose({...m, orient: undefined})`) avaient déjà le manifeste en portée et n'ont demandé qu'un second argument. Corrigé au passage : le commentaire de `channel-single-reader.test.ts` qui nommait `propose()` comme le demandeur du cas « aucun élément » — c'est faux depuis ce changement, le résolveur total répond désormais pour les lecteurs read-only seuls. Vérifié : `bunx tsc --noEmit -p lib/tsconfig.json` propre · `bun test lib` 1447 pass / 1 fail, l'échec (`lib/host/journey.test.ts`, « no buildable form … engine's dependencies ») étant mesuré **identique avant le changement**. ~~**`propose` garde le repli `m.elements[0]`.** Le driver passe désormais l'élément vivant, mais le paramètre reste optionnel — deux sources pour « l'élément vivant », dont une atteignable.~~ | `lib/loop/propose.ts:45-55` · `lib/loop/driver.ts:123` | une ligne (rendre le paramètre requis) | Une seule définition de l'élément vivant. Le fallback ne sert plus personne depuis que le seul appelant le passe. — ~~**Tenté puis ANNULÉ, avec sa mesure.** La forme exacte est connue : `propose(m, element, decor?)`, `element` requis, retirer `?? m.elements[0]`. Mais l'estimation « une ligne » était fausse deux fois : elle valait depuis la frontière du lot boucle, pas depuis le dépôt. Mesuré ici : la signature actuelle est `(m, decor?, element?)`, donc un requis ne peut pas suivre un optionnel — il faut RÉORDONNER, ce qui touche **~20 sites d'appel**, dont une quinzaine à arguments-expressions dans les tests (`propose(run([...]))`, `propose({...m, channel})`, `propose(angled(...))`) qu'aucune substitution mécanique ne couvre. **Bénéfice réel : modeste** — le repli `elements[0]` n'est jamais exercé en production, le driver passe toujours `live`. C'est une redondance, pas un bug. À faire dans une tranche qui possède `lib/loop` en entier, pas en queue de vague.~~ **Raison levée** : la tranche a existé, le réordonnement a été fait à la main. Le bénéfice reste ce qu'il annonçait — modeste, une redondance de moins, pas un bug corrigé. |
...
| ~~A20~~ **FERMÉ** | ~~**`sourceQuestion()` n'a aucun appelant.**~~ **Moitié fermée (2026-07-27) : elle en a un, `lib/loop/produce.ts`, et le refus PORTE désormais la question** (ce que le R5 de la spec source-wiring affirmait déjà alors que le message ne la portait pas). `null` ⇒ pas de rembourrage : une donnée `synthetic` en run réel a une déclaration complète et se corrige par une décision, pas par une réponse. Preuve : `lib/source/produce-question.test.ts` (3 cas par `produce()` réel). **Moitié ORDRE : ouverte, bloquée par la frontière de fichiers — et la relecture du code a corrigé la note ci-contre.** La place propre n'est PAS un `NextAction` de CADRAGE : `run.sources` est écrit **une seule fois**, par `initRun` (`lib/loop/init.ts:114-161`, champ `sources` de `RunDeclarationSchema`), et **aucune étape ultérieure ne peut l'ajouter** — un run initialisé sans ledger est définitivement coincé au produce, sans porte de sortie. Donc la question appartient au moment où la **déclaration de run** se compose, avant que le run existe. **Changement exact, dans l'ordre :** (1) `lib/host/drive.ts:162` — avant `initRun`, si `declaration.input.data` est présent et `declaration.sources?.data` absent, refuser en rendant `sourceQuestion(undefined)` comme question de l'hôte ; symétrique pour un `data` déclaré mais incomplet (`sourceQuestion(declaration.sources.data)`) ; (2) `lib/loop/init.ts` — rendre `sources.data` **requis** quand `input.data` est déclaré (aujourd'hui `optional`), pour que la règle vive dans le verbe et pas seulement dans la façade ; (3) `lib/newsroom/ui-copy.ts` — la copie fr/de/it de la question (`sourceQuestion` rend de l'anglais, R7). Les trois fichiers sont hors du périmètre de cette tranche. **MOITIÉ ORDRE FERMÉE (2026-07-27, `fix/res-a20`) — les trois changements, dans l'ordre annoncé.** (1) `initRun` refuse un `input.data` sans `sources.data` **avant d'écrire un octet** (étape 3 de son ordre d'opérations, entre le refus d'écrasement et le jugement du ledger) et le refus PORTE la question en anglais — la règle vit dans le verbe, donc tout appelant l'obtient, pas seulement la façade. `sources` reste `optional` au schéma **exprès** : un run article-seul n'a légitimement pas de ledger de données ; ce qui est requis est l'APPARIEMENT, et un appariement n'est pas un champ (une contrainte de schéma aurait refusé le run article-seul). (2) `initRunIn` (`lib/host/drive.ts`) pose la même question **avant `initRun`**, dans la langue de la rédaction — et seulement sur une déclaration que le schéma strict ACCEPTE, sinon un `chanel` mal tapé recevrait « d'où viennent ces données ? » au lieu de son diagnostic nommé. (3) `sourceQuestion(decl, copy?)` prend désormais sa copie en paramètre (défaut anglais = R7 intact, `EN_SOURCE_QUESTIONS`) et `lib/newsroom/ui-copy.ts` ajoute fr/de/it : **une seule table de branches** décide de ce qui manque, quatre vocabulaires le disent. Les ids de kind (`public`/`local`/…) ne sont pas traduits — ils retournent dans la déclaration. 2 fixtures amenées au contrat (`lib/loop/init.test.ts` : le cas « chemin inexistant » déclare sa source pour atteindre la porte qu'il teste ; `lib/host/drive.test.ts` en ajoute une propre) — aucune assertion affaiblie, cf. `2026-07-27-proofs-run-design.md` §2. Preuves : `bun test lib` 1459 pass / 1 fail (`journey.test.ts` full-e2e, échec d'environnement identique avant/après : moteur non installé dans le worktree) · `bunx tsc --noEmit -p lib` propre. | `lib/source/policy.ts:124` (question + copie EN) · appelants : `lib/loop/produce.ts` · `lib/loop/init.ts` (règle) · `lib/host/drive.ts` (question localisée) · `lib/newsroom/ui-copy.ts` (fr/de/it) | moyen | Le coût du refus se déplace de l'auteur du run vers le bon moment du parcours. |
...
| ~~A33b~~ **FERMÉ (2026-07-27)** | ~~**Les 14 types Family-B ne sont pas thémables du tout** — `ArcChart`, `CalendarChart`, `CandlestickChart`, `ChordChart`, `ComboChart`, `GanttChart`, `LorenzChart`, `MarimekkoChart`, `ParallelChart`, `PictogramChart`, `RadarChart`, `SankeyChart`, `StreamgraphChart`, `SunburstChart` ne déclarent pas `themeBg`, n'appellent pas `themeColors` et peignent les `COLORS` clairs en dur. Une rédaction à fond sombre ou à couleur maison n'obtient rien d'eux. Isolé en fermant A33 (ce n'est pas la même classe : là il manque la couture entière, pas une de ses deux moitiés). Ils sont aussi les 14 types **absents du mapper** (`spec-to-config.ts` en construit 27), donc aucun n'est atteignable par le flux aujourd'hui — ce qui explique le trou et en borne l'urgence.~~ **Fermé : les 14 sont câblés, la liste gelée est VIDE.** Même couture que les 27 autres, sans en inventer une seconde — `themeBg` + `baseColor` sur la config, un seul `themeColors(fond, teinte)` par corps de plot, la paire transmise à `<ChartFrame>`, `tooltipBorder(themeBg)` sur le panneau. Les MARQUES ne bougent pas : chaque type de cette famille encode par palette catégorielle, par rôle (haussier/baissier, gauche/droite d'axe) ou par rampe séquentielle — une teinte maison unique y écraserait ce qu'elles séparent, donc `baseColor` y est une teinte de FURNITURE et rien d'autre (règle déjà écrite dans `house-hue-furniture-parity`). **Trois choses que la note ne voyait pas.** (1) `ParallelChart` peignait ses polylignes de CONTEXTE en `COLORS.muted` depuis le composant de TÊTE, pas depuis le corps du plot — une marque qui est un gris de furniture, invisible à un balayage du corps ; elle dérive maintenant du fond (convention `SlopeChart`). (2) `RadarChart` et `SankeyChart` posent un HALO de texte (`paintOrder="stroke"`) codé `#fff` : sur un fond sombre c'est un pâté blanc derrière un texte quasi-blanc, donc ça fait partie de la couture couleur — passé au fond dérivé, ce que font déjà boxplot/beeswarm/bullet/radial-bar (`stroke={C.bg}`). (3) La légende de `RadarChart` est un composant FRÈRE du corps de plot : l'encre résolue y est transmise en prop plutôt que re-dérivée. **Garde-fou mécanique élargi, pas seulement satisfait** : `dark-theme-a11y` balaie désormais les 41 composants de base (27 avant) sur trois fonds — le préréglage sombre, un brun sombre arbitraire, un rose clair arbitraire — et échoue dur si un littéral de furniture clair survit ; le carve-out `#1A1A1A` in-mark passe de 3 à 6 types et reste exactement l'ensemble des importateurs de `labelInkOnFill`. **Vérifié au RENDU, pas au grep** (leçon gravée : un bundle single-file inline toute la registry de palettes) : radar · sankey · marimekko · calendar produits en `static` sur un fond maison bleu nuit `#12263A` avec teinte `#B3005E` — sankey/marimekko/calendar passent la porte `produce` COMPLÈTE (dont `snap-contrast` WCAG et `snap-label-fit`) et les PNG montrent le fond maison, l'encre dérivée, la furniture teintée mauve, et les marques intactes. **Deux constats mesurés, laissés hors périmètre.** (a) `radar` échoue `snap-contrast` (2 étiquettes d'anneau à 1,09:1 / 2,5:1) — **pré-existant, pas une régression** : le même produce sur le défaut CLAIR échoue les MÊMES deux étiquettes à 1,03:1 / 2,37:1. La cause est que les étiquettes d'échelle se posent sur les polygones translucides, pas le thème ; c'est une classe étiquette-sur-marque à traiter à part. (b) `CalendarChart` garde la rampe BLUES fixe de `calendar-geometry` là où `HeatmapChart` dérive la sienne du `baseColor` via `heatmapRamp` — une marque, donc hors de cette tranche, mais c'est le prochain grain si on veut la rampe maison sur le calendrier. **Ce qui n'est PAS résolu et ne doit pas être surclamé** : ces 14 restent absents du mapper (`spec-to-config` en construit 27), donc **aucun n'est atteignable par le flux aujourd'hui**. Cette tranche retire l'angle mort couleur pour qu'ils soient livrables sans re-travail, elle ne les rend pas atteignables. Gate : `bunx tsc --noEmit` propre, `bun test` 1446/0 dans `skills/chart-native`. | `skills/chart-native/src/{Arc,Calendar,Candlestick,Chord,Combo,Gantt,Lorenz,Marimekko,Parallel,Pictogram,Radar,Sankey,Streamgraph,Sunburst}Chart.tsx` | fait — moyen (14 composants × la couture `themeBg`+`baseColor`) | Le thème maison n'a plus d'angle mort : les 41 types natifs dérivent leur furniture du fond de la rédaction. La liste gelée de `house-hue-furniture-parity.test.tsx` est **vide** — ce n'est plus une place de parking : un nouveau type est thémé ou il ne ship pas. |
...
| ~~**La carte-scrolly : offerte, inconstructible.**~~ **FERMÉ pour la moitié câblage** (`feat/engine-assemblers`, 2026-07-28) : `map-native` et `scrolly` sont dans la table `ASSEMBLERS`, et `assembleScrolly` compose la piste du moteur hôte — un `nativeType` de `MAP_TYPES` part sur `map-native`. `LOOP_BUILDABLE_ENGINES` n'est plus une liste écrite à la main : elle est **dérivée** des clés de la table. **Ce qui reste bloqué** : un plan de beats écrit à la main sur une piste carte est refusé *loud* (`assemble/scrolly.ts:16-21` — la carte dérive sa propre marche via `deriveMapStory`), donc le map-scrolly *à beats rédigés* attend toujours le travail de beats de la seconde moitié de la branche article | Le travail de beats map-scrolly (moitié 2 de la branche article). La décision produit « modèle de facettes » n'est plus un prérequis du câblage. |
