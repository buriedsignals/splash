...
## Session 2026-07-29 — Family C : capability-and-validation, 24 tâches — ce que le système SAIT et DIT de ce qu'il peut faire, pas ce qu'il construit de neuf

Plan `docs/superpowers/plans/2026-07-29-family-c-capability-and-validation.md`, 24 tâches TDD, branche `feat/family-c-capability-and-validation`, worktree `splash-family-c`, base `main@47e83752`. **Ce que cette famille N'EST PAS (à dire en clair, spec § risque 5)** : elle n'écrit aucune capacité manquante — ni le tooltip Datawrapper par type, ni le top-N carte symbole, ni le clavier sur les marques WebGL. Elle porte sur ce que le système **SAIT** de sa propre capacité et **DIT** au journaliste — pas sur ce qu'il **TIENT** (construit) de neuf. Tant que la famille A (tenue à distance, non exécutée par ce plan) n'a pas livré, cette distinction reste réelle : un système qui déclare honnêtement ses limites n'est pas un système qui les a comblées.

**Phase 5 (tâches 13-18) rouvre le canal `social-vertical`** — fermé jusque-là parce que la garde `checkSymbolConformance` (écrite en juin, jamais appelée jusqu'à la tâche 17) laissait passer des cartes symbole dont le rayon pouvait dépasser le cadre 9:16 sans jamais être mesuré contre la vraie taille de canal. Corrigé : sol du sampler dérivé de `themeBg` (tâche 13), fenêtre de snap = taille réelle du canal via `CHANNEL_POLICY` (tâche 14), **échelle de contraste livrée : REVERTÉE** (tâche 15, voir ci-dessous), bande source alignée sur la bande titre (tâche 16), **`checkSymbolConformance` enfin branché dans `runProduceMapConformance`** — la mesure la plus importante du plan : retirer le bloc qui l'appelle fait rougir exactement 3 tests, reproduit indépendamment par l'reviewer OPUS à 3 reprises (block-removal, y compris après le threading du vrai `mediaSize` par canal qui a fermé une bande de faux-refus 169-270px sur social-vertical/feed, D-BB) — puis gouttière de légende mesurée sur tous les stops plutôt que sur la seule valeur max (tâche 18). **Portée réelle de ce branchement, à ne pas surlire** : sur une config qu'un run de journaliste produit vraiment, seules **3** des 9 règles symbole désormais alimentées peuvent effectivement lever un refus — `legendStops < 2`, `pointsWithData < 1` et `boundsNonEmpty`. Les 6 autres (`sizingMode`, `hasLegend`, `labeled`, `labelHasUnit`, `maxRadius` et les sous-règles de cadrage `format`/`checkMapFraming`) sont alimentées par des champs qu'aucune config réelle n'émet, donc leurs valeurs sont toujours les valeurs par défaut « conformes » — les 3 tests qui rougissent au block-removal utilisent des configs synthétiques portant ces champs. Le chiffre 3 prouve que le **site d'appel** est vivant, pas qu'une classe neuve de défaut réel est attrapée : le reste attend que ces champs soient réellement émis (le cas `mediaSize`→`checkMapFraming` est nommé en follow-up dans `CLAUDE.md`). **Risque 4 nommé en clair, non résolu par ce plan** : le canal est rouvert mécaniquement (la garde ne bloque plus à tort), mais **aucune passe de rendu n'a encore parcouru un vrai pin carré/portrait de bout en bout sur `social-vertical`** — c'est un risque réel qui reste ouvert, pas une case cochée.

**Tâche 15 — REVERTÉE après la revue finale : ce n'était pas une réparation de faux positif, c'était un relâchement WCAG non mesuré.** La tâche threadait `STATIC_DEVICE_SCALE = 2` dans `wcagMinContrast` au site d'appel de `skills/chart-native/scripts/snap-contrast.mjs`, et a été présentée ici comme « échelle de contraste livrée threadée dans les deux sens ». Mesuré par la revue : cela faisait passer TOUTE l'échelle typographique de chart-native (titre 22 / label 14 / axe 13 / source 12 px CSS, `tokens.ts:73-77`) au-dessus du seuil « grand texte » de SC 1.4.3 — donc **le plancher de contraste de chaque label statique tombait de 4,5:1 à 3:1**, seul le texte sous 12px CSS (que le design system n'émet jamais) gardant le plancher strict. La prémisse est fausse : `deviceScaleFactor` est un facteur de **résolution**, pas de taille perçue — un PNG ×2 est plus net, pas plus gros. Sur `social-vertical` (1080×1920 affiché sur un viewport téléphone de ~400px CSS) un label d'axe de 13px est perçu à ~9px, donc **plus petit** que son px de layout, et il était jugé comme s'il faisait 26px : faux d'environ ×3 dans le sens permissif, sur le canal qui a le moins de marge de lecture — celui-là même que la phase 5 rouvre. `lib/core/channel-policy.ts:54-58` dit déjà la même chose pour l'impression dans les mots du repo (« «Print-safe» is a density, not a word »), et la garde jumelle de map-native n'a jamais pris cette échelle (elle rend à `deviceScaleFactor: 1`), si bien que les deux gardes statiques du repo se contredisaient. **Reverté au site d'appel** (retour à la forme à 2 arguments) ; le paramètre optionnel de `wcagMinContrast` et ses tests unitaires sont conservés tels quels — l'arithmétique est juste, c'est l'application qui ne l'était pas. Restaurer le plancher strict ne casse rien : chart-native 1458 pass / 0 fail, `tsc` propre.

**Phase 6 (19-21) — la chaîne `feature-reach`** : `lib/core/feature-reach.ts` (registre pur, `measuredBy` obligatoire, validation paresseuse à la lecture) → map-native déclare ses limites mesurées (clavier sur l'interactif/scrolly, labels directs sur le symbole interactif) et map-dw cesse de promettre un « top-N » qu'aucun code n'implémente (la garde cite désormais la vraie limite `SYMBOL_LABELS_INTERACTIVE` de map-native) → la limite déclarée **voyage jusqu'à l'offre** (`Candidate.limits`/`OfferOption.limits`/**`FormOption.limits`**/`PhrasedOption.limitsAcknowledged`), prouvé structurellement séparé du rang (mutation readiness-swap : change la composition réelle de l'offre, `rank.test.ts` reste à zéro diff).

**Correctif de revue finale sur cette phase 6 : la chaîne s'arrêtait un saut avant le journaliste.** `buildOffer` posait bien `limits` sur son `OfferOption` et `offer.test.ts` le prouvait là — mais le SEUL chemin de production qui montre une offre à un journaliste est `propose()` → le `FormOption[]` persisté dans `run.json` → `offerFromManifest()` qui reconstruit l'`Offer` pour `verifyOffer`, et les trois le perdaient : `propose()` et `phrase.ts` reconstruisent l'option champ par champ sans porter `limits`, et `FormOptionSchema` n'avait aucune clé pour elle — donc zod l'aurait de toute façon effacée à la persistance. Trois conséquences, toutes fermées ici : le journaliste ne lisait jamais la limite clavier ; la garde `limitsAcknowledged` de `verifyOffer` **ne pouvait pas se déclencher en production** (`option.limits` y était toujours `undefined`) — exactement la classe de défaut que ce plan existe pour fermer, une garde écrite, testée contre un objet synthétique, jamais câblée au chemin réel ; et un desk qui suivait la règle 5 de `SKILL.md` en posant `limitsAcknowledged: true` se faisait refuser durement pour avoir reconnu une limite que la persistance venait d'effacer. Fix = trois portages d'une ligne (`lib/loop/propose.ts`, `FormOptionSchema` dans `lib/loop/manifest.ts`, `offerFromManifest` dans `lib/loop/phrase.ts`) + une paire de tests qui parcourt la VRAIE chaîne (`propose()` → `parseManifest` → `applyPhrasing`) au lieu d'appeler `buildOffer`+`verifyOffer` en direct — c'est précisément ce raccourci qui rendait le défaut invisible. **Mutation-prouvé** : retirer n'importe lequel des trois portages fait rougir les nouveaux tests.

**Phase 7 (22-23)** : l'avertissement narratif nomme le sibling concret du run courant au lieu d'un renvoi générique (`narrativeSiblingOf`, dérivé mécaniquement des catalogues, jamais une table tenue à la main) ; un refus tardif (contraste rendu, hors du contrôle config-time) doit à la fois **dévier** (proposer un prochain pas actionnable) et **être enregistré** (`late-refusals.jsonl`) — fermé pour chart-native (OUTDIR threadé aux deux sites d'appel des snaps contraste, tâche 23 + fix round 1) ; **map-native reste volontairement non-threadé** (son snap écrit un PNG de debug que `collectOutputs` ramasserait avec `static.png`, exactement l'incident `assertFileMedia` de la tâche 7 — nommé en follow-up ci-dessous, pas traité ici).

**Phase 4 (9-12)** : une affirmation fausse de la KB retirée (le baseColor teinte déjà la rampe heatmap, mesuré dans le code, pas cru sur parole) ; `HeaderSchema`/`TypeSheet` gagnent un état « aucun moteur n'atteint ce type » explicite (`unreachable?`) plutôt qu'un `engines: {}` ambigu ; la garde DRIFT ferme non pas 1 mais **11 fiches KB** déclarant un `engines:` vers un type `deferred` — le brief prévoyait 1 entrée, un sweep complet en a trouvé 11 réelles, toutes corrigées avec le vrai motif `deferred:` de `native-types.ts`, jamais inventé (D-AA) ; parité doc↔garde verrouillée pour `guardrails.md`.

**Phases 1-3 (1-8)** : le champ `accent` mort (capturé, jamais lu — 6 composants) retiré de bout en bout ; le CLI scrolly valide sa config AVANT de builder (guard comportemental, un CLI réel lancé sur une config malformée — pas une garde textuelle, celle-là aurait pu être satisfaite par un commentaire) ; `source` exigée et typée côté dw-chart ; parité de furniture (titre/source) étendue à dw-chart ; un spec `deferred` (chart-native family-B) est refusé au spine AVANT `validateByProducer`, pas seulement recalé plus tard par le moteur natif ; un scrolly qui enveloppe un `nativeType` deferred avertit désormais au lieu de rendre silencieusement `ok:true`.

**Le gate mesuré, honnêtement (pas arrondi).** Deux passes complètes du gate (`bun run check`, 22 checks : 9 `tsc --noEmit` + 13 `bun test`), seule dans le worktree, selon la règle du plan (« deux tentatives maximum, puis STOP, ne jamais relâcher une garde pour verdir »). **Passe 1 : 20/22** — `lib` (2 échecs : le couple ambiant documenté `readiness.ts:54` + un timeout `capture-html.test.ts`, contention connue) et `skills/map-dw/src` (1 échec, un e2e live-Datawrapper à 300s). **Passe 2 : 21/22** — `map-dw/src` repasse au vert (confirmant que la passe 1 était un pic de latence réseau transitoire, pas une régression : re-testé isolément avec le vrai token, 149 pass/0 fail en 36,93 s). **Le seul échec resté sur les deux passes** : `lib/brain/eligibility.test.ts:486-492`, « a mark can never carry an empty reason, even for a capability disabled with no reason (readiness.ts:54) » — le test filtre tous les candidats chart-native non-scrolly porteurs d'une marque de disponibilité et exige que CHAQUE raison matche `/Charts built in-house/`, mais une raison de disponibilité légitimement différente (« a dot strip plot cannot be shipped as a video yet… ») passe le même filtre sans matcher ce regex précis. Une assertion de test trop étroite, préexistante — documentée comme échec STABLE de baseline dès les tâches 9/10 de ce même plan, **avant** que la tâche 21 (dernière à avoir touché ce fichier) n'existe. Hors-sujet de tout ce que les 24 tâches ont changé. Ni un défaut de code, ni une garde relâchée pour verdir — un gap nommé, pas caché.

**Follow-ups nommés, transcrits sans les traiter** (détail + preuves : `CLAUDE.md` § backlog, entrée 2026-07-29) : `accent` option (b) si un besoin réel apparaît · `lateRefusalSentence`→`routed()` le jour où la famille A livre `lib/core/routed-refusal.ts` · le tooltip par type manquant chez dw-chart (mesure live requise, hors périmètre) · le calque DOM parallèle aux marques WebGL (D05, un chantier) · les 3 copies de listes de types en prose jamais comparées aux catalogues · re-runner le sweep · `wrapLabel` qui ne coupe que sur l'espace (« Saint-Étienne » tronqué) · **map-native n'enregistre toujours aucun refus tardif** (chart-native oui désormais) — cause + forme de fix nommées · le compte de règles numérotées de `SKILL.md` sans garde-fou mécanique (déjà rattrapé une fois par lecture humaine) · un 5ᵉ doublon `MAX_RADIUS_PX` dans `ScrollySymbolMap.tsx` · le cas-limite à 1,25px de la tâche 17 · `import.meta.main` pour `produce.mjs`, qui fermerait toute la classe des gardes textuelles de ce plan.
...
Preuve live (commit `7a4970c`) : un vrai MinIO sous colima, un vrai `produce()` via `propose()` (canal `social-feed`, format `static`), publié puis re-fetché — `GET` 200, `content-type: image/png`, 29450 octets, magic number PNG (mesuré via un script de sonde non committé, qui comparait aussi `deliveredProvenanceHash` au hash de l'artefact). `docs/splash/proposal-brain-followups.md` : le résidu cloudflare-pages est marqué fermé (barré) — pas comblé par un adressage `${url}/index.png`, retiré par construction (l'artefact non-HTML n'atteint plus jamais ce chemin).

**Revue finale (2026-07-26)** : le test committé (`lib/loop/delivery-genre-e2e.test.ts`) n'assertait pas la comparaison de hash ci-dessus — seule la sonde non committée le faisait. Corrigé par une assertion d'égalité d'octets entre le fichier servi et l'artefact produit sur disque, ajoutée au test committé : c'est le contrôle à vraie valeur (`deliveredProvenanceHash` est quasi tautologique côté `deliver()`, calculé du même objet dans le même appel que le garde-fou de fraîcheur qui le précède). Non rejouée en live dans cette passe — le conteneur MinIO utilisé pour la preuve ci-dessus a été démonté ; `bun test` sans `SPLASH_S3_E2E=1` confirme seulement que le test skip proprement et compile.
...
**T3 (CI harness + fix `sandbox.ts:305`) + T2 (lane render CI) — parallélisés (repos distincts).**
- **T3** (repo `../splash-harness`, master `06d8aa1`, NON poussé — push harness = décision Rémy) : le SEUL vrai bug non-gaté de l'audit. `src/sandbox.ts:305` `links.map(l => l.worktreeDir)` — `worktreeDir` n'existe pas sur `NodeModulesLink` (`{source,link}` seul) → `linkedTargets` tout-`undefined` → `realListUnlinkedSkillDirs` (teste `linked.has(nm)`, `nm=join(dir,node_modules)`) ne matchait jamais (masqué par le fallback `existsSync(nm)`). Corrigé `l.link`. **2ᵉ bug indépendant démasqué** : l'agent (scopé src+CI) a flaggé sans y toucher que le mock `tests/sandbox.test.ts:553` passait un chemin ABSOLU+suffixé au lieu d'un dir RELATIF (contrat réel prouvé par la parité `planNodeModulesLinks`) → `join` doublait `node_modules`. Corrigé. Suite **381/1 → 382/0**. **CI harness créée** (`.github/workflows/ci.yml`, `bun install`+`bun test` push/PR) — fin du « test rouge qui se cache faute de CI ».
- **T2** (branche `feat/t2-render-ci`, mergé main, poussé, gate 22/22) : le render n'était JAMAIS certifié en CI. 2 jobs ajoutés à `.github/workflows/ci.yml` : **`render-chart`** (toujours, sans clé ; smoke `produce.mjs bar … static` → assert PNG >1KB, re-prouvé local 47991 bytes ; + `bun run check:render` self-skip vert sans clé) · **`render-map`** (gaté `if: secrets.MAPTILER_KEY != ''` → skip vert sans secret ; `VITE_/REMOTION_MAPTILER_KEY` câblés). `check` intact.
- **Parallélisation propre** (leçon slice-2 appliquée) : 2 agents, 2 REPOS distincts = zéro conflit. Suivi : provisionner secret `MAPTILER_KEY` du repo pour activer `render-map` ; pousser splash-harness master (décision Rémy).



**S4b-2a — matérialiseur de cas (recombine)** (moitié autonome de S4b-2 ; repo `../splash-harness` master `0559aee`→`f306a0f`, 7 commits, review finale opus MERGE + Important corrigé+prouvé, suite 434/0, NON poussé). Spec/plan `2026-07-22-s4b2a-*`. Transforme les 66 cell-specs de S4b-1 en cas harness runnables en RECOMBINANT les fragments des 105 cas existants (stratégie choisie par Rémy).
- **Archi** : `src/materialize/source-pick.ts` (`pickSource` : source family-match, préfère même-langue, déterministe, null si family absente) · `src/materialize/recombine.ts` (assemble article+data+persona+expect(+newsroom-profile.md si themed) ; canal/format = *demande* persona+expect pas contenu → atteint video/scrolly ; langue = traduction `claude -p` injectée si langue-cible ≠ source ; thème = fixture profil) · `scripts/materialize-cells.mjs` (CLI + traducteur réel, écrit `cases/gen-*` gitignorés + rapport).
- **Invariant d'honnêteté = `extractAxes(cas matérialisé) == cell`** (round-trip test Task 3). **Le CLI SELF-VÉRIFIE** chaque cas écrit et DÉMOTE les dérives en un-materializable (fix final-review).
- **La boucle review a attrapé 3 vrais problèmes** : (1) Important — Task 1 réutilisait `CaseAxes` au lieu d'exporter `Cell`/`SourceCase`+`dir` (cassait le contrat Task 2) → corrigé ; (2) le round-trip Task 3 a attrapé un bug réel — `extractAxes` détecte le thème par le blob JSON (pas le fichier profil) → thème toujours `default` ; corrigé (recombine signale via `expect.hint` « newsroom », convention corpus réelle) ; (3) **Important review finale** — l'invariant fuit sur données RÉELLES : certaines sources portent « newsroom » dans un redLine → un cas `default` extrait `themed` (dérive silencieuse ; le test-stub le masquait). Corrigé par le self-verify CLI ; re-run a démoté **3 dérives réelles dont 2 que la review n'avait pas trouvées** (17/20 gardés, tous matchant).
- **Faux positif review réfuté** : « les cas themed ne seraient pas brandés (profil lowercase vs NEWSROOM-PROFILE.md uppercase) » — FAUX, le driver `driver.ts:513` copie `newsroom-profile.md`→`NEWSROOM-PROFILE.md` au runtime (corollaire « la review peut se tromper »).
- **Acceptance prouvée** : 3 cas matérialisés end-to-end (1 fr→de traduit ~12s), extractAxes 3/3. **PROCHAIN S4b-2b** = générer les 66 + les FAIRE TOURNER (spend event, pilote d'abord, Rémy).

**S4b-1 — coverage analyzer + covering-array cell generator** (moitié autonome/zéro-spend de S4b ; repo `../splash-harness` master `8bec831`→`9a64996`, 8 commits, review finale opus MERGE + Important corrigé, suite 416/0, NON poussé). Spec/plan `2026-07-22-s4b1-*`. Répond CHIFFRÉ à l'audit §6 « matrice re-run vs vrai stress-test ».
- **Livré** : CLI standalone `bun scripts/coverage.mjs` sur les 105 cas → **34,2% couverture pairwise de l'espace canonique réel** (91/266 paires valides), **175 trous**, **66 cell-specs** à ajouter. **video/scrolly JAMAIS testés** (surfacés comme trous). channel clusterisé (article-web 93, social-vertical 10, social-feed 2). format non-pinné 98/105 (`expectFormat` absent) → tally non-pinnable honnête.
- **Archi** : `src/coverage-axes.ts` (extraction : normalizeChannel free-text→canonique, detectLanguage heuristique stopwords sans-dép, extractAxes ; table contrainte `CHANNEL_FORMAT_ALLOWED` miroir `channel.ts` lock-testée) · `src/coverage.ts` (couverture pairwise contrainte + trous + tally non-pinnable ; domaines CANONIQUES pour channel/format/language/theme → une valeur valide absente = trou, pas invisible ; sentinelles exclues de la couverture réelle) · `src/covering-array.ts` (générateur IPOG **seed-driven** : chaque ligne seed une paire non-couverte → progrès garanti → couvre prouvablement chaque paire valide ; fail-loud sur domaine infaisable) · `scripts/coverage.mjs` (CLI, output `coverage/` gitignoré).
- **La boucle review a attrapé 3 bugs réels** (valeur du dispositif) : (1) **Critical** — le greedy IPOG du plan sous-couvrait SILENCIEUSEMENT dès qu'un axe a >2 valeurs (le test all-binary le masquait ; réel : 16-family → 13 paires manquantes) → réécrit seed-driven, prouvablement complet ; (2) **Important** — le fallback de remplissage émettait une cellule violant la contrainte sur domaine infaisable → fail-loud (throw) ; (3) **Important (review finale)** — domaines observés-seuls rendaient video/scrolly INVISIBLES + les sentinelles gonflaient le % → domaines canoniques + exclusion sentinelles = le vrai **34,2% honnête** (vs 49,8% trompeur du 1er jet). + Minor feed-vs-instagram normalizer corrigé.
- **Leçon** : le corpus de cert couvre à peine un tiers de l'espace combinatoire réel, et 2 formats (video/scrolly) sont totalement non-testés — l'audit avait raison, maintenant PROUVÉ chiffré. **PROCHAIN S4b-2** = materialize+run les 66 cellules (spend-gated, Rémy).

**S4a — rubrique flow-process A1-A8** (capstone S4 décomposée ; repo `../splash-harness` master `e276423`→`3b588ce`, 5 commits, review finale opus MERGE 0 crit/imp, suite 389→397/0, NON poussé). Répond au #1 trou de l'audit « rien testé du flow ». Spec/plan : `2026-07-22-s4a-flow-rubric-*`.
- **Grounding = demi-vérité** : la plupart des garde-fous flow DÉTERMINISTES existaient déjà comme `check:*` (A2=gate-discipline, A4=single-proposal+secondary-dropped, A5=render-shown-before-validation, A8=real-system+stray-tool-call+skills-invoked+suggester-invoked) — jamais organisés en rubrique A1-A8 ni gatés en CI (T3). Donc S4a = consolider + combler 2 trous + router le sémantique au juge, PAS construire A1-A8 de zéro. (Leçon « audit gaps souvent déjà faits » — 5ᵉ fois.)
- **`src/flow-rubric.ts`** : `FLOW_RUBRIC` data-only mappe chaque A-item→{id,label,gate,kind,checks} + **drift-guard** (`tests/flow-rubric.test.ts` : chaque check:* déterministe existe vraiment dans checks.ts, tous les A1-A8 présents, hard-gates={A5,A8} — un check renommé/orphelin casse le test).
- **`check:takeaway-confirmed` (A7)** : miroir de l'ordering `check:render-shown-before-validation` (1re production, scan strictement avant). Signal PRIMAIRE = marqueur `confirmedTakeaway` dans un tool input (langue-indép, requis SKILL.md Gate 1b) ; fallback dialogue FR+EN (review a démasqué que l'AFFIRM_RE 1er jet était FR-only → élargi EN). Golden-fixtures vérifiées true-positive (good-session manque vraiment Gate 1b).
- **A3 = JUDGE-FALLBACK honnête** : `check:over-ask` (re-ask source) PAS shippé — l'implémenteur a prouvé qu'il false-positivait sur tout article nommant INSEE/Eurostat (vocabulaire quotidien du data-journalisme = flux Splash courant). Un check false-positif déguisé en mécanique = le théâtre exact que T4 a viré → A3 devient `kind:"judge"` sans backing check. Le fallback honnête du plan appliqué.
- **Section « Flow rubric A1-A8 »** rapport (`report.ts`) + agrégat suite (`suite-report.ts`, per-case `failedFlowItems` threadé dans `suite.ts`) : chaque A-item = id·label·gate·badge kind (mécanique✓ fiable / juge~ advisory)·status. Déterministe FAIL ssi un backing check a tiré ; **juge = advisory, JAMAIS pass** (réutilise le split T4). Hard-gates A5/A8 surfacés distinctement.
- **A1/A6 = juge** (sémantiques, non-mécanisables). Review opus a re-rendu le HTML pour vérifier judge-jamais-pass. Labels anglicisés (standard English-only). **PROCHAIN S4b covering-array** (autonome) ; **S4c κ / S4d gate humain** attendent Yvan/Rinny.


**T4 — reframe cert honnête** (repo `../splash-harness` master `0f293b4`+`e2058aa`, NON poussé — décision Rémy). L'audit §6 : la cert faisait « du théâtre » sur 2 axes → corrigés.
- **Déterministe vs juge-opinion** : `Finding.source` distingue mécanique (`check:*`, `src/checks.ts`, fiable) de juge-LLM (`"judge"`, faillible) mais les counts de sévérité les lumpaient (« 79% findings juge-authored » traités comme du mécanique). Ajout `SourceSplit{mechanical,judgeOpinion}` sur `SeverityCounts.{critical,major,minor}BySource` (`src/suite.ts`) + helper partagé `isDeterministicSource()` (`src/ledger.ts`) → rendu « critical 2 (1 mechanical · 1 judge-opinion) » dans le rapport HTML (`suite-report.ts`) ET la sortie stdout CLI (`run-e2e.mjs`, les 4 sites de print) : un critical mécanique = signal de gate fiable, un critical juge = advisory à adjuger humainement.
- **« delivered ≈ ça n'a pas crashé »** : le headline « X/Y delivered » (vert `--ok`) se lisait comme un pass de cert. Reframé « N/M produced an artifact (delivery = did not crash, NOT a pass verdict) » + disclaimer + CSS green retiré.
- 6 tests HTML (`suite.test.ts`/`suite-report.test.ts`) + 1 test pur `fmtSev` CLI. Suite harness **382 → 389 / 0**. **Discipline** : le CLI a été complété APRÈS coup (l'agent scopé HTML avait flaggé que `run-e2e.mjs` gardait le count flat — même théâtre sur une 2ᵉ surface) → fermé pour qu'aucune surface ne contredise le rapport honnête.
