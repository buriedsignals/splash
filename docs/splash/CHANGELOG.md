...
**Note deps worktree** : un worktree frais nécessite `bun install` par-skill (dw-chart/scrolly playwright/react, image-native sharp) + root (`@noble/hashes`, `bun-types`) + copie `.env` — sinon tsc/tests faux-échouent sur deps manquantes.
