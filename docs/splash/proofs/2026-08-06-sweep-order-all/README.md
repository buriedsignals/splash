...
| `locator-few-114` / `-177` | Empty Paris → the Eiffel Tower marker and its label. **Weak evidence by construction**: `deriveLocatorStory`'s few-annotated regime frames every reveal on `allBounds`, so this type's camera never moves after the title and identical framing proves nothing about the trigger. The trigger is the same `atHoldStart` opt-in as the other four, and the coverage test pins it in all six files. |
