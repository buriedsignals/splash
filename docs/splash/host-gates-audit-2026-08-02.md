...
| **F3** | **S1** | `sourceHint` est recopié à la main et son omission **désarme deux gardes de source** — le document l'écrit lui-même. Le filet est un warning non bloquant. | `SKILL.md:862-872` ; `skills/splash/src/source-guard.ts` | Faire écrire `sourceHint` par un script (miroir de `save-opportunities.mjs`, `suggest-article/SKILL.md:137`) et faire échouer `validateAccepted` sur son absence quand `opportunities.json` existe et nomme une source — c'est-à-dire exactement la logique `articleEvidence` que `export-code.mjs:236-239` applique déjà au placement. Le précédent existe dans le dépôt. |
