...
## ★ État courant — 2026-07-10 (LIS CECI pour l'état de `main`)
...
**PROCHAIN / backlog :** (Wave 4, follow-ups) carte map-native interactive **dégrade en statique** sur données quasi-globales (clamp a11y bounded-nav trop strict) · **titre qui diverge du takeaway confirmé** (Gate 1b — récurrent, règle non obéie ; lèvier mécanique = champ `confirmedTakeaway` + check) · **`suggest-chart` émet `world`+`ISO_A3`** (clé de jointure cassée, attrapée au produce mais à corriger à la source) · renderers symboles **vidéo/scrolly** encore en `text-variable-anchor` (même classe edge-clip que le fix statique) · vidéo produite mais run **closed-early** sans registration de livraison (harness) · (export-forme, follow-ups) **valider le flux conversationnel a/b/c par un run harness** (l'orchestrateur propose-t-il bien les 3 formes et livre-t-il la bonne ?) · bundle React runnable pour **map-native/scrolly** (aujourd'hui = dossier fichiers, leur `src` n'est pas auto-contenu → factoriser un core auto-contenu) · (mineur) `produce.mjs` écrit `config.json`+`native-source.json` à chaque produce (inerte ailleurs) · bundle `README` EN vs bloc de relais FR (cosmétique) · (nouveau, mineur, non-bloquant) dw-chart **statique** ne devrait pas mettre l'embed hébergé en forme 1 (la forme possédée doit mener — souveraineté) · consolider la réserve source (6 charts self-clearing double-comptent la réserve partagée — cosmétique) · vérifier le groupement de la légende map-dw **symbol** (non vérifié) · (existant) capture source (persona/article → prose = flow/orchestration) · Family B types natifs (15 déférés) · **release MIT** (confirmer REPO_URL public + scrub trailers `<vendor>-Session` — input Rémy + destructif, au pré-release) · harness QA privé = `../atelier-harness` (WORKFLOW.md · `scripts/deep-verify.mjs` · le driver juge le `-export`).
