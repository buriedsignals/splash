...
1. **conformance-au-produce — FAIT pour chart-native** (commit `ef362f6`) : `resolveConformanceColors` partagé + les 7 types couleur (line/bar/scatter/histogram/beeswarm/connected-scatter/lollipop) câblés dans `produce.mjs` → une violation **échoue le run avant de builder** (garde-fous à l'exécution, plus test-only). **★ A trouvé un vrai bug a11y live** : `OKABE_ITO.vermillion` (#D55E00) en **TEXTE** = 3.87:1 sur blanc (< WCAG 4.5:1), sur histogram + lollipop → produce **warn** (pas fail) dessus en attendant un fix. **RESTE conformance** : (a) **fixer le vermillon-texte** (décision design : couleur texte sûre pour les labels, OU créditer un halo dans le check — le check flag le texte quel que soit le halo aujourd'hui) ; (b) les **~34 autres types** (palettes bespoke non modélisées) ; (c) **parité map-native**.
