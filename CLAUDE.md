...
- **RESTE parité** : **Slice 2** (conformance format-aware : L0 partagé `checkGlobalMapConformance` + règles cadrage/lisibilité titre-non-sur-donnée/source-visible/rien-hors-champ, prend les dimensions) · **Slice 3** (harnais vérif : `snap-responsive.mjs` 360→1600 + `snap-a11y.mjs`) · **Slice 4** (KB : `map/design-conformance.md` + `map/types/choropleth.md`). Specs à écrire au fil.
